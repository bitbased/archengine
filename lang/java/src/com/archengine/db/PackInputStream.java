 ...
 */
package com.archengine.db;

import java.io.ByteArrayInputStream;
import java.lang.StringBuffer;
import com.archengine.db.PackUtil;
import com.archengine.db.ArchEnginePackingException;

/**
 * An internal helper class for decoding ArchEngine packed values.
 *
...
public class PackInputStream {
     ...
     * Constructor.
     *
     * \param format A String that contains the ArchEngine format that
    ...
    throws ArchEnginePackingException {
        ...
        if ((signed && (ret > Short.MAX_VALUE || ret < Short.MIN_VALUE)) ||
                (!signed && (short)ret < 0)) {
            throw new ArchEnginePackingException("Overflow unpacking short.");
        }
        return (short)ret;
    }
    ...
    throws ArchEnginePackingException {
        ...
        if ((signed && (ret > Integer.MAX_VALUE || ret < Integer.MIN_VALUE)) ||
                (!signed && (int)ret < 0)) {
            throw new ArchEnginePackingException("Overflow unpacking integer.");
        }
        return (int)ret;
    }
     ...
     * checking, to ensure values fit, since some values may be encoded as
     * unsigned values, and Java types are all signed.
     * The packing format is defined in the ArchEngine C integer packing
