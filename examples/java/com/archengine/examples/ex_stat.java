...
public class ex_stat {
        ...
        throws ArchEngineException
    {
        ...
        return (ret == archengine.AE_NOTFOUND ? 0 : ret);
    }
    /*! [statistics display function] */
        ...
        throws ArchEngineException
    {
        ...
        cursor.putKeyInt(archengine.AE_STAT_DSRC_BTREE_OVERFLOW);
        ret = cursor.search();
        desc = cursor.getValueString();
        pvalue = cursor.getValueString();
        value = cursor.getValueLong();
        ...
        /*! [statistics calculate open table stats] */

            {
                ...
                long ckpt_size = get_stat(cursor,
                    archengine.AE_STAT_DSRC_BLOCK_CHECKPOINT_SIZE);
                long file_size = get_stat(cursor,
                    archengine.AE_STAT_DSRC_BLOCK_SIZE);
...

                {
                    ...
                    long app_insert = get_stat(cursor,
                        archengine.AE_STAT_DSRC_CURSOR_INSERT_BYTES);
                    long app_remove = get_stat(cursor,
                        archengine.AE_STAT_DSRC_CURSOR_REMOVE_BYTES);
                    long app_update = get_stat(cursor,
                        archengine.AE_STAT_DSRC_CURSOR_UPDATE_BYTES);

                    long fs_writes = get_stat(cursor,
                        archengine.AE_STAT_DSRC_CACHE_BYTES_WRITE);
