...
class AsyncKeys implements AsyncCallback {
    ...
    public int notify(AsyncOp op, int opReturn, int flags) {
        ...
        try {
            ...
            if (optype == AsyncOpType.AE_AOP_SEARCH) {
...
public class ex_async {
    ...
    public static AsyncOp tryAsyncNewOp(Connection conn, String uri,
        String config, AsyncCallback cb) throws ArchEngineException
    {
        ArchEngineException savedaee = null;
        ...
        for (int tries = 0; tries < 10; tries++)
            try {
                 ...
                 */
                System.err.println(
                    "asynchronous operation handle not available: " + aee);
                savedaee = aee;
                try {
                    Thread.sleep(1);
        ...
        throws ArchEngineException
    {
        ...
        conn = archengine.open(home, "create,cache_size=100MB," +
