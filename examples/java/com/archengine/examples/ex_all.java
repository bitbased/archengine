    ...
    throws ArchEngineException
{
    ...
    if (compare == 0) {
        /* archengine.iCursors reference the same key */
    } else {
    ...
    try {
        ...
        if ((ret = cursor.remove()) != 0) {
            System.err.println(
                "cursor.remove: " + archengine.archengine_strerror(ret));
            return (ret);
        }
    } catch (ArchEngineException aee) {  /* Catch severe errors. */
        System.err.println("cursor.remove exception: " + aee);
    }
    /*! [Display an error] */
    }
    ...
    throws ArchEngineException
{
    ...
    case 0:                    /* Update success */
         ...
         */
        break;
    case archengine.AE_ROLLBACK:            /* Update conflict */
    default:                /* Other error */
    ...
    throws ArchEngineException
{
    Connection conn;
    int ret = 0;
    String home = "/home/example/AE_TEST";

    /*! [Open a connection] */
    conn = archengine.open(home, "create,cache_size=500M");
    /*! [Open a connection] */
    ...
    conn = archengine.open(home,
        "create," +
        "extensions=[/usr/local/lib/libarchengine_bzip2.so]");
    /*! [Configure bzip2 extension] */
    conn.close(null);
    ...
    conn = archengine.open(home,
        "create," +
        "extensions=[/usr/local/lib/libarchengine_lz4.so]");
    /*! [Configure lz4 extension] */
    conn.close(null);
    ...
    conn = archengine.open(home,
        "create," +
        "extensions=[/usr/local/lib/libarchengine_snappy.so]");
    /*! [Configure snappy extension] */
    conn.close(null);
    ...
    conn = archengine.open(home,
        "create," +
        "extensions=[/usr/local/lib/libarchengine_zlib.so]");
    /*! [Configure zlib extension] */
    conn.close(null);
    ...
    conn = archengine.open(home, "create,direct_io=[data]");
    /*! [Configure direct_io for data files] */
    conn.close(null);
    } // if (false)

    /*! [Configure file_extend] */
    conn = archengine.open(
    ...
    conn = archengine.open(home, "create,statistics=(all)");
    /*! [Statistics configuration] */
    conn.close(null);
     ...
     * leak memory.
     */
    conn = archengine.open(home, "create");
