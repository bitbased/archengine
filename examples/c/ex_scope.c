...
static int
cursor_scope_ops(AE_CURSOR *cursor)
{
	struct {
		const char *op;
		const char *key;
	...
	for (op = ops; op->key != NULL; op++) {
		...
		if (op->apply == cursor->insert) {
			/*
			 * AE_CURSOR::insert no longer references application
		...
		if (op->apply == cursor->remove) {
			/*
			 * AE_CURSOR::remove no longer references application
			...
			/*
			 * AE_CURSOR::search and AE_CURSOR::update no longer
