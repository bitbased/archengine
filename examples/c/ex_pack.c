...
main(void)
{
	...
	ret = archengine_struct_size(session, &size, "iii", 42, 1000, -9);
	if (size > sizeof(buf)) {
		/* Allocate a bigger buffer. */
	}

	ret = archengine_struct_pack(session, buf, size, "iii", 42, 1000, -9);

	ret = archengine_struct_unpack(session, buf, size, "iii", &i, &j, &k);
	/*! [packing] */
	}
