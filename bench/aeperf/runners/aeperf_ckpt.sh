...
# A script for running the aeperf benchmark to analyze the performance
...
AEPERF=${BIN_DIR}/aeperf
if [ ! -x $AEPERF ]; then
	echo "Could not find or execute $AEPERF"
	exit 1
fi

DB_HOME="$ROOT_DIR/AE_TEST"
...
if [ $REUSE -eq 0 ]; then
	...
	$DEBUG $AEPERF $CREATE_OPTS || exit 1

	# Save the database so that it can be re-used by all runs.
	# I'd rather not hard code AE_TEST, but need to get the path right.
	rm -f $ROOT_DIR/AE_TEST.tgz
	tar zcf $ROOT_DIR/AE_TEST.tgz -C $ROOT_DIR AE_TEST
fi
...
for ckpt in "checkpoint_threads=1,checkpoint_interval=120"; do
	...
	for opts in ""; do
		if [ $VERBOSE -ne 0 ]; then
			...
			echo "\t$AEPERF $RUN_OPTS $ckpt $opts"
		fi
		res_name="run_${ckpt},${opts}"
		res_name=`echo $res_name | tr '[:upper:][=\- ,]' '[:lower:]_'`
		if [ $WORKLOAD -eq 0 ]; then
			rm -rf $DB_HOME && tar zxf $ROOT_DIR/AE_TEST.tgz -C $ROOT_DIR
		else
			rm -rf $DB_HOME && mkdir $DB_HOME
		fi
		if [ "$DEBUG" = '' ]; then
			$AEPERF $RUN_OPTS -o "$ckpt" -o "$opts" &
			pid=$!
			t=0
			...
			$DEBUG $AEPERF $RUN_OPTS $ckpt $opts
		fi
