...
# aeperf_track.sh - track performance regression trends on the Jenkins platform.
#
# This is run after a performance build, with a count or time value.
#    aeperf_track.sh -t [ -p percent ] name time_value
#    aeperf_track.sh -c [ -p percent ] name count_value
#
...
STATE_DIR="/home/jenkins/aeperf_track"
...
{
    cat >&2 <<EOF
Usage: aeperf_track.sh -t [ options ] name time_value
       aeperf_track.sh -c [ options ] name count_value
