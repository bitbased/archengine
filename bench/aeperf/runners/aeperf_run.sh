...
# aeperf_run.sh - run aeperf regression tests on the Jenkins platform.
#
# The Jenkins machines show variability so we run this script to run
# each aeperf test several times.  We throw away the min and max
...
# build_posix/bench/aeperf).
#
# This script should be invoked with the pathname of the aeperf test
# config to run and the number of runs.
#
if test "$#" -lt "2"; then
	echo "Must specify aeperf test to run and number of runs"
	exit 1
fi
aetest=$1
runmax=$2
...
outfile=./aeperf.out
rm -f $outfile
echo "Parsed $# args: test: $aetest runmax: $runmax args: $aearg $aearg2" >> $outfile
...
while test "$run" -le "$runmax"; do
	rm -rf $home
	mkdir $home
	LD_PRELOAD=/usr/lib64/libjemalloc.so.1 LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/lib ./aeperf -O $aetest $aearg $aearg2
	if test "$?" -ne "0"; then
		exit 1
	fi
	# Load is always using floating point, so handle separately
	l=`grep "^Load time:" ./AE_TEST/test.stat`
	if test "$?" -eq "0"; then
	...
	for i in ${!ops[*]}; do
		l=`grep "Executed.*${ops[$i]} operations" ./AE_TEST/test.stat`
		if test "$?" -eq "0"; then
			n=`echo $l | cut -d ' ' -f 2`
		else
			n=0
