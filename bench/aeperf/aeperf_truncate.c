...
int
setup_truncate(CONFIG *cfg, CONFIG_THREAD *thread, AE_SESSION *session) {
...
run_truncate(CONFIG *cfg, CONFIG_THREAD *thread,
    AE_CURSOR *cursor, AE_SESSION *session, int *truncatedp) {
	...
	*truncatedp = 0;
	...
	if (trunc_cfg->expected_total >
		...
		trunc_cfg->catchup_multiplier =
		    AE_MIN(trunc_cfg->catchup_multiplier + 1,
		    trunc_cfg->needed_stones - 1);
	} else {
		/* Back off if we start seeing an improvement */
		trunc_cfg->catchup_multiplier =
		    AE_MAX(trunc_cfg->catchup_multiplier - 1, 1);
	}
