...
static void	*thread_run_aeperf(void *);
static void	*worker(void *);
static void	worker_throttle(int64_t, int64_t *, struct timespec *);
static uint64_t	 aeperf_rand(CONFIG_THREAD *);
static uint64_t	 aeperf_value_range(CONFIG *);

#define	HELIUM_NAME	"dev1"
#define	HELIUM_PATH							\
	"../../ext/test/helium/.libs/libarchengine_helium.so"
...
{
	return (__ae_atomic_add64(&cfg->insert_key, 1));
}
...
randomize_value(CONFIG_THREAD *thread, char *value_buf)
{
	 ...
	 */
	i = __ae_random(&thread->rnd) % (thread->cfg->value_sz - 1);
	while (value_buf[i] == '\0' && i > 0)
		--i;
	if (i > 0) {
		vb = (uint8_t *)value_buf;
		vb[0] = (__ae_random(&thread->rnd) % 255) + 1;
		vb[i] = (__ae_random(&thread->rnd) % 255) + 1;
	}
}

static int
cb_asyncop(AE_ASYNC_CALLBACK *cb, AE_ASYNC_OP *op, int ret, uint32_t flags)
{
	CONFIG *cfg;
	...
	case AE_AOP_COMPACT:
		...
		(void)__ae_atomic_add32(tables, (uint32_t)-1);
		break;
	case AE_AOP_INSERT:
		trk = &thread->insert;
		break;
	 ...
	 *
	 * Reads and updates can fail with AE_NOTFOUND: we may be searching
	 * in a random range, or an insert op might have updated the
	 * last record in the table but not yet finished the actual insert.
	 */
	if (type == AE_AOP_COMPACT)
		return (0);
	if (ret == 0 || (ret == AE_NOTFOUND && type != AE_AOP_INSERT)) {
		if (!cfg->in_warmup)
			(void)__ae_atomic_add64(&trk->ops, 1);
		return (0);
	}
err:
	/* Panic if error */
...
static AE_ASYNC_CALLBACK cb = { cb_asyncop };
...
worker_async(void *arg)
{
	...
	while (!cfg->stop) {
		...
		case WORKER_UPDATE:
			 ...
			 * id.
			 */
			if (aeperf_value_range(cfg) < next_val)
				continue;
			break;
		default:
...
op_err:			lprintf(cfg, ret, 0,
			    ...
			    op_name(op), key_buf, aeperf_value_range(cfg));
			goto err;		/* can't happen */
		}
...
worker(void *arg)
{
	...
	TRACK *trk;
	AE_CONNECTION *conn;
	AE_CURSOR **cursors, *cursor, *tmp_cursor;
	AE_SESSION *session;
	...
	if ((ret = conn->open_session(
		...
		lprintf(cfg, ret, 0, "worker: AE_CONNECTION.open_session");
		goto err;
	}
	cursors = calloc(cfg->table_count, sizeof(AE_CURSOR *));
	if (cursors == NULL) {
	...
	for (i = 0; i < cfg->table_count; i++) {
		if ((ret = session->open_cursor(session,
			...
			lprintf(cfg, ret, 0,
			    "worker: AE_SESSION.open_cursor: %s",
			    cfg->uris[i]);
			goto err;
		}
	}
	...
	if (thread->workload->throttle != 0 &&
	    (ret = __ae_epoch(NULL, &interval)) != 0) {
	...
	while (!cfg->stop) {
		...
		if (measure_latency && (ret = __ae_epoch(NULL, &start)) != 0) {
			lprintf(cfg, ret, 0, "Get time call failed");
			goto err;
		}
		...
		case WORKER_READ:
			/*
			 * Reads can fail with AE_NOTFOUND: we may be searching
			...
			goto op_err;
		case WORKER_INSERT_RMW:
			if ((ret = cursor->search(cursor)) != AE_NOTFOUND)
				goto op_err;
...
op_err:			if (ret == AE_ROLLBACK && ops_per_txn != 0) {
				/*
				 * If we are running with explicit transactions
				 * configured and we hit a AE_ROLLBACK, then we
				...
				lprintf(cfg, ret, 1,
				    ...
				    key_buf, aeperf_value_range(cfg));
		...
		if (!cfg->in_warmup) {
			if (measure_latency) {
				if ((ret = __ae_epoch(NULL, &stop)) != 0) {
					lprintf(cfg, ret, 0,
					    "Get time call failed");
					goto err;
				}
				++trk->latency_ops;
				usecs = AE_TIMEDIFF_US(stop, start);
...
populate_thread(void *arg)
{
	...
	if ((ret = conn->open_session(
	    conn, NULL, cfg->sess_config, &session)) != 0) {
		lprintf(cfg, ret, 0, "populate: AE_CONNECTION.open_session");
		goto err;
	}
	...
	for (i = 0; i < cfg->table_count; i++) {
		if ((ret = session->open_cursor(
			...
			lprintf(cfg, ret, 0,
			    "populate: AE_SESSION.open_cursor: %s",
			    cfg->uris[i]);
			goto err;
		}
	}
	...
	for (intxn = 0, opcount = 0;;) {
		...
		if (cfg->random_value)
			randomize_value(thread, value_buf);
		cursor->set_value(cursor, value_buf);
		if ((ret = cursor->insert(cursor)) == AE_ROLLBACK) {
...
execute_populate(CONFIG *cfg)
{
	...
	if ((ret = __ae_epoch(NULL, &start)) != 0) {
	...
	if (cfg->compact) {
		...
		lprintf(cfg, 0, 1,
		    ...
		    (uint64_t)(AE_TIMEDIFF_SEC(stop, start)));
		assert(tables == 0);
	}
	return (0);
}
...
start_all_runs(CONFIG *cfg)
{
	...
	for (i = 0; i < cfg->database_count; i++) {
		...
		if ((ret = pthread_create(
		    &threads[i], NULL, thread_run_aeperf, next_cfg)) != 0) {
...
/* Run an instance of aeperf for a given configuration. */
static void *
thread_run_aeperf(void *arg)
{
	CONFIG *cfg;
	int ret;
...
start_run(CONFIG *cfg)
{
	...
	if ((ret = archengine_open(	/* Open the real connection. */
	    cfg->home, NULL, cfg->conn_config, &cfg->conn)) != 0) {
	...
	if (cfg->helium_mount != NULL) {
		snprintf(helium_buf, sizeof(helium_buf),
		    "entry=archengine_extension_init,config=["
...
extern int __ae_optind, __ae_optreset;
...
main(int argc, char *argv[])
{
	...
	while ((ch = __ae_getopt("aeperf", argc, argv, opts)) != EOF)
		switch (ch) {
		case 'C':
			if (user_cconfig == NULL)
				user_cconfig = strdup(__ae_optarg);
			else {
				user_cconfig = realloc(user_cconfig,
				    strlen(user_cconfig) +
				    strlen(__ae_optarg) + 2);
				strcat(user_cconfig, ",");
				strcat(user_cconfig, __ae_optarg);
			}
			break;
		case 'H':
			cfg->helium_mount = __ae_optarg;
			break;
		case 'O':
			config_opts = __ae_optarg;
			break;
		case 'T':
			if (user_tconfig == NULL)
				user_tconfig = strdup(__ae_optarg);
			else {
				user_tconfig = realloc(user_tconfig,
				    strlen(user_tconfig) +
				    strlen(__ae_optarg) + 2);
				strcat(user_tconfig, ",");
				strcat(user_tconfig, __ae_optarg);
			}
			break;
		case 'h':
			cfg->home = __ae_optarg;
			break;
		case 'm':
			cfg->monitor_dir = __ae_optarg;
			monitor_set = 1;
			break;
		case '?':
	...
	while ((ch = __ae_getopt("aeperf", argc, argv, opts)) != EOF)
		...
		case 'o':
			/* Allow -o key=value */
			if (config_opt_line(cfg, __ae_optarg) != 0)
				goto einval;
			break;
		}
    ...
    WORKLOAD *workp, CONFIG_THREAD *base, u_int num, void *(*func)(void *))
{
	...
	for (i = 0, thread = base; i < num; ++i, ++thread) {
		 ...
		 */
		if (i == 0)
			__ae_random_init(&thread->rnd);
		else
			thread->rnd = (thread - 1)->rnd;
		for (j = 0; j < 1000; ++j)
			(void)__ae_random(&thread->rnd);
...
worker_throttle(int64_t throttle, int64_t *ops, struct timespec *interval)
{
	 ...
	 */
	usecs_to_complete = AE_TIMEDIFF_US(now, *interval);
...
static uint64_t
aeperf_rand(CONFIG_THREAD *thread)
{
	CONFIG *cfg;
	double S1, S2, U;
	uint64_t rval;
	...
	/*
	 * Use ArchEngine's random number routine: it's lock-free and fairly
	 * good.
	 */
	rval = __ae_random(&thread->rnd);

	/* Use Pareto distribution to give 80/20 hot/cold values. */
	if (cfg->pareto != 0) {
#define	PARETO_SHAPE	1.5
		S1 = (-1 / PARETO_SHAPE);
		S2 = aeperf_value_range(cfg) *
		 ...
		 */
		if (rval > aeperf_value_range(cfg))
			rval = aeperf_value_range(cfg);
	}
	/*
	 * Wrap the key to within the expected range and avoid zero: we never
	 * insert that key.
	 */
	rval = (rval % aeperf_value_range(cfg)) + 1;
	return (rval);
}
