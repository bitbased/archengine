...
static int  config_opt(CONFIG *, AE_CONFIG_ITEM *, AE_CONFIG_ITEM *);
...
{
	WORKLOAD *workp;
	AE_CONFIG_ITEM groupk, groupv, k, v;
	AE_CONFIG_PARSER *group, *scan;
	int ret;
	 ...
	 */
	if ((ret =
	    archengine_config_parser_open(NULL, config, len, &group)) != 0)
		goto err;
 ...
 */
static int
config_opt(CONFIG *cfg, AE_CONFIG_ITEM *k, AE_CONFIG_ITEM *v)
{
	CONFIG_OPT *popt;
	char *newstr, **strp;
...
config_opt_file(CONFIG *cfg, const char *filename)
{
	...
	if ((fd = open(filename, O_RDONLY)) == -1) {
		fprintf(stderr, "aeperf: %s: %s\n", filename, strerror(errno));
		return (errno);
	}
	if ((ret = fstat(fd, &sb)) != 0) {
		fprintf(stderr, "aeperf: stat of %s: %s\n",
		    filename, strerror(errno));
		ret = errno;
		goto err;
	}
	...
	|| (size_t)read_size != buf_size
		...
		fprintf(stderr,
		    "aeperf: read unexpected amount from config file\n");
		ret = EINVAL;
		goto err;
	}
	 ...
	 * ArchEngine configuration string compatible string, and using
	 * the ArchEngine configuration parser to parse it at once.
	 */
#define	AEPERF_CONFIG_DELIMS	"\n\\"
	for (line = strtok(file_buf, AEPERF_CONFIG_DELIMS);
	    line != NULL;
	    line = strtok(NULL, AEPERF_CONFIG_DELIMS)) {
		linenum++;
		/* trim the line */
		...
		if (linelen + optionpos + 1 > sizeof(option)) {
			fprintf(stderr, "aeperf: %s: %d: line overflow\n",
			    filename, linenum);
			ret = EINVAL;
			break;
		}
		*rtrim = '\0';
		...
		if (contline)
			...
			if ((ret = config_opt_line(cfg, option)) != 0) {
				fprintf(stderr, "aeperf: %s: %d: parse error\n",
				    filename, linenum);
				break;
			}
			optionpos = 0;
	...
	if (ret == 0 && optionpos > 0) {
		fprintf(stderr, "aeperf: %s: %d: last line continues\n",
		    filename, linenum);
		ret = EINVAL;
		goto err;
	}
...
/*
 * usage --
 *	aeperf usage print, no error.
 */
void
usage(void)
{
	printf("aeperf [-C config] "
	...
	printf("\t-h <string> Arch Engine home must exist, default AE_TEST\n");
