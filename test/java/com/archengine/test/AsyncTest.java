...
import com.archengine.db.AsyncCallback;
import com.archengine.db.AsyncOpType;
import com.archengine.db.AsyncOp;
...
abstract class Callback<KeyType, ValueType> implements AsyncCallback {
    ...
    public int notify(AsyncOp op, int opReturn, int flags) {
        ...
        try {
            ...
            if (optype == AsyncOpType.AE_AOP_INSERT) {
                synchronized (syncobj) {
                    ninsert += 1;
                }
            }
            else if (optype == AsyncOpType.AE_AOP_SEARCH) {
                synchronized (syncobj) {
                    nsearch += 1;
                }
            }
