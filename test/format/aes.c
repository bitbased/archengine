 ...
 */
static int
handle_progress(AE_EVENT_HANDLER *handler,
...
/*
 * aes_open --
 *	Open a connection to a ArchEngine database.
 */
void
aes_open(const char *home, int set_api, AE_CONNECTION **connp)
{
	AE_CONNECTION *conn;
	int ret;
	...
	*connp = NULL;
	...
	if (REMAIN(p, end) == 0)
		die(ENOMEM, "archengine_open configuration buffer too small");
	...
	if ((ret = archengine_open(home, &event_handler, config, &conn)) != 0)
		die(ret, "archengine_open: %s", home);

	if (set_api)
		g.ae_api = conn->get_extension_api(conn);

	/*
	 * Load the Helium shared library: it would be possible to do this as
	 * part of the extensions configured for archengine_open, there's no
	...
	if (DATASOURCE("helium")) {
		...
		if ((ret =
			...
			die(ret,
			   "AE_CONNECTION.load_extension: %s:%s",
			   HELIUM_PATH, config);
	}
	*connp = conn;
}
...
aes_create(void)
{
	...
	if (REMAIN(p, end) == 0)
		die(ENOMEM, "AE_SESSION.create configuration buffer too small");
...
void
aes_dump(const char *tag, int dump_bdb)
{
