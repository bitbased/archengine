...
import Queue
import os, shutil, sys, threading, time, archengine, aetest
...
class backup_thread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.done.isSet():
            ...
            bkp_conn = archengine.archengine_open(self.backup_dir)
            bkp_session = bkp_conn.open_session()
            # Verify that the backup was OK.
            uris = list()
            for next_file in files:
                if next_file.startswith("ArchEngine"):
                    continue
                uri = "file:" + next_file
                uris.append(uri)

            # TODO: We want a self.assertTrue here - but need to be a aetest to
            # do that..
            if not compare_tables(
                        self, sess, uris, "checkpoint=ArchEngineCheckpoint"):
                print "Error: checkpoint tables differ."
            else:
                aetest.ArchEngineTestCase.printVerbose(
                    3, "Checkpoint tables match")

            if not compare_tables(self, bkp_session, uris):
                print "Error: backup tables differ."
            else:
                aetest.ArchEngineTestCase.printVerbose(3, "Backup tables match")
            bkp_conn.close()
