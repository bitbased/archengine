...
# test_util01.py
#    Utilities: ae dump, as well as the dump cursor
class test_util01(aetest.ArchEngineTestCase, suite_subprocess):
    """
    Test ae dump.  We check for specific output.
    ...
    def dump(self, usingapi, hexoutput):
        ...
        ver = archengine.archengine_version()
        ...
        with open("expect.out", "w") as expectout:
            if not usingapi:
                ...
                # generated by ae dump.  If this is likely to change, we should
                # make this test more accommodating.
                expectout.write('ArchEngine Dump (ArchEngine Version ' + verstring + ')\n')
                if hexoutput:
