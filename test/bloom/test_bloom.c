...
static struct {
	...
	AE_CONNECTION *ae_conn;			/* AE_CONNECTION handle */
	AE_SESSION *ae_session;			/* AE_SESSION handle */
...
main(int argc, char *argv[])
{
	...
	while ((ch = __ae_getopt(g.progname, argc, argv, "c:f:k:o:s:")) != EOF)
		switch (ch) {
		case 'c':			/* Cache size */
			g.c_cache = (u_int)atoi(__ae_optarg);
			break;
		case 'f':			/* Factor */
			g.c_factor = (u_int)atoi(__ae_optarg);
			break;
		case  'k':			/* Number of hash functions */
			g.c_k = (u_int)atoi(__ae_optarg);
			break;
		case 'o':			/* Number of ops */
			g.c_ops = (u_int)atoi(__ae_optarg);
			break;
		case 's':			/* Number of ops */
			g.c_srand = (u_int)atoi(__ae_optarg);
			break;
		default:
			usage();
		}
...
setup(void)
{
	...
	if ((ret = system("rm -f ArchEngine* *.bf")) != 0)
		testutil_die(ret, "system cleanup call failed");

	/*
	 * This test doesn't test public Arch Engine functionality, it still
	...
	if ((ret = archengine_open(NULL, NULL, config, &conn)) != 0)
...
run(void)
{
	AE_BLOOM *bloomp;
	AE_ITEM item;
	AE_SESSION_IMPL *sess;
	uint32_t fp, i;
	int ret;
	const char *uri = "file:my_bloom.bf";

	/* Use the internal session handle to access private APIs. */
	sess = (AE_SESSION_IMPL *)g.ae_session;

	if ((ret = __ae_bloom_create(
	    sess, uri, NULL, g.c_ops, g.c_factor, g.c_k, &bloomp)) != 0)
		testutil_die(ret, "__ae_bloom_create");

	item.size = g.c_key_max;
	for (i = 0; i < g.c_ops; i++) {
		item.data = g.entries[i];
		if ((ret = __ae_bloom_insert(bloomp, &item)) != 0)
			testutil_die(ret, "__ae_bloom_insert: %d", i);
	}

	if ((ret = __ae_bloom_finalize(bloomp)) != 0)
		testutil_die(ret, "__ae_bloom_finalize");

	for (i = 0; i < g.c_ops; i++) {
		item.data = g.entries[i];
		if ((ret = __ae_bloom_get(bloomp, &item)) != 0) {
			fprintf(stderr, "get failed at record: %d\n", i);
			testutil_die(ret, "__ae_bloom_get");
		}
	}
	if ((ret = __ae_bloom_close(bloomp)) != 0)
		testutil_die(ret, "__ae_bloom_close");

	if ((ret = g.ae_session->checkpoint(g.ae_session, NULL)) != 0)
		testutil_die(ret, "AE_SESSION.checkpoint");
	if ((ret = __ae_bloom_open(
	    sess, uri, g.c_factor, g.c_k, NULL, &bloomp)) != 0)
		testutil_die(ret, "__ae_bloom_open");
	for (i = 0; i < g.c_ops; i++) {
		item.data = g.entries[i];
		if ((ret = __ae_bloom_get(bloomp, &item)) != 0)
			testutil_die(ret, "__ae_bloom_get");
	...
	if ((ret = __ae_bloom_drop(bloomp, NULL)) != 0)
		testutil_die(ret, "__ae_bloom_drop");
}
...
cleanup(void)
{
	...
	for (i = 0; i < g.c_ops; i++)
		free(g.entries[i]);
	free(g.entries);
	if ((ret = g.ae_session->close(g.ae_session, NULL)) != 0)
		testutil_die(ret, "AE_SESSION.close");
	if ((g.ae_conn->close(g.ae_conn, NULL)) != 0)
