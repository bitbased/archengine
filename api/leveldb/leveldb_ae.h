 ...
 */
#ifndef _INCLUDE_LEVELDB_AE_H
#define _INCLUDE_LEVELDB_AE_H 1
...
#define AE_URI          "table:data"
#define AE_CONN_CONFIG                                                  \
...
extern Status ArchEngineErrorToStatus(int archEngineError, const char *msg = "");
...
public:
  ...
  AE_CURSOR *GetCursor() { return cursor_; }
  void SetCursor(AE_CURSOR *c) { cursor_ = c; }
#ifdef HAVE_ROCKSDB
  AE_CURSOR *GetCursor(u_int i) {
    return (i < cursors_.size()) ? cursors_[i] : NULL;
  }
  void SetCursor(u_int i, AE_CURSOR *c) {
    if (i >= cursors_.size())
      cursors_.resize(i + 1);
    cursors_[i] = c;
  }
#endif
  AE_SESSION *GetSession() { return session_; }

private:
  AE_SESSION *session_;
  AE_CURSOR *cursor_;
#ifdef HAVE_ROCKSDB
  std::vector<AE_CURSOR *> cursors_;
#endif
};
...
public:
  IteratorImpl(DbImpl *db, AE_CURSOR *cursor, uint32_t id=0) : db_(db), cursor_(cursor), id_(id) {}
...
private:
  ...
  void SetError(int archEngineError) {
    valid_ = false;
    status_ = ArchEngineErrorToStatus(archEngineError, NULL);
  }
...
public:
  ...
  int GetArchEngineStatus() { return status_; }
...
#ifdef HAVE_ROCKSDB
  // Implementations are in rocksdb_ae.cc
