...
Status leveldb::DestroyDB(const std::string& name, const Options& options) {
  ...
  if (access((name + "/ArchEngine").c_str(), F_OK) != 0)
    return Status::OK();
  if ((ret = ::archengine_open(name.c_str(), NULL, NULL, &conn)) != 0)
    return ArchEngineErrorToStatus(ret, NULL);
  AE_SESSION *session;
  if ((ret = conn->open_session(conn, NULL, NULL, &session)) != 0)
    goto cleanup;
  if ((ret = session->drop(session, AE_URI, "force")) != 0)
    goto cleanup;
...
Status ArchEngineErrorToStatus(int archEngineError, const char *msg) {
  if (archEngineError == 0)
    return Status::OK();

  if (msg == NULL)
    msg = archengine_strerror(archEngineError);

  if (archEngineError != AE_NOTFOUND)
    printf("Failing status: %d -> %s\n", archEngineError, msg);

  if (archEngineError == AE_NOTFOUND)
    return Status::NotFound(Slice(msg));
  else if (archEngineError == AE_ERROR || archEngineError == AE_PANIC)
    return Status::Corruption(Slice(msg));
  else if (archEngineError == ENOTSUP)
    return Status::NotSupported(Slice(msg));
  else if (archEngineError == EINVAL)
    return Status::InvalidArgument(Slice(msg));
  else if (archEngineError == EPERM || archEngineError == ENOENT ||
      archEngineError == EIO || archEngineError == EBADF ||
      archEngineError == EEXIST || archEngineError == ENOSPC)
    return Status::IOError(Slice(msg));
  else if (archEngineError == AE_ROLLBACK)
...
int
aeleveldb_create(
    AE_CONNECTION *conn, const Options &options, std::string const &uri)
{
  int ret;
  std::stringstream s_table;
  s_table << AE_TABLE_CONFIG;
...
{
  // Build the archengine_open config.
  std::stringstream s_conn;
  s_conn << AE_CONN_CONFIG;
  ...
  if (options.compression == kSnappyCompression)
    s_conn << "extensions=[libarchengine_snappy.so],";
#endif
#ifdef HAVE_ROCKSDB
#ifndef HAVE_BUILTIN_ZLIB
  if (options.compression == kZlibCompression)
    s_conn << "extensions=[libarchengine_zlib.so],";
#endif
#endif
  ...
  int ret = ::archengine_open(name.c_str(), NULL, conn_config.c_str(), &conn);
  if (ret == ENOENT)
  ...
  if (options.create_if_missing)
    ret = aeleveldb_create(conn, options, AE_URI);
...
{
  AE_CURSOR *cursor = GetContext()->GetCursor();
  AE_ITEM item;
...
DbImpl::Delete(const WriteOptions& options, const Slice& key)
{
  ...
  // Reset the ArchEngine cursor so it doesn't keep any pages pinned. Track
...
WriteBatchHandler::Put(const Slice& key, const Slice& value) {
  AE_CURSOR *cursor = context_->GetCursor();
  AE_ITEM item;
...
void WriteBatchHandler::Delete(const Slice& key) {
  ...
  cursor->set_key(cursor, &item);
  int ret = cursor->remove(cursor);
  if (ret != 0 && ret != AE_NOTFOUND && status_ == 0)
    status_ = ret;
}
...
DbImpl::Write(const WriteOptions& options, WriteBatch* updates)
{
  ...
  AE_SESSION *session = context->GetSession();
  int ret = 0, t_ret;
...
#endif
    if (!status.ok() || (ret = handler.GetArchEngineStatus()) != AE_ROLLBACK)
      break;
...
err:
  if (status.ok() && ret != 0)
    status = ArchEngineErrorToStatus(ret, errmsg);
  return status;
}
...
{
  AE_CURSOR *cursor = GetContext(options)->GetCursor();
  const char *errmsg = NULL;
  ...
  return ArchEngineErrorToStatus(ret, errmsg);
}
...
DbImpl::GetSnapshot()
{
  ...
  AE_SESSION *session = si->GetContext()->GetSession();
...
DbImpl::CompactRange(const Slice* begin, const Slice* end)
{
  ...
  AE_CURSOR *cursor = GetContext()->GetCursor();
  AE_SESSION *session = cursor->session;
  int ret = session->compact(session, AE_URI, NULL);
  assert(ret == 0);
}
