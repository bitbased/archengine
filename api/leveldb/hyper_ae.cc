...
class ReplayIteratorImpl : public ReplayIterator {
 public:
  ReplayIteratorImpl(OperationContext *context) : context_(context), cursor_(NULL) {
    AE_SESSION *session = context_->GetSession();
    int ret = session->open_cursor(
        session, "log:", NULL, NULL, &cursor_);
    status_ = ArchEngineErrorToStatus(ret);
    valid_ = false;
  ...
  std::string GetTimestamp() {
	char lsn[256];
        assert(Valid());
	snprintf(lsn, sizeof(lsn), AE_TIMESTAMP_FORMAT,
...
ReplayIteratorImpl::Next() {
	...
	if (cursor_ != NULL) {
		while ((ret = cursor_->next(cursor_)) == 0) {
			...
			if (AE_VALID_OPERATION(fileid, optype)) {
				valid_ = true;
				break;
			}
		}
...
ReplayIteratorImpl::SeekToLast() {
	...
	if (cursor_ != NULL) {
		...
		if (ret != AE_NOTFOUND || last_lsn.file == 0) {
			valid_ = false;
			ret = Close();
			assert(ret == 0);
		} else
...
ReplayIteratorImpl::SeekTo(const std::string& timestamp) {
	...
	sscanf(timestamp.c_str(), AE_TIMESTAMP_FORMAT,
...
// given LSN.
void
ReplayIteratorImpl::SeekTo(AE_LSN *target_lsn) {
	int ret = 0;
...
DbImpl::LiveBackup(const Slice& name)
{
	...
	// AE single-threads hot backups.  If we get here we already have
