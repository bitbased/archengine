...
static int
aerocks_get_cursor(OperationContext *context, ColumnFamilyHandle *cfhp, AE_CURSOR **cursorp, int acquire=0)
{
...
err:	delete db;
	/*
	 * AE_NOTFOUND is not an error: it just means we got to the end of the
	 * list of tables.
	 */
	...
	return ArchEngineErrorToStatus(ret);
}
...
{
	AE_CURSOR *cursor;
	int ret = aerocks_get_cursor(context_, db_->GetCF(column_family_id), &cursor);
	if (ret != 0)
		return ArchEngineErrorToStatus(ret);
	AE_ITEM item;
	item.data = key.data();
...
{
	return ArchEngineErrorToStatus(ENOTSUP);
}

Status
DbImpl::CreateColumnFamily(Options const &options, std::string const &name, ColumnFamilyHandle **cfhp)
{
	extern int aeleveldb_create(AE_CONNECTION *,
	    const Options &, std::string const &uri);
	int ret = aeleveldb_create(conn_, options, "table:" + name);
	if (ret != 0)
		return ArchEngineErrorToStatus(ret);
...
DbImpl::DropColumnFamily(ColumnFamilyHandle *cfhp)
{
	ColumnFamilyHandleImpl *cf =
	    static_cast<ColumnFamilyHandleImpl *>(cfhp);
	AE_SESSION *session = GetContext()->GetSession();
...
{
	AE_CURSOR *cursor;
	int ret = aerocks_get_cursor(GetContext(), cfhp, &cursor);
	if (ret != 0)
		return ArchEngineErrorToStatus(ret);
	AE_ITEM item;
	item.data = key.data();
	...
	cursor->set_key(cursor, &item);
	ret = cursor->remove(cursor);
	// Reset the ArchEngine cursor so it doesn't keep any pages pinned.
...
DbImpl::Flush(FlushOptions const&, ColumnFamilyHandle* cfhp)
{
	ColumnFamilyHandleImpl *cf =
	    static_cast<ColumnFamilyHandleImpl *>(cfhp);
	AE_SESSION *session = GetContext()->GetSession();
	return ArchEngineErrorToStatus(session->checkpoint(session, ("target=(\"" + cf->GetURI() + "\")").c_str()));
}
...
DbImpl::Get(ReadOptions const &options, ColumnFamilyHandle *cfhp, Slice const &key, std::string *value)
{
	...
	AE_CURSOR *cursor;
	int ret = aerocks_get_cursor(context, cfhp, &cursor);
	if (ret != 0)
		return ArchEngineErrorToStatus(ret);
...
DbImpl::MultiGet(ReadOptions const&, std::vector<ColumnFamilyHandle*> const&, std::vector<Slice> const&, std::vector<std::string, std::allocator<std::string> >*)
{
	...
	ret.push_back(ArchEngineErrorToStatus(ENOTSUP));
	return ret;
}
...
DbImpl::NewIterator(ReadOptions const &options, ColumnFamilyHandle *cfhp)
{
	...
	AE_CURSOR *c;
	int ret = aerocks_get_cursor(context, cfhp, &c, 1);
	assert(ret == 0);
