...
def munge(args, title, values):
    ...
    if title.split(' ')[1] != 'spinlock' and \
      title.split(' ', 1)[1] in no_scale_per_second_list:
        seconds = 1
    elif 'aeperf' in title and 'per second' not in title:
        seconds = 1
    else:
        t1, v1 = values[1]
    ...
    stats_cleared = False
    if args.clear or title.split(' ', 1)[1] in no_clear_list or 'aeperf' in title:
        stats_cleared = True
    ...
    for t, v in sorted(values):
        ...
        if not stats_cleared:
            ...
            # Sometimes ArchEngine stats go backwards without clear, assume
...
def parse_aestats_file(file, result):
    """ parse aestats file, one stat per line, example format:
           Dec 05 14:43:14 0 /data/b block-manager: mapped bytes read
    """
    print 'Processing aestats file: ' + file
...
def parse_aeperf_file(file, result):
    """ parse aeperf file, all stats on single line, example format:
           Feb 13 17:55:14,0,0,156871,0,N,0,0,0,49,6,6146,0,0,0
    """
    print 'Processing aeperf file: ' + file
    fh = open(file, 'rU')
    ...
    for line in fh:
        ...
        for i, v in enumerate(values):
            ...
            if '(ms)' in headings[i]:
                v = float(v) / 1000.0
            result['aeperf: ' + headings[i]].append((month + " " + day + " " + time, v))


def skip_constants(result):
    # Process the series, eliminate constants, delete totalsec for aeperf
...
def parse_files(files_or_dir):
    """ walk through file list or directory and parse according to file type (aestats / aeperf). """
        ...
        if os.path.isfile(f):
            ...
            with open(f, 'rU') as fh:
                ...
                if line.startswith('#time'):
                    parse_aeperf_file(f, result)
                else: 
                    parse_aestats_file(f, result)
            
        elif os.path.isdir(f):
            for s in glob(os.path.join(f, 'ArchEngineStat*')):
                parse_aestats_file(s, result)

            for s in glob(os.path.join(f, 'monitor*')):
                parse_aeperf_file(s, result)
...
def output_series(results, args, prefix=None, grouplist=[]):
    ...
    srcfile = os.path.join(this_path, 'aestats.html.template')
    try: 
        srcfile = open(srcfile)
        contents = srcfile.read()
    except IOError: 
        print >>sys.stderr, "Cannot find template file 'aestats.html." \
...
def main():   
    ...
    parser.add_argument('--output', '-o', metavar='file', default='aestats',
        help='HTML output file prefix')
    parser.add_argument('--json', action='store_true', 
        help='additionally output data series in json format')
    parser.add_argument('files', metavar='file', nargs='+',
        help='input files or directories generated by ArchEngine statistics' \
        'logging')
