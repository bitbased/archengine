...
def helper_run_with_fixture(kwargs=None):
    ...
    if (not '--output' in kwargs) or (kwargs['--output'] == None):
        kwargs['--output'] = '_test_aestats.html'

    # path replacement
    kwargs['--output'] = os.path.join(test_dir, kwargs['--output'])

    statsfile = os.path.join(test_dir, kwargs['files'] if 'files' in kwargs else 'ArchEngineStat.fixture')
    print "ST", statsfile

    arglist = ['./aestats', statsfile]
...
def test_helper_runner():
    ...
    assert os.path.exists(os.path.join(tool_dir, 'test', '_test_aestats.html'))


def test_help_working():
    """ aestast should output the help screen when using --help argument """
    sys.argv = ['./aestats.py', '--help']
    try:
        main()
    except SystemExit:
        pass
...
def test_create_html_file_basic():
    """ aestats should create an html file from the fixture stats """

    outfile = os.path.join(test_dir, 'aestats_test.html')
    statsfile = os.path.join(test_dir, 'ArchEngineStat.fixture')
    
    sys.argv = ['./aestats', statsfile, '--output', outfile]
    try:
        main()
    except SystemExit:
        pass
...
def test_output_default():
    """ aestats should choose default output if not specified """

    statsfile = os.path.join(test_dir, 'ArchEngineStat.fixture')
    
    sys.argv = ['./aestats', statsfile]
    try:
        main()
    except SystemExit:
        pass

    assert os.path.exists(os.path.join('./aestats.html'))
    helper_delete_file('./aestats.html')


@with_setup(setUp, tearDown)
def test_output_option():
    """ aestats should use the specified filename with --output """
...
@with_setup(setUp, tearDown)
def test_monitor_stats_start_with_aeperf():
    """ aestats should be able to parse aeperf monitor files """
...
def test_monitor_stats_convert_us_to_ms():
    """ aestats should convert monitor stats us to ms """
    ...
    values = (item['values'] for item in data['series'] if item['key'] == 'aeperf: insert maximum latency (ms)').next().values()
    assert max(values) == 103687 / 1000.



@with_setup(setUp, tearDown)
def test_directory_with_aestats_and_aeperf():
    """ aestats should be able to parse directories containing both types """
    ...
    assert any(map(lambda title: 'aeperf' in title, series_keys))


@with_setup(setUp, tearDown)
def test_add_ext_if_missing():
    """ aestats should only add ".html" extension if it's missing in the --output value """
...
def test_replace_data_in_template():
    """ aestats should replace the placeholder with real data """

    outfile = '_test_output_file.html'
    helper_run_with_fixture({'--output': outfile})

    templfile = open(os.path.join(tool_dir, 'aestats.html.template'), 'r')
...
def test_data_with_options():
    """ aestats should output the data as expected to the html file """
...
def test_include_option():
    """ aestats should only parse the matched stats with --include """
...
def test_include_skip_prefix():
    """ aestats should remove the common prefix from titles """ 
...
def test_list_option():
    """ aestats should only output list of series titles with --list """
...
def test_json_option():
    """ aestats should additionally output json file with --json """
...
@with_setup(setUp, tearDown)
def test_all_option():
    """ aestats should create grouped html files with --all """
