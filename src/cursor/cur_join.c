/*-
 * Copyright (c) 2014-2015 MongoDB, Inc.
 * Copyright (c) 2008-2014 ArchEngine, Inc.
 *	All rights reserved.
 *
 * See the file LICENSE for redistribution information.
 */

#include "ae_internal.h"

/*
 * __curjoin_entry_iter_init --
 *	Initialize an iteration for the index managed by a join entry.
 *
 */
static int
__curjoin_entry_iter_init(AE_SESSION_IMPL *session, AE_CURSOR_JOIN *cjoin,
    AE_CURSOR_JOIN_ENTRY *entry, AE_CURSOR_JOIN_ITER **iterp)
{
	AE_CURSOR *newcur;
	AE_CURSOR *to_dup;
	AE_DECL_RET;
	const char *raw_cfg[] = { AE_CONFIG_BASE(
	    session, AE_SESSION_open_cursor), "raw", NULL };
	const char *def_cfg[] = { AE_CONFIG_BASE(
	    session, AE_SESSION_open_cursor), NULL };
	const char *uri, **config;
	char *uribuf;
	AE_CURSOR_JOIN_ITER *iter;
	size_t size;

	iter = NULL;
	uribuf = NULL;
	to_dup = entry->ends[0].cursor;

	uri = to_dup->uri;
	if (F_ISSET((AE_CURSOR *)cjoin, AE_CURSTD_RAW))
		config = &raw_cfg[0];
	else
		config = &def_cfg[0];

	if (cjoin->projection != NULL) {
		size = strlen(uri) + strlen(cjoin->projection) + 1;
		AE_ERR(__ae_calloc(session, size, 1, &uribuf));
		snprintf(uribuf, size, "%s%s", uri, cjoin->projection);
		uri = uribuf;
	}
	AE_ERR(__ae_open_cursor(session, uri, (AE_CURSOR *)cjoin, config,
	    &newcur));
	AE_ERR(__ae_cursor_dup_position(to_dup, newcur));
	AE_ERR(__ae_calloc_one(session, &iter));
	iter->cjoin = cjoin;
	iter->session = session;
	iter->entry = entry;
	iter->cursor = newcur;
	iter->advance = false;
	*iterp = iter;

	if (0) {
err:		__ae_free(session, iter);
	}
	__ae_free(session, uribuf);
	return (ret);
}

/*
 * __curjoin_pack_recno --
 *	Pack the given recno into a buffer; prepare an item referencing it.
 *
 */
static int
__curjoin_pack_recno(AE_SESSION_IMPL *session, uint64_t r, uint8_t *buf,
    size_t bufsize, AE_ITEM *item)
{
	AE_DECL_RET;
	AE_SESSION *aesession;
	size_t sz;

	aesession = (AE_SESSION *)session;
	AE_ERR(archengine_struct_size(aesession, &sz, "r", r));
	AE_ASSERT(session, sz < bufsize);
	AE_ERR(archengine_struct_pack(aesession, buf, bufsize, "r", r));
	item->size = sz;
	item->data = buf;

err:	return (ret);
}

/*
 * __curjoin_entry_iter_next --
 *	Get the next item in an iteration.
 *
 */
static int
__curjoin_entry_iter_next(AE_CURSOR_JOIN_ITER *iter, AE_ITEM *primkey,
    uint64_t *rp)
{
	AE_CURSOR *firstcg_cur;
	AE_CURSOR_JOIN *cjoin;
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	uint64_t r;

	if (iter->advance)
		AE_ERR(iter->cursor->next(iter->cursor));
	else
		iter->advance = true;

	session = iter->session;
	cjoin = iter->cjoin;

	/*
	 * Set our key to the primary key, we'll also need this
	 * to check membership.
	 */
	if (iter->entry->index != NULL)
		firstcg_cur = ((AE_CURSOR_INDEX *)iter->cursor)->cg_cursors[0];
	else
		firstcg_cur = ((AE_CURSOR_TABLE *)iter->cursor)->cg_cursors[0];
	if (AE_CURSOR_RECNO(&cjoin->iface)) {
		r = *(uint64_t *)firstcg_cur->key.data;
		AE_ERR(__curjoin_pack_recno(session, r, cjoin->recno_buf,
		    sizeof(cjoin->recno_buf), primkey));
		*rp = r;
	} else {
		AE_ITEM_SET(*primkey, firstcg_cur->key);
		*rp = 0;
	}
	iter->curkey = primkey;
	iter->entry->stats.actual_count++;
	iter->entry->stats.accesses++;

err:	return (ret);
}

/*
 * __curjoin_entry_iter_reset --
 *	Reset an iteration to the starting point.
 *
 */
static int
__curjoin_entry_iter_reset(AE_CURSOR_JOIN_ITER *iter)
{
	AE_DECL_RET;

	if (iter->advance) {
		AE_ERR(iter->cursor->reset(iter->cursor));
		AE_ERR(__ae_cursor_dup_position(
		    iter->cjoin->entries[0].ends[0].cursor, iter->cursor));
		iter->advance = false;
		iter->entry->stats.actual_count = 0;
	}

err:	return (ret);
}

/*
 * __curjoin_entry_iter_ready --
 *	The iterator is positioned.
 *
 */
static bool
__curjoin_entry_iter_ready(AE_CURSOR_JOIN_ITER *iter)
{
	return (iter->advance);
}

/*
 * __curjoin_entry_iter_close --
 *	Close the iteration, release resources.
 *
 */
static int
__curjoin_entry_iter_close(AE_CURSOR_JOIN_ITER *iter)
{
	AE_DECL_RET;

	if (iter->cursor != NULL)
		AE_TRET(iter->cursor->close(iter->cursor));
	__ae_free(iter->session, iter);

	return (ret);
}

/*
 * __curjoin_get_key --
 *	AE_CURSOR->get_key for join cursors.
 */
static int
__curjoin_get_key(AE_CURSOR *cursor, ...)
{
	AE_CURSOR_JOIN *cjoin;
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	va_list ap;

	cjoin = (AE_CURSOR_JOIN *)cursor;

	va_start(ap, cursor);
	CURSOR_API_CALL(cursor, session, get_key, NULL);

	if (!F_ISSET(cjoin, AE_CURJOIN_INITIALIZED) ||
	    !__curjoin_entry_iter_ready(cjoin->iter))
		AE_ERR_MSG(session, EINVAL,
		    "join cursor must be advanced with next()");
	AE_ERR(__ae_cursor_get_keyv(cursor, cursor->flags, ap));

err:	va_end(ap);
	API_END_RET(session, ret);
}

/*
 * __curjoin_get_value --
 *	AE_CURSOR->get_value for join cursors.
 */
static int
__curjoin_get_value(AE_CURSOR *cursor, ...)
{
	AE_CURSOR_JOIN *cjoin;
	AE_CURSOR_JOIN_ITER *iter;
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	va_list ap;

	cjoin = (AE_CURSOR_JOIN *)cursor;
	iter = cjoin->iter;

	va_start(ap, cursor);
	CURSOR_API_CALL(cursor, session, get_value, NULL);

	if (!F_ISSET(cjoin, AE_CURJOIN_INITIALIZED) ||
	    !__curjoin_entry_iter_ready(iter))
		AE_ERR_MSG(session, EINVAL,
		    "join cursor must be advanced with next()");
	if (iter->entry->index != NULL)
		AE_ERR(__ae_curindex_get_valuev(iter->cursor, ap));
	else
		AE_ERR(__ae_curtable_get_valuev(iter->cursor, ap));

err:	va_end(ap);
	API_END_RET(session, ret);
}

/*
 * __curjoin_init_bloom --
 *	Populate Bloom filters
 */
static int
__curjoin_init_bloom(AE_SESSION_IMPL *session, AE_CURSOR_JOIN *cjoin,
    AE_CURSOR_JOIN_ENTRY *entry, AE_BLOOM *bloom)
{
	AE_COLLATOR *collator;
	AE_CURSOR *c;
	AE_CURSOR_INDEX *cindex;
	AE_CURSOR_JOIN_ENDPOINT *end, *endmax;
	AE_DECL_RET;
	AE_DECL_ITEM(uribuf);
	AE_ITEM curkey, curvalue, *k;
	AE_TABLE *maintable;
	const char *raw_cfg[] = { AE_CONFIG_BASE(
	    session, AE_SESSION_open_cursor), "raw", NULL };
	const char *mainkey_str, *p;
	void *allocbuf;
	size_t mainkey_len, size;
	u_int i;
	int cmp, skip;

	c = NULL;
	allocbuf = NULL;
	skip = 0;

	if (entry->index != NULL) {
		/*
		 * Open a cursor having a projection of the keys of the
		 * index we're comparing against.  Open it raw, we're
		 * going to compare it to the raw keys of the
		 * reference cursors.
		 */
		maintable = ((AE_CURSOR_TABLE *)entry->main)->table;
		mainkey_str = maintable->colconf.str + 1;
		for (p = mainkey_str, i = 0;
		     p != NULL && i < maintable->nkey_columns; i++)
			p = strchr(p + 1, ',');
		AE_ASSERT(session, p != 0);
		mainkey_len = AE_PTRDIFF(p, mainkey_str);
		size = strlen(entry->index->name) + mainkey_len + 3;
		AE_ERR(__ae_scr_alloc(session, size, &uribuf));
		AE_ERR(__ae_buf_fmt(session, uribuf, "%s(%.*s)",
		    entry->index->name, (int)mainkey_len, mainkey_str));
	} else {
		/*
		 * For joins on the main table, we just need the primary
		 * key for comparison, we don't need any values.
		 */
		size = strlen(cjoin->table->name) + 3;
		AE_ERR(__ae_scr_alloc(session, size, &uribuf));
		AE_ERR(__ae_buf_fmt(session, uribuf, "%s()",
		    cjoin->table->name));
	}
	AE_ERR(__ae_open_cursor(
	    session, uribuf->data, &cjoin->iface, raw_cfg, &c));

	/* Initially position the cursor if necessary. */
	endmax = &entry->ends[entry->ends_next];
	if ((end = &entry->ends[0]) < endmax &&
	    F_ISSET(end, AE_CURJOIN_END_GE)) {
		AE_ERR(__ae_cursor_dup_position(end->cursor, c));
		if (end->flags == AE_CURJOIN_END_GE)
			skip = 1;
	}
	collator = (entry->index == NULL) ? NULL : entry->index->collator;
	while (ret == 0) {
		AE_ERR(c->get_key(c, &curkey));
		if (entry->index != NULL) {
			cindex = (AE_CURSOR_INDEX *)c;
			if (cindex->index->extractor == NULL) {
				/*
				 * Repack so it's comparable to the
				 * reference endpoints.
				 */
				k = &cindex->child->key;
				AE_ERR(__ae_struct_repack(session,
				    cindex->child->key_format,
				    entry->main->value_format, k, &curkey,
				    &allocbuf));
			} else
				curkey = cindex->child->key;
		}
		for (end = &entry->ends[skip]; end < endmax; end++) {
			AE_ERR(__ae_compare(session, collator, &curkey,
			    &end->key, &cmp));
			if (!F_ISSET(end, AE_CURJOIN_END_LT)) {
				if (cmp < 0 || (cmp == 0 &&
				    !F_ISSET(end, AE_CURJOIN_END_EQ)))
					goto advance;
				if (cmp > 0) {
					if (F_ISSET(end, AE_CURJOIN_END_GT))
						skip = 1;
					else
						goto done;
				}
			} else {
				if (cmp > 0 || (cmp == 0 &&
				    !F_ISSET(end, AE_CURJOIN_END_EQ)))
					goto done;
			}
		}
		if (entry->index != NULL)
			AE_ERR(c->get_value(c, &curvalue));
		else
			AE_ERR(c->get_key(c, &curvalue));
		AE_ERR(__ae_bloom_insert(bloom, &curvalue));
		entry->stats.actual_count++;
advance:
		if ((ret = c->next(c)) == AE_NOTFOUND)
			break;
	}
done:
	AE_ERR_NOTFOUND_OK(ret);

err:	if (c != NULL)
		AE_TRET(c->close(c));
	__ae_scr_free(session, &uribuf);
	__ae_free(session, allocbuf);
	return (ret);
}

/*
 * __curjoin_endpoint_init_key --
 *	Set the key in the reference endpoint.
 */
static int
__curjoin_endpoint_init_key(AE_SESSION_IMPL *session,
    AE_CURSOR_JOIN_ENTRY *entry, AE_CURSOR_JOIN_ENDPOINT *endpoint)
{
	AE_CURSOR *cursor;
	AE_CURSOR_INDEX *cindex;
	AE_DECL_RET;
	AE_ITEM *k;
	uint64_t r;
	void *allocbuf;

	allocbuf = NULL;
	if ((cursor = endpoint->cursor) != NULL) {
		if (entry->index != NULL) {
			cindex = (AE_CURSOR_INDEX *)endpoint->cursor;
			if (cindex->index->extractor == NULL) {
				AE_ERR(__ae_struct_repack(session,
				    cindex->child->key_format,
				    entry->main->value_format,
				    &cindex->child->key, &endpoint->key,
				    &allocbuf));
				if (allocbuf != NULL)
					F_SET(endpoint, AE_CURJOIN_END_OWN_KEY);
			} else
				endpoint->key = cindex->child->key;
		} else {
			k = &((AE_CURSOR_TABLE *)cursor)->cg_cursors[0]->key;
			if (AE_CURSOR_RECNO(cursor)) {
				r = *(uint64_t *)k->data;
				AE_ERR(__curjoin_pack_recno(session, r,
				    endpoint->recno_buf,
				    sizeof(endpoint->recno_buf),
				    &endpoint->key));
			}
			else
				endpoint->key = *k;
		}
	}
	if (0) {
err:		__ae_free(session, allocbuf);
	}
	return (ret);
}

/*
 * __curjoin_init_iter --
 *	Initialize before any iteration.
 */
static int
__curjoin_init_iter(AE_SESSION_IMPL *session, AE_CURSOR_JOIN *cjoin)
{
	AE_BLOOM *bloom;
	AE_DECL_RET;
	AE_CURSOR_JOIN_ENTRY *je, *jeend, *je2;
	AE_CURSOR_JOIN_ENDPOINT *end;
	uint32_t f, k;

	if (cjoin->entries_next == 0)
		AE_RET_MSG(session, EINVAL,
		    "join cursor has not yet been joined with any other "
		    "cursors");

	je = &cjoin->entries[0];
	AE_RET(__curjoin_entry_iter_init(session, cjoin, je, &cjoin->iter));

	jeend = &cjoin->entries[cjoin->entries_next];
	for (je = cjoin->entries; je < jeend; je++) {
		__ae_stat_join_init_single(&je->stats);
		for (end = &je->ends[0]; end < &je->ends[je->ends_next];
		     end++)
			AE_RET(__curjoin_endpoint_init_key(session, je, end));

		/*
		 * The first entry is iterated as the 'outermost' cursor.
		 * For the common GE case, we don't have to test against
		 * the left reference key, we know it will be true since
		 * the btree is ordered.
		 */
		if (je == cjoin->entries && je->ends[0].flags ==
		    (AE_CURJOIN_END_GT | AE_CURJOIN_END_EQ))
			F_SET(cjoin, AE_CURJOIN_SKIP_FIRST_LEFT);

		if (F_ISSET(je, AE_CURJOIN_ENTRY_BLOOM)) {
			if (je->bloom == NULL) {
				/*
				 * Look for compatible filters to be shared,
				 * pick compatible numbers for bit counts
				 * and number of hashes.
				 */
				f = je->bloom_bit_count;
				k = je->bloom_hash_count;
				for (je2 = je + 1; je2 < jeend; je2++)
					if (F_ISSET(je2,
					    AE_CURJOIN_ENTRY_BLOOM) &&
					    je2->count == je->count) {
						f = AE_MAX(
						    je2->bloom_bit_count, f);
						k = AE_MAX(
						    je2->bloom_hash_count, k);
					}
				je->bloom_bit_count = f;
				je->bloom_hash_count = k;
				AE_RET(__ae_bloom_create(session, NULL,
				    NULL, je->count, f, k, &je->bloom));
				F_SET(je, AE_CURJOIN_ENTRY_OWN_BLOOM);
				AE_RET(__curjoin_init_bloom(session, cjoin,
				    je, je->bloom));
				/*
				 * Share the Bloom filter, making all
				 * config info consistent.
				 */
				for (je2 = je + 1; je2 < jeend; je2++)
					if (F_ISSET(je2,
					    AE_CURJOIN_ENTRY_BLOOM) &&
					    je2->count == je->count) {
						AE_ASSERT(session,
						    je2->bloom == NULL);
						je2->bloom = je->bloom;
						je2->bloom_bit_count = f;
						je2->bloom_hash_count = k;
					}
			} else {
				/*
				 * Create a temporary filter that we'll
				 * merge into the shared one.  The Bloom
				 * parameters of the two filters must match.
				 */
				AE_RET(__ae_bloom_create(session, NULL,
				    NULL, je->count, je->bloom_bit_count,
				    je->bloom_hash_count, &bloom));
				AE_RET(__curjoin_init_bloom(session, cjoin,
				    je, bloom));
				AE_RET(__ae_bloom_intersection(je->bloom,
				    bloom));
				AE_RET(__ae_bloom_close(bloom));
			}
		}
	}

	F_SET(cjoin, AE_CURJOIN_INITIALIZED);
	return (ret);
}

/*
 * __curjoin_entry_in_range --
 *	Check if a key is in the range specified by the entry, returning
 *	AE_NOTFOUND if not.
 */
static int
__curjoin_entry_in_range(AE_SESSION_IMPL *session, AE_CURSOR_JOIN_ENTRY *entry,
    AE_ITEM *curkey, bool skip_left)
{
	AE_COLLATOR *collator;
	AE_CURSOR_JOIN_ENDPOINT *end, *endmax;
	AE_DECL_RET;
	int cmp;

	collator = (entry->index != NULL) ? entry->index->collator : NULL;
	endmax = &entry->ends[entry->ends_next];
	for (end = &entry->ends[skip_left ? 1 : 0]; end < endmax; end++) {
		AE_ERR(__ae_compare(session, collator, curkey, &end->key,
		    &cmp));
		if (!F_ISSET(end, AE_CURJOIN_END_LT)) {
			if (cmp < 0 ||
			    (cmp == 0 &&
			    !F_ISSET(end, AE_CURJOIN_END_EQ)) ||
			    (cmp > 0 && !F_ISSET(end, AE_CURJOIN_END_GT)))
				AE_ERR(AE_NOTFOUND);
		} else {
			if (cmp > 0 ||
			    (cmp == 0 &&
			    !F_ISSET(end, AE_CURJOIN_END_EQ)) ||
			    (cmp < 0 && !F_ISSET(end, AE_CURJOIN_END_LT)))
				AE_ERR(AE_NOTFOUND);
		}
	}
err:	return (ret);
}

typedef struct {
	AE_CURSOR iface;
	AE_CURSOR_JOIN_ENTRY *entry;
	int ismember;
} AE_CURJOIN_EXTRACTOR;

/*
 * __curjoin_extract_insert --
 *	Handle a key produced by a custom extractor.
 */
static int
__curjoin_extract_insert(AE_CURSOR *cursor) {
	AE_CURJOIN_EXTRACTOR *cextract;
	AE_DECL_RET;
	AE_ITEM ikey;
	AE_SESSION_IMPL *session;

	cextract = (AE_CURJOIN_EXTRACTOR *)cursor;
	/*
	 * This insert method may be called multiple times during a single
	 * extraction.  If we already have a definitive answer to the
	 * membership question, exit early.
	 */
	if (cextract->ismember)
		return (0);

	session = (AE_SESSION_IMPL *)cursor->session;

	AE_ITEM_SET(ikey, cursor->key);
	/*
	 * We appended a padding byte to the key to avoid rewriting the last
	 * column.  Strip that away here.
	 */
	AE_ASSERT(session, ikey.size > 0);
	--ikey.size;

	ret = __curjoin_entry_in_range(session, cextract->entry, &ikey, false);
	if (ret == AE_NOTFOUND)
		ret = 0;
	else
		cextract->ismember = 1;

	return (ret);
}

/*
 * __curjoin_entry_member --
 *	Do a membership check for a particular index that was joined,
 *	if not a member, returns AE_NOTFOUND.
 */
static int
__curjoin_entry_member(AE_SESSION_IMPL *session, AE_CURSOR_JOIN *cjoin,
    AE_CURSOR_JOIN_ENTRY *entry, bool skip_left)
{
	AE_CURJOIN_EXTRACTOR extract_cursor;
	AE_CURSOR *c;
	AE_CURSOR_STATIC_INIT(iface,
	    __ae_cursor_get_key,	/* get-key */
	    __ae_cursor_get_value,	/* get-value */
	    __ae_cursor_set_key,	/* set-key */
	    __ae_cursor_set_value,	/* set-value */
	    __ae_cursor_notsup,		/* compare */
	    __ae_cursor_notsup,		/* equals */
	    __ae_cursor_notsup,		/* next */
	    __ae_cursor_notsup,		/* prev */
	    __ae_cursor_notsup,		/* reset */
	    __ae_cursor_notsup,		/* search */
	    __ae_cursor_notsup,		/* search-near */
	    __curjoin_extract_insert,	/* insert */
	    __ae_cursor_notsup,		/* update */
	    __ae_cursor_notsup,		/* reconfigure */
	    __ae_cursor_notsup,		/* remove */
	    __ae_cursor_notsup);	/* close */
	AE_DECL_RET;
	AE_INDEX *idx;
	AE_ITEM *key, v;
	bool bloom_found;

	key = cjoin->iter->curkey;
	entry->stats.accesses++;
	bloom_found = false;

	if (entry->bloom != NULL) {
		/*
		 * If we don't own the Bloom filter, we must be sharing one
		 * in a previous entry. So the shared filter has already
		 * been checked and passed.
		 */
		if (!F_ISSET(entry, AE_CURJOIN_ENTRY_OWN_BLOOM))
			return (0);

		/*
		 * If the item is not in the Bloom filter, we return
		 * immediately, otherwise, we still need to check the
		 * long way.
		 */
		AE_ERR(__ae_bloom_inmem_get(entry->bloom, key));
		bloom_found = true;
	}
	if (entry->index != NULL) {
		memset(&v, 0, sizeof(v));  /* Keep lint quiet. */
		c = entry->main;
		c->set_key(c, key);
		if ((ret = c->search(c)) == 0)
			ret = c->get_value(c, &v);
		else if (ret == AE_NOTFOUND)
			AE_ERR_MSG(session, AE_ERROR,
			    "main table for join is missing entry.");
		AE_TRET(c->reset(c));
		AE_ERR(ret);
	} else
		v = *key;

	if ((idx = entry->index) != NULL && idx->extractor != NULL) {
		extract_cursor.iface = iface;
		extract_cursor.iface.session = &session->iface;
		extract_cursor.iface.key_format = idx->exkey_format;
		extract_cursor.ismember = 0;
		extract_cursor.entry = entry;
		AE_ERR(idx->extractor->extract(idx->extractor,
		    &session->iface, key, &v, &extract_cursor.iface));
		if (!extract_cursor.ismember)
			AE_ERR(AE_NOTFOUND);
	} else
		AE_ERR(__curjoin_entry_in_range(session, entry, &v, skip_left));

	if (0) {
err:		if (ret == AE_NOTFOUND && bloom_found)
			entry->stats.bloom_false_positive++;
	}
	return (ret);
}

/*
 * __curjoin_next --
 *	AE_CURSOR::next for join cursors.
 */
static int
__curjoin_next(AE_CURSOR *cursor)
{
	AE_CURSOR_JOIN *cjoin;
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	bool skip_left;
	u_int i;

	cjoin = (AE_CURSOR_JOIN *)cursor;

	CURSOR_API_CALL(cursor, session, next, NULL);

	if (F_ISSET(cjoin, AE_CURJOIN_ERROR))
		AE_ERR_MSG(session, AE_ERROR,
		    "join cursor encountered previous error");
	if (!F_ISSET(cjoin, AE_CURJOIN_INITIALIZED))
		AE_ERR(__curjoin_init_iter(session, cjoin));

nextkey:
	if ((ret = __curjoin_entry_iter_next(cjoin->iter, &cursor->key,
	    &cursor->recno)) == 0) {
		F_SET(cursor, AE_CURSTD_KEY_EXT);

		/*
		 * We may have already established membership for the
		 * 'left' case for the first entry, since we're
		 * using that in our iteration.
		 */
		skip_left = F_ISSET(cjoin, AE_CURJOIN_SKIP_FIRST_LEFT);
		for (i = 0; i < cjoin->entries_next; i++) {
			ret = __curjoin_entry_member(session, cjoin,
			    &cjoin->entries[i], skip_left);
			if (ret == AE_NOTFOUND)
				goto nextkey;
			skip_left = false;
			AE_ERR(ret);
		}
	}

	if (0) {
err:		F_SET(cjoin, AE_CURJOIN_ERROR);
	}
	API_END_RET(session, ret);
}

/*
 * __curjoin_reset --
 *	AE_CURSOR::reset for join cursors.
 */
static int
__curjoin_reset(AE_CURSOR *cursor)
{
	AE_CURSOR_JOIN *cjoin;
	AE_DECL_RET;
	AE_SESSION_IMPL *session;

	cjoin = (AE_CURSOR_JOIN *)cursor;

	CURSOR_API_CALL(cursor, session, reset, NULL);

	if (F_ISSET(cjoin, AE_CURJOIN_INITIALIZED))
		AE_ERR(__curjoin_entry_iter_reset(cjoin->iter));

err:	API_END_RET(session, ret);
}

/*
 * __curjoin_close --
 *	AE_CURSOR::close for join cursors.
 */
static int
__curjoin_close(AE_CURSOR *cursor)
{
	AE_CURSOR_JOIN *cjoin;
	AE_CURSOR_JOIN_ENDPOINT *end;
	AE_CURSOR_JOIN_ENTRY *entry;
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	u_int i;

	cjoin = (AE_CURSOR_JOIN *)cursor;

	CURSOR_API_CALL(cursor, session, close, NULL);

	__ae_schema_release_table(session, cjoin->table);
	/* These are owned by the table */
	cursor->internal_uri = NULL;
	cursor->key_format = NULL;
	if (cjoin->projection != NULL) {
		__ae_free(session, cjoin->projection);
		__ae_free(session, cursor->value_format);
	}

	for (entry = cjoin->entries, i = 0; i < cjoin->entries_next;
		entry++, i++) {
		if (entry->main != NULL)
			AE_TRET(entry->main->close(entry->main));
		if (F_ISSET(entry, AE_CURJOIN_ENTRY_OWN_BLOOM))
			AE_TRET(__ae_bloom_close(entry->bloom));
		for (end = &entry->ends[0];
		     end < &entry->ends[entry->ends_next]; end++) {
			F_CLR(end->cursor, AE_CURSTD_JOINED);
			if (F_ISSET(end, AE_CURJOIN_END_OWN_KEY))
				__ae_free(session, end->key.data);
		}
		__ae_free(session, entry->ends);
	}

	if (cjoin->iter != NULL)
		AE_TRET(__curjoin_entry_iter_close(cjoin->iter));
	__ae_free(session, cjoin->entries);
	AE_TRET(__ae_cursor_close(cursor));

err:	API_END_RET(session, ret);
}

/*
 * __ae_curjoin_open --
 *	Initialize a join cursor.
 *
 *	Join cursors are read-only.
 */
int
__ae_curjoin_open(AE_SESSION_IMPL *session,
    const char *uri, AE_CURSOR *owner, const char *cfg[], AE_CURSOR **cursorp)
{
	AE_CURSOR_STATIC_INIT(iface,
	    __curjoin_get_key,		/* get-key */
	    __curjoin_get_value,	/* get-value */
	    __ae_cursor_notsup,		/* set-key */
	    __ae_cursor_notsup,		/* set-value */
	    __ae_cursor_notsup,		/* compare */
	    __ae_cursor_notsup,		/* equals */
	    __curjoin_next,		/* next */
	    __ae_cursor_notsup,		/* prev */
	    __curjoin_reset,		/* reset */
	    __ae_cursor_notsup,		/* search */
	    __ae_cursor_notsup,		/* search-near */
	    __ae_cursor_notsup,		/* insert */
	    __ae_cursor_notsup,		/* update */
	    __ae_cursor_notsup,		/* remove */
	    __ae_cursor_notsup,		/* reconfigure */
	    __curjoin_close);		/* close */
	AE_CURSOR *cursor;
	AE_CURSOR_JOIN *cjoin;
	AE_DECL_ITEM(tmp);
	AE_DECL_RET;
	AE_TABLE *table;
	size_t size;
	const char *tablename, *columns;

	AE_STATIC_ASSERT(offsetof(AE_CURSOR_JOIN, iface) == 0);

	if (!AE_PREFIX_SKIP(uri, "join:"))
		return (EINVAL);
	tablename = uri;
	if (!AE_PREFIX_SKIP(tablename, "table:"))
		return (EINVAL);

	columns = strchr(tablename, '(');
	if (columns == NULL)
		size = strlen(tablename);
	else
		size = AE_PTRDIFF(columns, tablename);
	AE_RET(__ae_schema_get_table(session, tablename, size, 0, &table));

	AE_RET(__ae_calloc_one(session, &cjoin));
	cursor = &cjoin->iface;
	*cursor = iface;
	cursor->session = &session->iface;
	cursor->internal_uri = table->name;
	cursor->key_format = table->key_format;
	cursor->value_format = table->value_format;
	cjoin->table = table;

	/* Handle projections. */
	AE_ERR(__ae_scr_alloc(session, 0, &tmp));
	if (columns != NULL) {
		AE_ERR(__ae_struct_reformat(session, table,
		    columns, strlen(columns), NULL, 1, tmp));
		AE_ERR(__ae_strndup(
		    session, tmp->data, tmp->size, &cursor->value_format));
		AE_ERR(__ae_strdup(session, columns, &cjoin->projection));
	}

	if (owner != NULL)
		AE_ERR(EINVAL);

	AE_ERR(__ae_cursor_init(cursor, uri, owner, cfg, cursorp));

	if (0) {
err:		AE_TRET(__curjoin_close(cursor));
		*cursorp = NULL;
	}

	__ae_scr_free(session, &tmp);
	return (ret);
}

/*
 * __ae_curjoin_join --
 *	Add a new join to a join cursor.
 */
int
__ae_curjoin_join(AE_SESSION_IMPL *session, AE_CURSOR_JOIN *cjoin,
    AE_INDEX *idx, AE_CURSOR *ref_cursor, uint8_t flags, uint8_t range,
    uint64_t count, uint32_t bloom_bit_count, uint32_t bloom_hash_count)
{
	AE_CURSOR_JOIN_ENTRY *entry;
	AE_DECL_RET;
	AE_CURSOR_JOIN_ENDPOINT *end, *newend;
	bool hasins, needbloom, range_eq;
	u_int i, ins, nonbloom;
	const char *raw_cfg[] = { AE_CONFIG_BASE(
	    session, AE_SESSION_open_cursor), "raw", NULL };
	char *main_uri;
	size_t namesize, newsize;

	entry = NULL;
	hasins = needbloom = false;
	ins = 0; /* -Wuninitialized */
	main_uri = NULL;
	nonbloom = 0; /* -Wuninitialized */
	namesize = strlen(cjoin->table->name);

	for (i = 0; i < cjoin->entries_next; i++) {
		if (cjoin->entries[i].index == idx) {
			entry = &cjoin->entries[i];
			break;
		}
		if (!needbloom && i > 0 &&
		    !F_ISSET(&cjoin->entries[i], AE_CURJOIN_ENTRY_BLOOM)) {
			needbloom = true;
			nonbloom = i;
		}
	}
	if (entry == NULL) {
		AE_ERR(__ae_realloc_def(session, &cjoin->entries_allocated,
		    cjoin->entries_next + 1, &cjoin->entries));
		if (LF_ISSET(AE_CURJOIN_ENTRY_BLOOM) && needbloom) {
			/*
			 * Reorder the list so that after the first entry,
			 * the Bloom filtered entries come next, followed by
			 * the non-Bloom entries.  Once the Bloom filters
			 * are built, determining membership via Bloom is
			 * faster than without Bloom, so we can answer
			 * membership questions more quickly, and with less
			 * I/O, with the Bloom entries first.
			 */
			entry = &cjoin->entries[nonbloom];
			memmove(entry + 1, entry,
			    (cjoin->entries_next - nonbloom) *
			    sizeof(AE_CURSOR_JOIN_ENTRY));
			memset(entry, 0, sizeof(AE_CURSOR_JOIN_ENTRY));
		}
		else
			entry = &cjoin->entries[cjoin->entries_next];
		entry->index = idx;
		entry->flags = flags;
		entry->count = count;
		entry->bloom_bit_count = bloom_bit_count;
		entry->bloom_hash_count = bloom_hash_count;
		++cjoin->entries_next;
	} else {
		/* Merge the join into an existing entry for this index */
		if (count != 0 && entry->count != 0 && entry->count != count)
			AE_ERR_MSG(session, EINVAL,
			    "count=%" PRIu64 " does not match "
			    "previous count=%" PRIu64 " for this index",
			    count, entry->count);
		if (LF_MASK(AE_CURJOIN_ENTRY_BLOOM) !=
		    F_MASK(entry, AE_CURJOIN_ENTRY_BLOOM))
			AE_ERR_MSG(session, EINVAL,
			    "join has incompatible strategy "
			    "values for the same index");

		/*
		 * Check against other comparisons (we call them endpoints)
		 * already set up for this index.
		 * We allow either:
		 *   - one or more "eq" (with disjunction)
		 *   - exactly one "eq" (with conjunction)
		 *   - exactly one of "gt" or "ge" (conjunction or disjunction)
		 *   - exactly one of "lt" or "le" (conjunction or disjunction)
		 *   - one of "gt"/"ge" along with one of "lt"/"le"
		 *         (currently restricted to conjunction).
		 *
		 * Some other combinations, although expressible either do
		 * not make sense (X == 3 AND X == 5) or are reducible (X <
		 * 7 AND X < 9).  Other specific cases of (X < 7 OR X > 15)
		 * or (X == 4 OR X > 15) make sense but we don't handle yet.
		 */
		for (i = 0; i < entry->ends_next; i++) {
			end = &entry->ends[i];
			range_eq = (range == AE_CURJOIN_END_EQ);
			if ((F_ISSET(end, AE_CURJOIN_END_GT) &&
			    ((range & AE_CURJOIN_END_GT) != 0 || range_eq)) ||
			    (F_ISSET(end, AE_CURJOIN_END_LT) &&
			    ((range & AE_CURJOIN_END_LT) != 0 || range_eq)) ||
			    (end->flags == AE_CURJOIN_END_EQ &&
			    (range & (AE_CURJOIN_END_LT | AE_CURJOIN_END_GT))
			    != 0))
				AE_ERR_MSG(session, EINVAL,
				    "join has overlapping ranges");
			if (range == AE_CURJOIN_END_EQ &&
			    end->flags == AE_CURJOIN_END_EQ &&
			    !F_ISSET(entry, AE_CURJOIN_ENTRY_DISJUNCTION))
				AE_ERR_MSG(session, EINVAL,
				    "compare=eq can only be combined "
				    "using operation=or");

			/*
			 * Sort "gt"/"ge" to the front, followed by any number
			 * of "eq", and finally "lt"/"le".
			 */
			if (!hasins &&
			    ((range & AE_CURJOIN_END_GT) != 0 ||
			    (range == AE_CURJOIN_END_EQ &&
			    !F_ISSET(end, AE_CURJOIN_END_GT)))) {
				ins = i;
				hasins = true;
			}
		}
		/* All checks completed, merge any new configuration now */
		entry->count = count;
		entry->bloom_bit_count =
		    AE_MAX(entry->bloom_bit_count, bloom_bit_count);
		entry->bloom_hash_count =
		    AE_MAX(entry->bloom_hash_count, bloom_hash_count);
	}
	AE_ERR(__ae_realloc_def(session, &entry->ends_allocated,
	    entry->ends_next + 1, &entry->ends));
	if (!hasins)
		ins = entry->ends_next;
	newend = &entry->ends[ins];
	memmove(newend + 1, newend,
	    (entry->ends_next - ins) * sizeof(AE_CURSOR_JOIN_ENDPOINT));
	memset(newend, 0, sizeof(AE_CURSOR_JOIN_ENDPOINT));
	entry->ends_next++;
	newend->cursor = ref_cursor;
	F_SET(newend, range);

	/* Open the main file with a projection of the indexed columns. */
	if (entry->main == NULL && entry->index != NULL) {
		namesize = strlen(cjoin->table->name);
		newsize = namesize + entry->index->colconf.len + 1;
		AE_ERR(__ae_calloc(session, 1, newsize, &main_uri));
		snprintf(main_uri, newsize, "%s%.*s",
		    cjoin->table->name, (int)entry->index->colconf.len,
		    entry->index->colconf.str);
		AE_ERR(__ae_open_cursor(session, main_uri,
		    (AE_CURSOR *)cjoin, raw_cfg, &entry->main));
	}

err:	if (main_uri != NULL)
		__ae_free(session, main_uri);
	return (ret);
}
