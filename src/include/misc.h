 ...
 */
#define	AE_UNUSED(var)		(void)(var)

/* Basic constants. */
#define	AE_THOUSAND	(1000)
#define	AE_MILLION	(1000000)
#define	AE_BILLION	(1000000000)

#define	AE_KILOBYTE	(1024)
#define	AE_MEGABYTE	(1048576)
#define	AE_GIGABYTE	(1073741824)
#define	AE_TERABYTE	((uint64_t)1099511627776)
#define	AE_PETABYTE	((uint64_t)1125899906842624)
#define	AE_EXABYTE	((uint64_t)1152921504606846976)
 ...
 */
#define	AE_STORE_SIZE(s)	((uint32_t)(s))
#define	AE_PTRDIFF(end, begin)						\
	((size_t)((uint8_t *)(end) - (uint8_t *)(begin)))
#define	AE_PTRDIFF32(end, begin)					\
	AE_STORE_SIZE(AE_PTRDIFF((end), (begin)))
#define	AE_BLOCK_FITS(p, len, begin, maxlen)				\
	((uint8_t *)(p) >= (uint8_t *)(begin) &&			\
	((uint8_t *)(p) + (len) <= (uint8_t *)(begin) + (maxlen)))
#define	AE_PTR_IN_RANGE(p, begin, maxlen)				\
	AE_BLOCK_FITS((p), 1, (begin), (maxlen))
...
#define	AE_MAX(a, b)	((a) < (b) ? (b) : (a))

/* Elements in an array. */
#define	AE_ELEMENTS(a)	(sizeof(a) / sizeof(a[0]))

/* 10 level skip lists, 1/4 have a link to the next element. */
#define	AE_SKIP_MAXDEPTH	10
#define	AE_SKIP_PROBABILITY	(UINT32_MAX >> 2)

/*
 * Encryption needs to know its original length before either the
 * block or logging subsystems pad.  Constant value.
 */
#define	AE_ENCRYPT_LEN_SIZE	sizeof(uint32_t)

/*
 * __ae_calloc_def, __ae_calloc_one --
 *	Most calloc calls don't need separate count or sizeof arguments.
 */
#define	__ae_calloc_def(session, number, addr)				\
	__ae_calloc(session, (size_t)(number), sizeof(**(addr)), addr)
#define	__ae_calloc_one(session, addr)					\
	__ae_calloc(session, (size_t)1, sizeof(**(addr)), addr)
 ...
 */
#define	__ae_realloc_def(session, sizep, number, addr)			\
	(((number) * sizeof(**(addr)) <= *(sizep)) ? 0 :		\
	    __ae_realloc(session, sizep, AE_MAX(*(sizep) * 2,		\
		AE_MAX(10, (number)) * sizeof(**(addr))), addr))
/*
 ...
 */
#define	__ae_free(session, p) do {					\
	if ((p) != NULL)						\
		__ae_free_int(session, (void *)&(p));			\
} while (0)
#ifdef HAVE_DIAGNOSTIC
#define	__ae_overwrite_and_free(session, p) do {			\
	memset(p, AE_DEBUG_BYTE, sizeof(*(p)));				\
	__ae_free(session, p);						\
} while (0)
#define	__ae_overwrite_and_free_len(session, p, len) do {		\
	memset(p, AE_DEBUG_BYTE, len);					\
	__ae_free(session, p);						\
} while (0)
#else
#define	__ae_overwrite_and_free(session, p)		__ae_free(session, p)
#define	__ae_overwrite_and_free_len(session, p, len)	__ae_free(session, p)
#endif
 ...
 */
#define	AE_INSERTION_SORT(arrayp, n, value_type, compare_lt) do {	\
	value_type __v;							\
 ...
 */
#define	AE_BINARY_SEARCH(key, arrayp, n, found) do {			\
...
/* Verbose messages. */
#ifdef HAVE_VERBOSE
#define	AE_VERBOSE_ISSET(session, f)					\
	(FLD_ISSET(S2C(session)->verbose, f))
#else
#define	AE_VERBOSE_ISSET(session, f)	0
#endif
 ...
 */
#define	AE_UNCHECKED_STRING(str) #str

/* Function return value and scratch buffer declaration and initialization. */
#define	AE_DECL_ITEM(i)	AE_ITEM *i = NULL
#define	AE_DECL_RET	int ret = 0

/* If a AE_ITEM data field points somewhere in its allocated memory. */
#define	AE_DATA_IN_ITEM(i)						\
	((i)->mem != NULL && (i)->data >= (i)->mem &&			\
	    AE_PTRDIFF((i)->data, (i)->mem) < (i)->memsize)

/* Copy the data and size fields of an item. */
#define	AE_ITEM_SET(dst, src) do {					\
	(dst).data = (src).data;					\
	(dst).size = (src).size;					\
} while (0)
 ...
 */
#ifdef HAVE_DIAGNOSTIC
#define	__ae_scr_alloc(session, size, scratchp)				\
	__ae_scr_alloc_func(session, size, scratchp, __FILE__, __LINE__)
#define	__ae_page_in(session, ref, flags)				\
	__ae_page_in_func(session, ref, flags, __FILE__, __LINE__)
#define	__ae_page_swap(session, held, want, flags)			\
	__ae_page_swap_func(session, held, want, flags, __FILE__, __LINE__)
#else
#define	__ae_scr_alloc(session, size, scratchp)				\
	__ae_scr_alloc_func(session, size, scratchp)
#define	__ae_page_in(session, ref, flags)				\
	__ae_page_in_func(session, ref, flags)
#define	__ae_page_swap(session, held, want, flags)			\
	__ae_page_swap_func(session, held, want, flags)
#endif
