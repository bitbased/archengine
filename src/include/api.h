...
#define	API_SESSION_INIT(s, h, n, cur, dh)				\
	AE_DATA_HANDLE *__olddh = (s)->dhandle;				\
...
#define	API_CALL_NOCONF(s, h, n, cur, dh) do {				\
	...
	AE_ERR(AE_SESSION_CHECK_PANIC(s));				\
	AE_ERR(__ae_verbose((s), AE_VERB_API, "CALL: " #h ":" #n))

#define	API_CALL(s, h, n, cur, dh, config, cfg) do {			\
	const char *cfg[] =						\
	    { AE_CONFIG_BASE(s, h##_##n), config, NULL };		\
	API_SESSION_INIT(s, h, n, cur, dh);				\
	AE_ERR(AE_SESSION_CHECK_PANIC(s));				\
	if ((config) != NULL)						\
		AE_ERR(__ae_config_check((s),				\
		    AE_CONFIG_REF(session, h##_##n), (config), 0));	\
	AE_ERR(__ae_verbose((s), AE_VERB_API, "CALL: " #h ":" #n))
...
#define	API_END(s, ret)							\
	if ((s) != NULL) {						\
		(s)->dhandle = __olddh;					\
		(s)->name = __oldname;					\
		if (F_ISSET(&(s)->txn, AE_TXN_RUNNING) &&		\
		    (ret) != 0 &&					\
		    (ret) != AE_NOTFOUND &&				\
		    (ret) != AE_DUPLICATE_KEY)				\
			F_SET(&(s)->txn, AE_TXN_ERROR);			\
	}								\
} while (0)
...
#define	TXN_API_CALL(s, h, n, cur, bt, config, cfg) do {		\
	...
	__autotxn = !F_ISSET(&(s)->txn, AE_TXN_AUTOCOMMIT | AE_TXN_RUNNING);\
	if (__autotxn)							\
		F_SET(&(s)->txn, AE_TXN_AUTOCOMMIT)
...
#define	TXN_API_END_RETRY(s, ret, retry)				\
	API_END(s, ret);						\
	if (__autotxn) {						\
		if (F_ISSET(&(s)->txn, AE_TXN_AUTOCOMMIT))		\
			F_CLR(&(s)->txn, AE_TXN_AUTOCOMMIT);		\
		else if (ret == 0 && !F_ISSET(&(s)->txn, AE_TXN_ERROR))	\
			ret = __ae_txn_commit((s), NULL);		\
		else {							\
			AE_TRET(__ae_txn_rollback((s), NULL));		\
			if ((ret == 0 || ret == AE_ROLLBACK) &&		\
			    (retry)) {					\
				ret = 0;				\
				continue;				\
			}						\
			AE_TRET(__ae_session_reset_cursors(s, false));	\
		}							\
	}								\
	break;								\
} while (1)
 ...
 * If a session or connection method is about to return AE_NOTFOUND (some
...
#define	CONNECTION_API_CALL(conn, s, n, config, cfg)			\
	...
	API_CALL(s, AE_CONNECTION, n, NULL, NULL, config, cfg)

#define	CONNECTION_API_CALL_NOCONF(conn, s, n)				\
	s = (conn)->default_session;					\
	API_CALL_NOCONF(s, AE_CONNECTION, n, NULL, NULL)

#define	SESSION_API_CALL(s, n, config, cfg)				\
	API_CALL(s, AE_SESSION, n, NULL, NULL, config, cfg)

#define	SESSION_API_CALL_NOCONF(s, n)					\
	API_CALL_NOCONF(s, AE_SESSION, n, NULL, NULL)

#define	SESSION_TXN_API_CALL(s, n, config, cfg)				\
	TXN_API_CALL(s, AE_SESSION, n, NULL, NULL, config, cfg)

#define	CURSOR_API_CALL(cur, s, n, bt)					\
	(s) = (AE_SESSION_IMPL *)(cur)->session;			\
	API_CALL_NOCONF(s, AE_CURSOR, n, cur,				\
	    ((bt) == NULL) ? NULL : ((AE_BTREE *)(bt))->dhandle)

#define	JOINABLE_CURSOR_CALL_CHECK(cur)					\
	if (F_ISSET(cur, AE_CURSTD_JOINED))				\
		AE_ERR(__ae_curindex_joined(cur))
...
#define	CURSOR_REMOVE_API_CALL(cur, s, bt)				\
	(s) = (AE_SESSION_IMPL *)(cur)->session;			\
	TXN_API_CALL_NOCONF(s, AE_CURSOR, remove, cur,			\
	    ((bt) == NULL) ? NULL : ((AE_BTREE *)(bt))->dhandle);
...
#define	CURSOR_UPDATE_API_CALL(cur, s, n, bt)				\
	(s) = (AE_SESSION_IMPL *)(cur)->session;			\
	TXN_API_CALL_NOCONF(s, AE_CURSOR, n, cur,			\
	    ((bt) == NULL) ? NULL : ((AE_BTREE *)(bt))->dhandle);	\
	if (F_ISSET(S2C(s), AE_CONN_IN_MEMORY) && __ae_cache_full(s))	\
		AE_ERR(AE_CACHE_FULL);
