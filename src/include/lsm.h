...
struct __ae_lsm_worker_args {
	AE_SESSION_IMPL	*session;	/* Session */
	AE_CONDVAR	*work_cond;	/* Owned by the manager */
	ae_thread_t	tid;		/* Thread id */
	u_int		id;		/* My manager slot id */
	uint32_t	type;		/* Types of operations handled */
#define	AE_LSM_WORKER_RUN	0x01
...
struct __ae_cursor_lsm {
	...
	AE_BLOOM **blooms;		/* Bloom filter handles. */
	size_t bloom_alloc;

	AE_CURSOR **cursors;		/* Cursor handles. */
	size_t cursor_alloc;

	AE_CURSOR *current;     	/* The current cursor for iteration */
	AE_LSM_CHUNK *primary_chunk;	/* The current primary chunk */
...
#define	AE_CLSM_ACTIVE		0x001   /* Incremented the session count */
#define	AE_CLSM_BULK		0x002   /* Open for snapshot isolation */
#define	AE_CLSM_ITERATE_NEXT    0x004   /* Forward iteration */
#define	AE_CLSM_ITERATE_PREV    0x008   /* Backward iteration */
#define	AE_CLSM_MERGE           0x010   /* Merge cursor, don't update */
#define	AE_CLSM_MINOR_MERGE	0x020   /* Minor merge, include tombstones */
#define	AE_CLSM_MULTIPLE        0x040   /* Multiple cursors have values for the
					   current key */
#define	AE_CLSM_OPEN_READ	0x080   /* Open for reads */
#define	AE_CLSM_OPEN_SNAPSHOT	0x100   /* Open for snapshot isolation */
	uint32_t flags;
};
...
#define	AE_LSM_CHUNK_BLOOM	0x01
#define	AE_LSM_CHUNK_MERGING	0x02
#define	AE_LSM_CHUNK_ONDISK	0x04
#define	AE_LSM_CHUNK_STABLE	0x08
	uint32_t flags;
};
 ...
 * is required.
 */
#define	AE_LSM_WORK_BLOOM	0x01	/* Create a bloom filter */
#define	AE_LSM_WORK_DROP	0x02	/* Drop unused chunks */
#define	AE_LSM_WORK_FLUSH	0x04	/* Flush a chunk to disk */
#define	AE_LSM_WORK_MERGE	0x08	/* Look for a tree merge */
#define	AE_LSM_WORK_SWITCH	0x10	/* Switch to new in-memory chunk */
...
struct __ae_lsm_work_unit {
	TAILQ_ENTRY(__ae_lsm_work_unit) q;	/* Worker unit queue */
	uint32_t	type;			/* Type of operation */
#define	AE_LSM_WORK_FORCE	0x0001		/* Force operation */
...
struct __ae_lsm_manager {
	 ...
	 */
	TAILQ_HEAD(__ae_lsm_work_switch_qh, __ae_lsm_work_unit)  switchqh;
	TAILQ_HEAD(__ae_lsm_work_app_qh, __ae_lsm_work_unit)	  appqh;
	TAILQ_HEAD(__ae_lsm_work_manager_qh, __ae_lsm_work_unit) managerqh;
	AE_SPINLOCK	switch_lock;	/* Lock for switch queue */
	AE_SPINLOCK	app_lock;	/* Lock for application queue */
	AE_SPINLOCK	manager_lock;	/* Lock for manager queue */
	AE_CONDVAR     *work_cond;	/* Used to notify worker of activity */
	uint32_t	lsm_workers;	/* Current number of LSM workers */
	uint32_t	lsm_workers_max;
#define	AE_LSM_MAX_WORKERS	20
#define	AE_LSM_MIN_WORKERS	3
	AE_LSM_WORKER_ARGS lsm_worker_cookies[AE_LSM_MAX_WORKERS];
};
 ...
 */
#define	AE_LSM_AGGRESSIVE_THRESHOLD	2
...
#define	AE_LSM_BLOOM_MERGED				0x00000001
#define	AE_LSM_BLOOM_OFF				0x00000002
#define	AE_LSM_BLOOM_OLDEST				0x00000004
	uint32_t bloom;			/* Bloom creation policy */

	AE_LSM_CHUNK **chunk;		/* Array of active LSM chunks */
	...
	AE_LSM_CHUNK **old_chunks;	/* Array of old LSM chunks */
	 ...
	 */
#define	AE_LSM_TREE_STAT_INCR(session, fld) do {			\
	if (FLD_ISSET(S2C(session)->stat_flags, AE_CONN_STAT_FAST))	\
		++(fld);						\
} while (0)
#define	AE_LSM_TREE_STAT_INCRV(session, fld, v) do {			\
	if (FLD_ISSET(S2C(session)->stat_flags, AE_CONN_STAT_FAST))	\
		(fld) += (int64_t)(v);					\
} while (0)
...
#define	AE_LSM_TREE_ACTIVE		0x01	/* Workers are active */
#define	AE_LSM_TREE_AGGRESSIVE_TIMER	0x02	/* Timer for merge aggression */
#define	AE_LSM_TREE_COMPACTING		0x04	/* Tree being compacted */
#define	AE_LSM_TREE_MERGES		0x08	/* Tree should run merges */
#define	AE_LSM_TREE_NEED_SWITCH		0x10	/* New chunk needs creating */
#define	AE_LSM_TREE_OPEN		0x20	/* The tree is open */
#define	AE_LSM_TREE_THROTTLE		0x40	/* Throttle updates */
	uint32_t flags;
};

/*
 * AE_LSM_DATA_SOURCE --
 *	Implementation of the AE_DATA_SOURCE interface for LSM.
 */
struct __ae_lsm_data_source {
	AE_DATA_SOURCE iface;
