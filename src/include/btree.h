 ...
 */
#define	AE_BTREE_MAJOR_VERSION_MIN	1	/* Oldest version supported */
#define	AE_BTREE_MINOR_VERSION_MIN	1

#define	AE_BTREE_MAJOR_VERSION_MAX	1	/* Newest version supported */
#define	AE_BTREE_MINOR_VERSION_MAX	1
 ...
 * for the magic.
 */
#define	AE_BTREE_PAGE_SIZE_MAX		(512 * AE_MEGABYTE)
 ...
 * However, in the AE_UPDATE structure we use the UINT32_MAX size as a "deleted"
 ...
 */
#define	AE_BTREE_MAX_OBJECT_SIZE	(UINT32_MAX - 1024)
 ...
 */
#define	AE_BTREE_MAX_ADDR_COOKIE	255	/* Maximum address cookie */

/* Evict pages if we see this many consecutive deleted records. */
#define	AE_BTREE_DELETE_THRESHOLD	1000
...
struct __ae_btree {
	...
	AE_CKPT	  *ckpt;		/* Checkpoint information */
	...
	AE_COLLATOR *collator;		/* Row-store comparator */
...
#define	AE_SPLIT_DEEPEN_MIN_CHILD_DEF	10000
	u_int split_deepen_min_child;	/* Minimum entries to deepen tree */
#define	AE_SPLIT_DEEPEN_PER_CHILD_DEF	100
	u_int split_deepen_per_child;	/* Entries per child when deepened */
	int   split_pct;		/* Split page percent */
	AE_COMPRESSOR *compressor;	/* Page compressor */
	AE_KEYED_ENCRYPTOR *kencryptor;	/* Page encryptor */
	AE_RWLOCK *ovfl_lock;		/* Overflow lock */

	uint64_t last_recno;		/* Column-store last record number */

	AE_REF root;			/* Root page reference */
	int modified;			/* If the tree ever modified */
	bool bulk_load_ok;		/* Bulk-load is a possibility */

	AE_BM	*bm;			/* Block manager reference */
	u_int	 block_header;		/* AE_PAGE_HEADER_BYTE_SIZE */

	uint64_t checkpoint_gen;	/* Checkpoint generation */
	uint64_t rec_max_txn;		/* Maximum txn seen (clean trees) */
	uint64_t write_gen;		/* Write generation */

	AE_REF  *evict_ref;		/* Eviction thread's location */
	...
	enum {
		AE_CKPT_OFF, AE_CKPT_PREPARE, AE_CKPT_RUNNING
	 ...
	 */
	AE_SPINLOCK	flush_lock;	/* Lock to flush the tree's pages */

	/* Flags values up to 0xff are reserved for AE_DHANDLE_* */
#define	AE_BTREE_BULK		0x00100	/* Bulk-load handle */
#define	AE_BTREE_IN_MEMORY	0x00200	/* Cache-resident object */
#define	AE_BTREE_LOOKASIDE	0x00400	/* Look-aside table */
#define	AE_BTREE_NO_CHECKPOINT	0x00800	/* Disable checkpoints */
#define	AE_BTREE_NO_EVICTION	0x01000	/* Disable eviction */
#define	AE_BTREE_NO_LOGGING	0x02000	/* Disable logging */
#define	AE_BTREE_SALVAGE	0x04000	/* Handle is for salvage */
#define	AE_BTREE_SKIP_CKPT	0x08000	/* Handle skipped checkpoint */
#define	AE_BTREE_UPGRADE	0x10000	/* Handle is for upgrade */
#define	AE_BTREE_VERIFY		0x20000	/* Handle is for verify */
	uint32_t flags;
};

/* Flags that make a btree handle special (not for normal use). */
#define	AE_BTREE_SPECIAL_FLAGS	 					\
	(AE_BTREE_BULK | AE_BTREE_SALVAGE | AE_BTREE_UPGRADE | AE_BTREE_VERIFY)
