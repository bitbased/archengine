...
/*
 * Initialize a static AE_CURSOR structure.
 */
#define	AE_CURSOR_STATIC_INIT(n,					\
	get_key,							\
	get_value,							\
	set_key,							\
	set_value,							\
	...
	reconfigure,							\
	close)								\
	static const AE_CURSOR n = {					\
	NULL,				/* session */			\
	NULL,				/* uri */			\
	NULL,				/* key_format */		\
	NULL,				/* value_format */		\
	(int (*)(AE_CURSOR *, ...))(get_key),				\
	(int (*)(AE_CURSOR *, ...))(get_value),				\
	(void (*)(AE_CURSOR *, ...))(set_key),				\
	(void (*)(AE_CURSOR *, ...))(set_value),			\
	(int (*)(AE_CURSOR *, AE_CURSOR *, int *))(compare),		\
	(int (*)(AE_CURSOR *, AE_CURSOR *, int *))(equals),		\
	next,								\
	prev,								\
	reset,								\
	search,								\
	(int (*)(AE_CURSOR *, int *))(search_near),			\
	insert,								\
	update,								\
	remove,								\
	close,								\
	(int (*)(AE_CURSOR *, const char *))(reconfigure),		\
	{ NULL, NULL },			/* TAILQ_ENTRY q */		\
	0,				/* recno key */			\
	{ 0 },				/* recno raw buffer */		\
	NULL,				/* json_private */		\
	NULL,				/* lang_private */		\
	{ NULL, 0, 0, NULL, 0 },	/* AE_ITEM key */		\
	{ NULL, 0, 0, NULL, 0 },	/* AE_ITEM value */		\
	0,				/* int saved_err */		\
	NULL,				/* internal_uri */		\
	0				/* uint32_t flags */		\
}
...
struct __ae_cursor_backup {
	...
	AE_CURSOR_BACKUP_ENTRY *list;	/* List of files to be copied. */
	size_t list_allocated;
	size_t list_next;
};
#define	AE_CURSOR_BACKUP_ID(cursor)	(((AE_CURSOR_BACKUP *)cursor)->maxid)

struct __ae_cursor_btree {
	AE_CURSOR iface;

	AE_BTREE *btree;		/* Enclosing btree */

	/*
	 * The following fields are set by the search functions as a precursor
	 * to page modification: we have a page, a AE_COL/AE_ROW slot on the
	 ...
	 */
	AE_REF	  *ref;			/* Current page */
	uint32_t   slot;		/* AE_COL/AE_ROW 0-based slot */

	AE_INSERT_HEAD	*ins_head;	/* Insert chain head */
	AE_INSERT	*ins;		/* Current insert node */
					/* Search stack */
	AE_INSERT	**ins_stack[AE_SKIP_MAXDEPTH];

					/* Next item(s) found during search */
	AE_INSERT	*next_stack[AE_SKIP_MAXDEPTH];
	 ...
	 * vice-versa is just too complicated), so we map the AE_ROW and
	 ...
	 */
	AE_COL *cip_saved;		/* Last iteration reference */
	 ...
	 * search points).  We can't use the application's AE_CURSOR key field
	 ...
	 */
	AE_ROW *rip_saved;		/* Last-returned key reference */
	 ...
	 * functions, used to avoid a data copy in the AE_CURSOR.update call.
	 */
	AE_UPDATE *modify_update;
...
#define	AE_CBT_ACTIVE		0x01	/* Active in the tree */
#define	AE_CBT_ITERATE_APPEND	0x02	/* Col-store: iterating append list */
#define	AE_CBT_ITERATE_NEXT	0x04	/* Next iteration configuration */
#define	AE_CBT_ITERATE_PREV	0x08	/* Prev iteration configuration */
#define	AE_CBT_MAX_RECORD	0x10	/* Col-store: past end-of-table */
#define	AE_CBT_NO_TXN   	0x20	/* Non-transactional cursor
					   (e.g. on a checkpoint) */
#define	AE_CBT_SEARCH_SMALLEST	0x40	/* Row-store: small-key insert list */

#define	AE_CBT_POSITION_MASK		/* Flags associated with position */ \
	(AE_CBT_ITERATE_APPEND | AE_CBT_ITERATE_NEXT | AE_CBT_ITERATE_PREV | \
	AE_CBT_MAX_RECORD | AE_CBT_SEARCH_SMALLEST)
...
struct __ae_cursor_bulk {
	 ...
	 * to avoid corruption.
	 */
	AE_ITEM last;			/* Last key/value seen */
...
struct __ae_cursor_data_source {
	...
	AE_COLLATOR *collator;		/* Configured collator */
	int collator_owned;		/* Collator needs to be terminated */

	AE_CURSOR *source;		/* Application-owned cursor */
};
...
#define	AE_CURJOIN_END_LT	0x01		/* include values <  cursor */
#define	AE_CURJOIN_END_EQ	0x02		/* include values == cursor */
#define	AE_CURJOIN_END_GT	0x04		/* include values >  cursor */
#define	AE_CURJOIN_END_GE	(AE_CURJOIN_END_GT | AE_CURJOIN_END_EQ)
#define	AE_CURJOIN_END_LE	(AE_CURJOIN_END_LT | AE_CURJOIN_END_EQ)
#define	AE_CURJOIN_END_OWN_KEY	0x08		/* must free key's data */
	uint8_t			 flags;		/* range for this endpoint */
};

struct __ae_cursor_join_entry {
	AE_INDEX		*index;
	AE_CURSOR		*main;		/* raw main table cursor */
	AE_BLOOM		*bloom;		/* Bloom filter handle */
...
#define	AE_CURJOIN_ENTRY_BLOOM		0x01	/* use a bloom filter */
#define	AE_CURJOIN_ENTRY_DISJUNCTION	0x02	/* endpoints are or-ed */
#define	AE_CURJOIN_ENTRY_OWN_BLOOM	0x04	/* this entry owns the bloom */
	uint8_t			 flags;

	AE_CURSOR_JOIN_ENDPOINT	*ends;		/* reference endpoints */
	size_t			 ends_allocated;
	u_int			 ends_next;

	AE_JOIN_STATS		 stats;		/* Join statistics */
};
...
struct __ae_cursor_join {
	...
	AE_CURSOR_JOIN_ENTRY	*entries;
	size_t			 entries_allocated;
	u_int			 entries_next;
	uint8_t			 recno_buf[10];	/* holds packed recno */

#define	AE_CURJOIN_ERROR		0x01	/* Error in initialization */
#define	AE_CURJOIN_INITIALIZED		0x02	/* Successful initialization */
#define	AE_CURJOIN_SKIP_FIRST_LEFT	0x04	/* First check not needed */
	uint8_t			 flags;
};
...
struct __ae_cursor_json {
	...
	AE_CONFIG_ITEM key_names;	/* Names of key columns */
	AE_CONFIG_ITEM value_names;	/* Names of value columns */
};

struct __ae_cursor_log {
	AE_CURSOR iface;

	AE_LSN		*cur_lsn;	/* LSN of current record */
	AE_LSN		*next_lsn;	/* LSN of next record */
	AE_ITEM		*logrec;	/* Copy of record for cursor */
	AE_ITEM		*opkey, *opvalue;	/* Op key/value copy */
...
struct __ae_cursor_metadata {
	...
	AE_CURSOR *file_cursor;		/* Queries of regular metadata */

#define	AE_MDC_CREATEONLY	0x01
#define	AE_MDC_ONMETADATA	0x02
#define	AE_MDC_POSITIONED	0x04
	uint32_t flags;
};

struct __ae_join_stats_group {
	const char *desc_prefix;	/* Prefix appears before description */
	AE_CURSOR_JOIN *join_cursor;
	ssize_t join_cursor_entry;	/* Position in entries */
	AE_JOIN_STATS join_stats;
};
...
struct __ae_cursor_stat {
	...
	int	     (*stats_desc)(AE_CURSOR_STAT *, int, const char **);
					/* Statistics descriptions */
	int	     (*next_set)(AE_SESSION_IMPL *, AE_CURSOR_STAT *, bool,
			 bool);		/* Advance to next set */

	union {				/* Copies of the statistics */
		AE_DSRC_STATS dsrc_stats;
		AE_CONNECTION_STATS conn_stats;
		AE_JOIN_STATS_GROUP join_stats_group;
	} u;
	...
	AE_ITEM	 pv;			/* Current stats value (string) */

	/* Uses the same values as AE_CONNECTION::stat_flags field */
	uint32_t flags;
};
 ...
 */
#define	AE_CURSOR_STATS(cursor)						\
	(((AE_CURSOR_STAT *)cursor)->stats)
...
struct __ae_cursor_table {
	...
	AE_ITEM *cg_valcopy;		/*
					 ...
					 */
	AE_CURSOR **idx_cursors;
};

#define	AE_CURSOR_PRIMARY(cursor)					\
	(((AE_CURSOR_TABLE *)cursor)->cg_cursors[0])

#define	AE_CURSOR_RECNO(cursor)	AE_STREQ((cursor)->key_format, "r")

/*
 * AE_CURSOR_NEEDKEY, AE_CURSOR_NEEDVALUE --
 ...
 */
#define	AE_CURSOR_CHECKKEY(cursor) do {					\
	if (!F_ISSET(cursor, AE_CURSTD_KEY_SET))			\
		AE_ERR(__ae_cursor_kv_not_set(cursor, true));		\
} while (0)
#define	AE_CURSOR_CHECKVALUE(cursor) do {				\
	if (!F_ISSET(cursor, AE_CURSTD_VALUE_SET))			\
		AE_ERR(__ae_cursor_kv_not_set(cursor, false));		\
} while (0)
#define	AE_CURSOR_NEEDKEY(cursor) do {					\
	if (F_ISSET(cursor, AE_CURSTD_KEY_INT)) {			\
		if (!AE_DATA_IN_ITEM(&(cursor)->key))			\
			AE_ERR(__ae_buf_set(				\
			    (AE_SESSION_IMPL *)(cursor)->session,	\
			    &(cursor)->key,				\
...
#define	AE_CURSOR_NEEDVALUE(cursor) do {				\
	if (F_ISSET(cursor, AE_CURSTD_VALUE_INT)) {			\
		if (!AE_DATA_IN_ITEM(&(cursor)->value))			\
			AE_ERR(__ae_buf_set(				\
			    (AE_SESSION_IMPL *)(cursor)->session,	\
			    &(cursor)->value,				\
...
#define	AE_CURSOR_NOVALUE(cursor) do {					\
	F_CLR(cursor, AE_CURSTD_VALUE_INT);				\
} while (0)

#define	AE_CURSOR_RAW_OK						\
	AE_CURSTD_DUMP_HEX | AE_CURSTD_DUMP_PRINT | AE_CURSTD_RAW
