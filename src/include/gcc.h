...
#define	AE_COMPILER_TYPE_ALIGN(x)	__attribute__((aligned(x)))
...
#define	AE_GCC_FUNC_DECL_ATTRIBUTE(x) __attribute__(x)

/*
 * Atomic writes:
 *
 * ArchEngine requires pointers (void *) and some variables to be read/written
 ...
 * 24-bits were written, ArchEngine would break. Or, if two threads of control
 ...
 *
 * ArchEngine additionally requires atomic writes for 64-bit memory locations,
 ...
 * AE_READ_BARRIER and AE_WRITE_BARRIER macros.  ArchEngine's requirement as
 ...
 *
 * In other words, the AE_WRITE_BARRIER macro must ensure that memory stores by
 * the processor, made before the AE_WRITE_BARRIER call, be visible to all
 * processors in the system before any memory stores by the processor, made
 * after the AE_WRITE_BARRIER call, are visible to any processor.  The
 * AE_READ_BARRIER macro ensures that all loads before the barrier are complete
 ...
 * for details.
 */
#define	AE_ATOMIC_CAS(ptr, oldval, newval)				\
...
{									\
	return (AE_ATOMIC_CAS(vp, old, new));				\
}
...
{
	return (AE_ATOMIC_CAS((void **)vp, old, new));
}

/* Compile read-write barrier */
#define	AE_BARRIER() __asm__ volatile("" ::: "memory")

#if defined(x86_64) || defined(__x86_64__)
/* Pause instruction to prevent excess processor bus usage */
#define	AE_PAUSE() __asm__ volatile("pause\n" ::: "memory")

#define	AE_FULL_BARRIER() do {						\
	__asm__ volatile ("mfence" ::: "memory");			\
} while (0)
#define	AE_READ_BARRIER() do {						\
	__asm__ volatile ("lfence" ::: "memory");			\
} while (0)
#define	AE_WRITE_BARRIER() do {						\
...
#define	AE_READ_BARRIER()	AE_FULL_BARRIER()
#define	AE_WRITE_BARRIER()	AE_FULL_BARRIER()

#elif defined(__PPC64__) || defined(PPC64)
#define	AE_PAUSE()	__asm__ volatile("ori 0,0,0" ::: "memory")
#define	AE_FULL_BARRIER()	do {
...
#define	AE_PAUSE()	__asm__ volatile("yield" ::: "memory")
#define	AE_FULL_BARRIER() do {						\
