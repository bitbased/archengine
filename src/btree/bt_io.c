 ...
 */
int
__ae_bt_read(AE_SESSION_IMPL *session,
    AE_ITEM *buf, const uint8_t *addr, size_t addr_size)
{
	...
	if (btree->compressor == NULL && btree->kencryptor == NULL) {
		AE_RET(bm->read(bm, session, buf, addr, addr_size));
		dsk = buf->data;
		ip = NULL;
	} else {
		AE_RET(__ae_scr_alloc(session, 0, &tmp));
		AE_ERR(bm->read(bm, session, tmp, addr, addr_size));
		dsk = tmp->data;
		ip = tmp;
	}
	...
	if (F_ISSET(dsk, AE_PAGE_ENCRYPTED)) {
		...
		AE_ERR(__ae_scr_alloc(session, 0, &etmp));
		if ((ret = __ae_decrypt(session,
		    encryptor, AE_BLOCK_ENCRYPT_SKIP, ip, etmp)) != 0) {
	...
	if (F_ISSET(dsk, AE_PAGE_COMPRESSED)) {
		 ...
		 * from decompression.
		 */
		AE_ERR(__ae_buf_initsize(session, buf, dsk->mem_size));
		 ...
		 * an example.
		 */
		memcpy(buf->mem, ip->data, AE_BLOCK_COMPRESS_SKIP);
		ret = btree->compressor->decompress(
		    btree->compressor, &session->iface,
		    (uint8_t *)ip->data + AE_BLOCK_COMPRESS_SKIP,
		    tmp->size - AE_BLOCK_COMPRESS_SKIP,
		    (uint8_t *)buf->mem + AE_BLOCK_COMPRESS_SKIP,
		    dsk->mem_size - AE_BLOCK_COMPRESS_SKIP, &result_len);
		 ...
		 */
		if (ret != 0 ||
		    result_len != dsk->mem_size - AE_BLOCK_COMPRESS_SKIP) {
			fail_msg = "block decryption failed";
			goto corrupt;
		}
	} else
	...
	if (F_ISSET(btree, AE_BTREE_VERIFY)) {
		if (tmp == NULL)
			AE_ERR(__ae_scr_alloc(session, 0, &tmp));
		AE_ERR(bm->addr_string(bm, session, tmp, addr, addr_size));
		AE_ERR(__ae_verify_dsk(session, tmp->data, buf));
	}

	AE_STAT_FAST_CONN_INCR(session, cache_read);
	AE_STAT_FAST_DATA_INCR(session, cache_read);
	if (F_ISSET(dsk, AE_PAGE_COMPRESSED))
		AE_STAT_FAST_DATA_INCR(session, compress_read);
	AE_STAT_FAST_CONN_INCRV(session, cache_bytes_read, dsk->mem_size);
	AE_STAT_FAST_DATA_INCRV(session, cache_bytes_read, dsk->mem_size);
...
corrupt:	if (ret == 0)
		...
		if (!F_ISSET(btree, AE_BTREE_VERIFY) &&
			...
			__ae_err(session, ret, "%s", fail_msg);
			ret = __ae_illegal_value(session, btree->dhandle->name);
		}
	}

err:	__ae_scr_free(session, &tmp);
	__ae_scr_free(session, &etmp);
	return (ret);
}
 ...
 */
int
__ae_bt_write(AE_SESSION_IMPL *session, AE_ITEM *buf,
    uint8_t *addr, size_t *addr_sizep, bool checkpoint, bool compressed)
{
	...
	AE_ASSERT(session, !F_ISSET(S2C(session), AE_CONN_IN_MEMORY));
	...
	dsk = buf->mem;
	if (compressed) {
		AE_ERR(__ae_scr_alloc(session, dsk->mem_size, &ctmp));

		memcpy(ctmp->mem, buf->data, AE_BLOCK_COMPRESS_SKIP);
		AE_ERR(btree->compressor->decompress(
		    btree->compressor, &session->iface,
		    (uint8_t *)buf->data + AE_BLOCK_COMPRESS_SKIP,
		    buf->size - AE_BLOCK_COMPRESS_SKIP,
		    (uint8_t *)ctmp->data + AE_BLOCK_COMPRESS_SKIP,
		    ctmp->memsize - AE_BLOCK_COMPRESS_SKIP,
		    &result_len));
		AE_ASSERT(session,
		    dsk->mem_size == result_len + AE_BLOCK_COMPRESS_SKIP);
		ctmp->size = (uint32_t)result_len + AE_BLOCK_COMPRESS_SKIP;
		ip = ctmp;
	} else {
		AE_ASSERT(session, dsk->mem_size == buf->size);
		ip = buf;
	}
	AE_ERR(__ae_verify_dsk(session, "[write-check]", ip));
	__ae_scr_free(session, &ctmp);
#endif
	...
	if (btree->compressor == NULL ||
		...
		ip = buf;
		AE_STAT_FAST_DATA_INCR(session, compress_write_too_small);
	} else {
		/* Skip the header bytes of the source data. */
		src = (uint8_t *)buf->mem + AE_BLOCK_COMPRESS_SKIP;
		src_len = buf->size - AE_BLOCK_COMPRESS_SKIP;
		...
		if (btree->compressor->pre_size == NULL)
			...
			AE_ERR(btree->compressor->pre_size(btree->compressor,
			    &session->iface, src, src_len, &len));

		size = len + AE_BLOCK_COMPRESS_SKIP;
		AE_ERR(bm->write_size(bm, session, &size));
		AE_ERR(__ae_scr_alloc(session, size, &ctmp));

		/* Skip the header bytes of the destination data. */
		dst = (uint8_t *)ctmp->mem + AE_BLOCK_COMPRESS_SKIP;
		dst_len = len;

		compression_failed = 0;
		AE_ERR(btree->compressor->compress(btree->compressor,
		    &session->iface,
		    src, src_len,
		    dst, dst_len,
		    &result_len, &compression_failed));
		result_len += AE_BLOCK_COMPRESS_SKIP;
		...
		if (compression_failed ||
			...
			ip = buf;
			AE_STAT_FAST_DATA_INCR(session, compress_write_fail);
		} else {
			compressed = true;
			AE_STAT_FAST_DATA_INCR(session, compress_write);

			/*
			 * Copy in the skipped header bytes, set the final data
			 * size.
			 */
			memcpy(ctmp->mem, buf->mem, AE_BLOCK_COMPRESS_SKIP);
	...
	if ((kencryptor = btree->kencryptor) != NULL) {
		 ...
		 */
		__ae_encrypt_size(session, kencryptor, ip->size, &size);

		AE_ERR(bm->write_size(bm, session, &size));
		AE_ERR(__ae_scr_alloc(session, size, &etmp));
		AE_ERR(__ae_encrypt(session,
		    kencryptor, AE_BLOCK_ENCRYPT_SKIP, ip, etmp));
	...
	if (compressed)
		F_SET(dsk, AE_PAGE_COMPRESSED);
	if (encrypted)
		F_SET(dsk, AE_PAGE_ENCRYPTED);

	/*
	 * We increment the block's write generation so it's easy to identify
	 * newer versions of blocks during salvage.  (It's common in ArchEngine,
	...
	AE_STAT_FAST_CONN_INCR(session, cache_write);
	AE_STAT_FAST_DATA_INCR(session, cache_write);
	AE_STAT_FAST_CONN_INCRV(session, cache_bytes_write, dsk->mem_size);
	AE_STAT_FAST_DATA_INCRV(session, cache_bytes_write, dsk->mem_size);

err:	__ae_scr_free(session, &ctmp);
	__ae_scr_free(session, &etmp);
	return (ret);
}
