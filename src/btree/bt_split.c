...
#define	AE_MEM_TRANSFER(from_decr, to_incr, len) do {			\
	size_t __len = (len);						\
	from_decr += __len;						\
	to_incr += __len;						\
} while (0)
 ...
 */
static uint64_t
__split_oldest_gen(AE_SESSION_IMPL *session)
{
...
static int
__split_stash_add(
    AE_SESSION_IMPL *session, uint64_t split_gen, void *p, size_t len)
{
	AE_CONNECTION_IMPL *conn;
	AE_SPLIT_STASH *stash;

	AE_ASSERT(session, p != NULL);

	conn = S2C(session);

	/* Grow the list as necessary. */
	AE_RET(__ae_realloc_def(session, &session->split_stash_alloc,
	...
	(void)__ae_atomic_add64(&conn->split_stashed_bytes, len);
	(void)__ae_atomic_add64(&conn->split_stashed_objects, 1);
...
/*
 * __ae_split_stash_discard --
 *	Discard any memory from a session's split stash that we can.
 */
void
__ae_split_stash_discard(AE_SESSION_IMPL *session)
{
	AE_CONNECTION_IMPL *conn;
	AE_SPLIT_STASH *stash;
	uint64_t oldest;
	...
	for (i = 0, stash = session->split_stash;
		 ...
		 */
		(void)__ae_atomic_sub64(&conn->split_stashed_bytes, stash->len);
		(void)__ae_atomic_sub64(&conn->split_stashed_objects, 1);
		__ae_overwrite_and_free_len(session, stash->p, stash->len);
	}
...
/*
 * __ae_split_stash_discard_all --
 *	Discard all memory from a session's split stash.
 */
void
__ae_split_stash_discard_all(
    AE_SESSION_IMPL *session_safe, AE_SESSION_IMPL *session)
{
	AE_SPLIT_STASH *stash;
	size_t i;

	/*
	 * This function is called during AE_CONNECTION.close to discard any
	 * memory that remains.  For that reason, we take two AE_SESSION_IMPL
	 * arguments: session_safe is still linked to the AE_CONNECTION and
	 * can be safely used for calls to other ArchEngine functions, while
	 * session is the AE_SESSION_IMPL we're cleaning up.
	 */
	for (i = 0, stash = session->split_stash;
	    i < session->split_stash_cnt;
	    ++i, ++stash)
		if (stash->p != NULL)
			__ae_free(session_safe, stash->p);

	__ae_free(session_safe, session->split_stash);
 ...
 */
static void
__split_verify_intl_key_order(AE_SESSION_IMPL *session, AE_PAGE *page)
{
	AE_BTREE *btree;
	AE_ITEM *next, _next, *last, _last, *tmp;
	AE_REF *ref;
	uint64_t recno;
	int cmp;
	bool first;
	...
	case AE_PAGE_COL_INT:
		...
		AE_INTL_FOREACH_BEGIN(session, page, ref) {
			AE_ASSERT(session, ref->key.recno > recno);
			recno = ref->key.recno;
		} AE_INTL_FOREACH_END;
		break;
	case AE_PAGE_ROW_INT:
		...
		first = true;
		AE_INTL_FOREACH_BEGIN(session, page, ref) {
			__ae_ref_key(page, ref, &next->data, &next->size);
			if (last->size == 0) {
				if (first)
					first = false;
				else {
					AE_ASSERT(session, __ae_compare(
					    session, btree->collator, last,
					    next, &cmp) == 0);
					AE_ASSERT(session, cmp < 0);
				}
			}
			tmp = last;
			last = next;
 ...
 */
static int
__split_ovfl_key_cleanup(AE_SESSION_IMPL *session, AE_PAGE *page, AE_REF *ref)
{
	AE_CELL *cell;
	AE_CELL_UNPACK kpack;
	AE_IKEY *ikey;
	...
	cell = AE_PAGE_REF_OFFSET(page, cell_offset);
	__ae_cell_unpack(cell, &kpack);
	if (kpack.ovfl && kpack.raw != AE_CELL_KEY_OVFL_RM) {
		/*
		 ...
		 */
		AE_ASSERT(session,
		    S2BT(session)->checkpointing != AE_CKPT_RUNNING);

		AE_RET(__ae_ovfl_discard(session, cell));
	}
...
/*
 * __split_ref_move --
 *	Move a AE_REF from one page to another, including updating accounting
 * information.
 */
static int
__split_ref_move(AE_SESSION_IMPL *session, AE_PAGE *from_home,
    AE_REF **from_refp, size_t *decrp, AE_REF **to_refp, size_t *incrp)
{
	AE_ADDR *addr;
	...
	/*
	 * The from-home argument is the page into which the "from" AE_REF may
	 * point, for example, if there's an on-page key the "from" AE_REF
	 * references, it will be on the page "from-home".
	 *
	 * Instantiate row-store keys, and column- and row-store addresses in
	 * the AE_REF structures referenced by a page that's being split. The
	 * AE_REF structures aren't moving, but the index references are moving
	 ...
	 * split.
	 *
	 * No locking is required to update the AE_REF structure because we're
	 ...
	 * problem.
	 */
	if (from_home->type == AE_PAGE_ROW_INT) {
		/*
		 ...
		 */
		if ((ikey = __ae_ref_key_instantiated(ref)) == NULL) {
			__ae_ref_key(from_home, ref, &key, &size);
			AE_RET(__ae_row_ikey(session, 0, key, size, ref));
			ikey = ref->key.ikey;
		} else {
			AE_RET(
			    __split_ovfl_key_cleanup(session, from_home, ref));
			*decrp += sizeof(AE_IKEY) + ikey->size;
		}
		*incrp += sizeof(AE_IKEY) + ikey->size;
	}
	 ...
	 */
	addr = ref->addr;
	if (addr != NULL && !__ae_off_page(from_home, addr)) {
		__ae_cell_unpack((AE_CELL *)ref->addr, &unpack);
		...
		case AE_CELL_ADDR_INT:
			addr->type = AE_ADDR_INT;
			break;
		case AE_CELL_ADDR_LEAF:
			addr->type = AE_ADDR_LEAF;
			break;
		case AE_CELL_ADDR_LEAF_NO:
			addr->type = AE_ADDR_LEAF_NO;
			break;
	...
	/* And finally, copy the AE_REF pointer itself. */
	*to_refp = ref;
	AE_MEM_TRANSFER(*decrp, *incrp, sizeof(AE_REF));
 ...
 */
static void
__split_child_block_evict_and_split(AE_PAGE *child)
{
	/*
	 * Once the split is live, newly created internal pages might be evicted
	 * and their AE_REF structures freed. If that happens before all threads
	 * exit the index of the page which previously "owned" the AE_REF, a
	 * thread might see a freed AE_REF. To ensure that doesn't happen, the
	 ...
	 * we fix up its AE_REF (in other words, a AE_REF we move is then moved
	 * again, before we reset the underlying page's parent reference), it's
	 * OK because the test we use to find a AE_REF and AE_PAGE that require
	 * fixing up is only that the AE_REF points to the wrong parent, not it
	 * points to a specific wrong parent. The problem is our fix up of the
	 * AE_REFs in the created page could race with the subsequent fix of the
	 * same AE_REFs (in a different created page), we'd have to acquire some
	 ...
	 * have been fixed up.
	 */
	F_SET_ATOMIC(child, AE_PAGE_SPLIT_BLOCK);
}

/*
 * __split_ref_move_final --
 *	Finalize the moved AE_REF structures after the split succeeds.
 */
static int
__split_ref_move_final(
    AE_SESSION_IMPL *session, AE_REF **refp, uint32_t entries)
{
	AE_DECL_RET;
	AE_PAGE *child;
	AE_REF *ref, *child_ref;
	uint32_t i;

	/*
	 * The AE_REF structures moved to newly allocated child pages reference
	...
	for (i = 0; i < entries; ++i, ++refp) {
		 ...
		 * thread might be attempting to evict the page (the AE_REF may
		 * be AE_REF_LOCKED), or it may be a disk based page (the AE_REF
		 * may be AE_REF_READING), or it may be in some other state.
		 ...
		 */
		if ((ret = __ae_page_in(session,
		    ref, AE_READ_CACHE | AE_READ_NO_EVICT)) == AE_NOTFOUND)
			continue;
		AE_ERR(ret);
		...
		AE_INTL_FOREACH_BEGIN(session, child, child_ref) {
			/*
		...
		F_CLR_ATOMIC(child, AE_PAGE_SPLIT_BLOCK);

		AE_ERR(__ae_hazard_clear(session, child));
	}
...
err:	/* Something really bad just happened. */
	AE_PANIC_RET(session, ret, "fatal error resolving a split");
}
 ...
 */
static int
__split_root(AE_SESSION_IMPL *session, AE_PAGE *root)
{
	AE_BTREE *btree;
	AE_DECL_RET;
	AE_PAGE *child;
	AE_PAGE_INDEX *alloc_index, *child_pindex, *pindex;
	AE_REF **alloc_refp;
	AE_REF **child_refp, *ref, **root_refp;
	...
	AE_STAT_FAST_CONN_INCR(session, cache_eviction_deepen);
	AE_STAT_FAST_DATA_INCR(session, cache_eviction_deepen);
	AE_STAT_FAST_CONN_INCR(session, cache_eviction_split_internal);
	AE_STAT_FAST_DATA_INCR(session, cache_eviction_split_internal);
	...
	AE_RET(__ae_page_modify_init(session, root));
	...
	/*
	 * Allocate a new AE_PAGE_INDEX and set of AE_REF objects to be inserted
	 * into the root page, replacing the root's page-index.
	 */
	size = sizeof(AE_PAGE_INDEX) + children * sizeof(AE_REF *);
	AE_ERR(__ae_calloc(session, 1, size, &alloc_index));
	root_incr += size;
	alloc_index->index = (AE_REF **)(alloc_index + 1);
	alloc_index->entries = children;
	alloc_refp = alloc_index->index;
	for (i = 0; i < children; alloc_refp++, ++i)
		AE_ERR(__ae_calloc_one(session, alloc_refp));
	root_incr += children * sizeof(AE_REF);
	...
	for (root_refp = pindex->index,
		...
		ref->page = child;
		ref->addr = NULL;
		if (root->type == AE_PAGE_ROW_INT) {
			__ae_ref_key(root, *root_refp, &p, &size);
			AE_ERR(__ae_row_ikey(session, 0, p, size, ref));
			root_incr += sizeof(AE_IKEY) + size;
		} else
			ref->key.recno = (*root_refp)->key.recno;
		ref->state = AE_REF_MEM;

		/* Initialize the child page. */
		if (root->type == AE_PAGE_COL_INT)
			child->pg_intl_recno = (*root_refp)->key.recno;
		child->pg_intl_parent_ref = ref;

		/* Mark it dirty. */
		AE_ERR(__ae_page_modify_init(session, child));
		__ae_page_modify_set(session, child);
		 ...
		 * structures as the root.  (We cannot move AE_REF structures,
		 * threads may be underneath us right now changing the structure
		 * state.)  However, if the AE_REF structures reference on-page
		 * information, we have to fix that, because the disk image for
		 * the page that has an page index entry for the AE_REF is about
		 * to change.
		 */
		child_pindex = AE_INTL_INDEX_GET_SAFE(child);
		child_incr = 0;
		for (child_refp = child_pindex->index,
		    j = 0; j < slots; ++child_refp, ++root_refp, ++j)
			AE_ERR(__split_ref_move(session, root,
			    root_refp, &root_decr, child_refp, &child_incr));

		__ae_cache_page_inmem_incr(session, child, child_incr);
	}
	AE_ASSERT(session,
	 ...
	 * AE_REFs and a new set of child pages -- if an error occurred, the
	 ...
	 */
	AE_ASSERT(session, AE_INTL_INDEX_GET_SAFE(root) == pindex);
	AE_INTL_INDEX_SET(root, alloc_index);
	complete = true;
...
#endif
	/* Fix up the moved AE_REF structures. */
	AE_ERR(__split_ref_move_final(
	 ...
	 */
	split_gen = __ae_atomic_addv64(&S2C(session)->split_gen, 1);
	size = sizeof(AE_PAGE_INDEX) + pindex->entries * sizeof(AE_REF *);
	AE_TRET(__split_safe_free(session, split_gen, false, pindex, size));
	root_decr += size;

	/* Adjust the root's memory footprint and mark it dirty. */
	__ae_cache_page_inmem_incr(session, root, root_incr);
	__ae_cache_page_inmem_decr(session, root, root_decr);
	__ae_page_modify_set(session, root);
...
err:	/*
	 ...
	 */
	if (!complete)
		__ae_free_ref_index(session, root, alloc_index, true);

	if (ret != 0 && ret != AE_PANIC)
		__ae_err(session, ret,
		    "ignoring not-fatal error during root page split to "
		    "deepen the tree");
	return (ret == AE_PANIC || !complete ? ret : 0);
}
 ...
 */
static int
__split_parent(AE_SESSION_IMPL *session, AE_REF *ref, AE_REF **ref_new,
    uint32_t new_entries, size_t parent_incr, bool exclusive, bool discard)
{
	...
	AE_IKEY *ikey;
	AE_PAGE *parent;
	AE_PAGE_INDEX *alloc_index, *pindex;
	AE_REF **alloc_refp, *next_ref;
	 ...
	 */
	pindex = AE_INTL_INDEX_GET_SAFE(parent);
	 ...
	 */
	AE_RET(__ae_scr_alloc(session, 10 * sizeof(uint32_t), &scr));
	for (deleted_entries = 0, i = 0; i < parent_entries; ++i) {
		next_ref = pindex->index[i];
		AE_ASSERT(session, next_ref->state != AE_REF_SPLIT);
		if ((discard && next_ref == ref) ||
		    (next_ref->state == AE_REF_DELETED &&
		    __ae_delete_page_skip(session, next_ref, true) &&
		    __ae_atomic_casv32(
		    &next_ref->state, AE_REF_DELETED, AE_REF_SPLIT))) {
			AE_ERR(__ae_buf_grow(session, scr,
	 ...
	 * pages, less any AE_REFs we're removing (deleted entries plus the
	 ...
	 */
	size = sizeof(AE_PAGE_INDEX) + result_entries * sizeof(AE_REF *);
	AE_ERR(__ae_calloc(session, 1, size, &alloc_index));
	parent_incr += size;
	alloc_index->index = (AE_REF **)(alloc_index + 1);
	...
	for (alloc_refp = alloc_index->index, i = 0; i < parent_entries; ++i) {
		...
		else if (next_ref->state != AE_REF_SPLIT)
	 ...
	 */
	AE_ASSERT(session, AE_INTL_INDEX_GET_SAFE(parent) == pindex);
	AE_INTL_INDEX_SET(parent, alloc_index);
	alloc_index = NULL;
	...
	/*
	 * If discarding the page's original AE_REF field, reset it to split.
	 * Threads cursoring through the tree were blocked because that AE_REF
	...
	if (discard) {
		/*
		 * Page-delete information is only read when the AE_REF state is
		 * AE_REF_DELETED.  The page-delete memory wasn't added to the
		...
		AE_PUBLISH(ref->state, AE_REF_SPLIT);
	}
	...
	AE_ERR(__ae_verbose(session, AE_VERB_SPLIT,
	    ...
	    "unknown page type" : __ae_page_type_string(ref->page->type),
	...
	/*
	 * The new page index is in place, free the AE_REF we were splitting and
	 * any deleted AE_REFs we found, modulo the usual safe free semantics.
	 *
	...
	for (i = 0, deleted_refs = scr->mem; i < deleted_entries; ++i) {
		...
		/*
		 * We set the AE_REF to split, discard it, freeing any resources
		 * it holds.
		 *
		 ...
		 * blocks.  We are exchanging the AE_REF that referenced it for
		 * the split page AE_REFs and their keys, and there's no longer
		 * any reference to it.  Done after completing the split (if we
		 * failed, we'd leak the underlying blocks, but the parent page
		 * would be unaffected).
		 */
		if (parent->type == AE_PAGE_ROW_INT) {
			AE_TRET(__split_ovfl_key_cleanup(
			    session, parent, next_ref));
			ikey = __ae_ref_key_instantiated(next_ref);
			if (ikey != NULL) {
				size = sizeof(AE_IKEY) + ikey->size;
				AE_TRET(__split_safe_free(
		 ...
		 */
		AE_ASSERT(session, next_ref->page_del == NULL);

		AE_TRET(__ae_ref_block_free(session, next_ref));
		AE_TRET(__split_safe_free(
		    session, split_gen, exclusive, next_ref, sizeof(AE_REF)));
		parent_decr += sizeof(AE_REF);
	}
	 ...
	 */
	size = sizeof(AE_PAGE_INDEX) + pindex->entries * sizeof(AE_REF *);
	AE_TRET(__split_safe_free(session, split_gen, exclusive, pindex, size));
	parent_decr += size;

	/* Adjust the parent's memory footprint and mark it dirty. */
	__ae_cache_page_inmem_incr(session, parent, parent_incr);
	__ae_cache_page_inmem_decr(session, parent, parent_decr);
	__ae_page_modify_set(session, parent);

err:	__ae_scr_free(session, &scr);
	/*
	 * A note on error handling: if we completed the split, return success,
	 * nothing really bad can have happened, and our caller has to proceed
	 * with the split.
	...
	if (!complete) {
		for (i = 0; i < parent_entries; ++i) {
			next_ref = pindex->index[i];
			if (next_ref->state == AE_REF_SPLIT)
				next_ref->state = AE_REF_DELETED;
		}

		__ae_free_ref_index(session, NULL, alloc_index, false);

		/*
		 * The split couldn't proceed because the parent would be empty,
		 * return EBUSY so our caller knows to unlock the AE_REF that's
 ...
 */
static int
__split_internal(AE_SESSION_IMPL *session, AE_PAGE *parent, AE_PAGE *page)
{
	AE_BTREE *btree;
	AE_DECL_RET;
	AE_PAGE *child;
	AE_PAGE_INDEX *alloc_index, *child_pindex, *pindex, *replace_index;
	AE_REF **alloc_refp;
	AE_REF **child_refp, *page_ref, **page_refp, *ref;
	 ...
	 * moving all except the first chunk of the page's AE_REF objects to new
	 * pages.
	 *
	 * Create and initialize a replacement AE_PAGE_INDEX for the original
	 * page.
	 */
	size = sizeof(AE_PAGE_INDEX) + chunk * sizeof(AE_REF *);
	AE_ERR(__ae_calloc(session, 1, size, &replace_index));
	page_incr += size;
	replace_index->index = (AE_REF **)(replace_index + 1);
	 ...
	 *
	 * The first slot of the new AE_PAGE_INDEX is the original page AE_REF.
	 * The remainder of the slots are allocated AE_REFs.
	 */
	size = sizeof(AE_PAGE_INDEX) + children * sizeof(AE_REF *);
	AE_ERR(__ae_calloc(session, 1, size, &alloc_index));
	parent_incr += size;
	...
	*alloc_refp++ = page_ref;
	for (i = 1; i < children; ++alloc_refp, ++i)
		AE_ERR(__ae_calloc_one(session, alloc_refp));
	parent_incr += children * sizeof(AE_REF);

	/* Allocate child pages, and connect them into the new page index. */
	AE_ASSERT(session, page_refp == pindex->index + chunk);
	for (alloc_refp = alloc_index->index + 1, i = 1; i < children; ++i) {
		...
		if (page->type == AE_PAGE_ROW_INT) {
			__ae_ref_key(page, *page_refp, &p, &size);
			AE_ERR(__ae_row_ikey(session, 0, p, size, ref));
			parent_incr += sizeof(AE_IKEY) + size;
		} else
		...
		/*
		 * The newly allocated child's page index references the same
		 * structures as the parent. (We cannot move AE_REF structures,
		 * threads may be underneath us right now changing the structure
		 * state.)  However, if the AE_REF structures reference on-page
		 * information, we have to fix that, because the disk image for
		 * the page that has an page index entry for the AE_REF is about
	...
	AE_ASSERT(session, AE_INTL_INDEX_GET_SAFE(page) == pindex);
	AE_INTL_INDEX_SET(page, replace_index);
	 ...
	 * the array of AE_REF structures, which has now been split into the
	 * parent page.
	 */
	__ae_free(session, alloc_index);
	...
	__ae_cache_page_inmem_incr(session, page, page_incr);
	__ae_cache_page_inmem_decr(session, page, page_decr);
...
err:	/*
	 ...
	 */
	if (!complete) {
		__ae_free_ref_index(session, page, alloc_index, true);
		__ae_free_ref_index(session, page, replace_index, false);
	}
 ...
 */
static int
__split_internal_lock(AE_SESSION_IMPL *session, AE_REF *ref, bool trylock,
    AE_PAGE **parentp, bool *hazardp)
{
	AE_DECL_RET;
	AE_PAGE *parent;
	AE_REF *parent_ref;
	...
	*parentp = NULL;
	 ...
	 */
	if (!trylock && S2BT(session)->checkpointing != AE_CKPT_OFF)
		return (EBUSY);
	...
	for (;;) {
		...
		if (F_ISSET_ATOMIC(parent, AE_PAGE_SPLIT_BLOCK))
			return (EBUSY);

		if (trylock)
			AE_RET(__ae_fair_trylock(session, &parent->page_lock));
		else
			AE_RET(__ae_fair_lock(session, &parent->page_lock));
		if (parent == ref->home)
			break;
		AE_RET(__ae_fair_unlock(session, &parent->page_lock));
	}
	 ...
	 */
	if (!__ae_ref_is_root(parent_ref = parent->pg_intl_parent_ref)) {
		AE_ERR(__ae_page_in(session, parent_ref, AE_READ_NO_EVICT));
		*hazardp = true;
	}
...
err:	AE_TRET(__ae_fair_unlock(session, &parent->page_lock));
	return (ret);
}
 ...
 */
static int
__split_internal_unlock(AE_SESSION_IMPL *session, AE_PAGE *parent, bool hazard)
{
	AE_DECL_RET;

	if (hazard)
		ret = __ae_hazard_clear(session, parent);

	AE_TRET(__ae_fair_unlock(session, &parent->page_lock));
	return (ret);
}
 ...
 */
static bool
__split_internal_should_split(AE_SESSION_IMPL *session, AE_REF *ref)
{
	AE_BTREE *btree;
	AE_PAGE *page;
 ...
 */
static int
__split_parent_climb(AE_SESSION_IMPL *session, AE_PAGE *page, bool page_hazard)
{
	AE_DECL_RET;
	AE_PAGE *parent;
	AE_REF *ref;
	...
	for (;;) {
		 ...
		 */
		AE_ERR(__split_internal_lock(
		    session, ref, true, &parent, &parent_hazard));
		ret = __split_internal(session, parent, page);
		AE_TRET(__split_internal_unlock(session, page, page_hazard));
...
static int
__split_multi_inmem(
    AE_SESSION_IMPL *session, AE_PAGE *orig, AE_REF *ref, AE_MULTI *multi)
{
	AE_CURSOR_BTREE cbt;
	AE_DECL_ITEM(key);
	AE_DECL_RET;
	 ...
	 *
	 * Clear the disk image and link the page into the passed-in AE_REF to
	 ...
	 * allocated page on error, when discarding the allocated AE_REF.
	 */
	AE_RET(__ae_page_inmem(session, ref,
	    multi->disk_image, ((AE_PAGE_HEADER *)multi->disk_image)->mem_size,
	    AE_PAGE_DISK_ALLOC, &page));
	multi->disk_image = NULL;

	if (orig->type == AE_PAGE_ROW_LEAF)
	...
	for (i = 0, supd = multi->supd; i < multi->supd_entries; ++i, ++supd)
		...
		case AE_PAGE_COL_VAR:
			/* Build a key. */
			upd = supd->ins->upd;
			recno = AE_INSERT_RECNO(supd->ins);

			/* Search the page. */
			AE_ERR(__ae_col_search(session, recno, ref, &cbt));

			/* Apply the modification. */
			AE_ERR(__ae_col_modify(
			    session, &cbt, recno, NULL, upd, false));
			break;
		case AE_PAGE_ROW_LEAF:
			/* Build a key. */
			if (supd->ins == NULL) {
				slot = AE_ROW_SLOT(orig, supd->rip);
			...
			/* Search the page. */
			AE_ERR(__ae_row_search(session, key, ref, &cbt, true));

			/* Apply the modification. */
			AE_ERR(__ae_row_modify(
	 ...
	 */
	if (page->modify != NULL)
		page->modify->first_dirty_txn = AE_TXN_FIRST;
 ...
 */
static void
__split_multi_inmem_final(AE_PAGE *orig, AE_MULTI *multi)
{
	AE_SAVE_UPD *supd;
	uint32_t i, slot;
 ...
 */
static void
__split_multi_inmem_fail(AE_SESSION_IMPL *session, AE_PAGE *orig, AE_REF *ref)
{
	/*
	 ...
	 */
	if (ref->page != NULL) {
		F_SET_ATOMIC(ref->page, AE_PAGE_UPDATE_IGNORE);
		__ae_free_ref(session, ref, orig->type, true);
	}
}

/*
 * __ae_multi_to_ref --
 *	Move a multi-block list into an array of AE_REF structures.
 */
int
__ae_multi_to_ref(AE_SESSION_IMPL *session,
    AE_PAGE *page, AE_MULTI *multi, AE_REF **refp, size_t *incrp)
{
	AE_ADDR *addr;
	AE_IKEY *ikey;
	AE_REF *ref;
	...
	/* Allocate an underlying AE_REF. */
	AE_RET(__ae_calloc_one(session, refp));
	ref = *refp;
	incr += sizeof(AE_REF);
	...
	if (multi->disk_image == NULL) {
		...
		AE_RET(__ae_strndup(session,
		    multi->addr.addr, addr->size, &addr->addr));
	} else
		AE_RET(__split_multi_inmem(session, page, ref, multi));

	switch (page->type) {
	case AE_PAGE_ROW_INT:
	case AE_PAGE_ROW_LEAF:
		ikey = multi->key.ikey;
		AE_RET(__ae_row_ikey(
		    session, 0, AE_IKEY_DATA(ikey), ikey->size, ref));
		incr += sizeof(AE_IKEY) + ikey->size;
		break;
	default:
	...
	ref->state = addr != NULL ? AE_REF_DISK : AE_REF_MEM;
 ...
 */
static int
__split_insert(AE_SESSION_IMPL *session, AE_REF *ref)
{
	AE_DECL_RET;
	AE_DECL_ITEM(key);
	AE_INSERT *ins, **insp, *moved_ins, *prev_ins;
	AE_INSERT_HEAD *ins_head;
	AE_PAGE *page, *right;
	AE_REF *child, *split_ref[2] = { NULL, NULL };
	size_t page_decr, parent_incr, right_incr;
	int i;

	AE_STAT_FAST_CONN_INCR(session, cache_inmem_split);
	AE_STAT_FAST_DATA_INCR(session, cache_inmem_split);
	 ...
	 */
	AE_ASSERT(session, __ae_leaf_page_can_split(session, page));
	AE_ASSERT(session, __ae_page_is_modified(page));
	F_SET_ATOMIC(page, AE_PAGE_SPLIT_INSERT);

	/* Find the last item on the page. */
	ins_head = page->pg_row_entries == 0 ?
	    AE_ROW_INSERT_SMALLEST(page) :
	    AE_ROW_INSERT_SLOT(page, page->pg_row_entries - 1);
	moved_ins = AE_SKIP_LAST(ins_head);

	/*
	 * The first page in the split is the current page, but we still have
	 * to create a replacement AE_REF, the original AE_REF will be set to
	 * split status and eventually freed.
	 *
	 * The new AE_REF is not quite identical: we have to instantiate a key,
	 ...
	 */
	AE_ERR(__ae_calloc_one(session, &split_ref[0]));
	child = split_ref[0];
	child->page = ref->page;
	child->home = ref->home;
	child->pindex_hint = ref->pindex_hint;
	child->state = AE_REF_MEM;
	child->addr = ref->addr;

	/*
	 * The address has moved to the replacement AE_REF.  Make sure it isn't
	...
	AE_ERR(__ae_row_ikey(session, 0, key->data, key->size, child));
	parent_incr += sizeof(AE_REF) + sizeof(AE_IKEY) + key->size;
	__ae_scr_free(session, &key);

	/*
	 * The second page in the split is a new AE_REF/page pair.
	 */
	AE_ERR(__ae_page_alloc(session, AE_PAGE_ROW_LEAF, 0, 0, false, &right));
	AE_ERR(__ae_calloc_one(session, &right->pg_row_ins));
	AE_ERR(__ae_calloc_one(session, &right->pg_row_ins[0]));
	right_incr += sizeof(AE_INSERT_HEAD);
	right_incr += sizeof(AE_INSERT_HEAD *);

	AE_ERR(__ae_calloc_one(session, &split_ref[1]));
	child = split_ref[1];
	child->page = right;
	child->state = AE_REF_MEM;
	AE_ERR(__ae_row_ikey(session, 0,
	    AE_INSERT_KEY(moved_ins), AE_INSERT_KEY_SIZE(moved_ins),
	    child));
	parent_incr +=
	    sizeof(AE_REF) + sizeof(AE_IKEY) + AE_INSERT_KEY_SIZE(moved_ins);

	/* The new page is dirty by definition. */
	AE_ERR(__ae_page_modify_init(session, right));
	__ae_page_modify_set(session, right);
	 ...
	 * in a checkpoint.
	 */
	right->modify->first_dirty_txn = AE_TXN_FIRST;
	 ...
	 */
	for (i = 0; i < AE_SKIP_MAXDEPTH && ins_head->tail[i] == moved_ins; ++i)
		;
	AE_MEM_TRANSFER(page_decr, right_incr, sizeof(AE_INSERT) +
	    (size_t)i * sizeof(AE_INSERT *) + AE_INSERT_KEY_SIZE(moved_ins));
	AE_MEM_TRANSFER(
	    page_decr, right_incr, __ae_update_list_memsize(moved_ins->upd));
	...
	for (i = AE_SKIP_MAXDEPTH - 1, insp = &ins_head->head[i];
	    i >= 0;
	    i--, insp--) {
		...
		if (ins == moved_ins) {
			...
			AE_ASSERT(session, ins_head->head[i] != moved_ins);
			AE_ASSERT(session, prev_ins->next[i] == moved_ins);
			*insp = NULL;
	...
	for (i = AE_SKIP_MAXDEPTH - 1, insp = &ins_head->head[i];
		...
		for (ins = *insp; ins != NULL; ins = ins->next[i])
			AE_ASSERT(session, ins != moved_ins);
#endif
	 ...
	 */
	__ae_cache_page_inmem_decr(session, page, page_decr);
	__ae_cache_page_inmem_incr(session, right, right_incr);
	...
	if ((ret = __split_parent(
		 ...
		 */
		__ae_page_modify_clear(session, right);
...
err:	if (split_ref[0] != NULL) {
		/*
		 * The address was moved to the replacement AE_REF, restore it.
		 */
		ref->addr = split_ref[0]->addr;

		__ae_free(session, split_ref[0]->key.ikey);
		__ae_free(session, split_ref[0]);
	}
	if (split_ref[1] != NULL) {
		__ae_free(session, split_ref[1]->key.ikey);
		__ae_free(session, split_ref[1]);
	}
	if (right != NULL)
		__ae_page_out(session, &right);
	__ae_scr_free(session, &key);
	return (ret);
}
 ...
 */
int
__ae_split_insert(AE_SESSION_IMPL *session, AE_REF *ref)
{
	AE_DECL_RET;
	AE_PAGE *parent;
	bool hazard;
	...
	AE_RET(__ae_verbose(
	    session, AE_VERB_SPLIT, "%p: split-insert", ref->page));

	AE_RET(__split_internal_lock(session, ref, true, &parent, &hazard));
	if ((ret = __split_insert(session, ref)) != 0) {
		AE_TRET(__split_internal_unlock(session, parent, hazard));
		return (ret);
	}
 ...
 */
static int
__split_multi(AE_SESSION_IMPL *session, AE_REF *ref, bool closing)
{
	AE_DECL_RET;
	AE_PAGE *page;
	AE_PAGE_MODIFY *mod;
	...
	AE_STAT_FAST_CONN_INCR(session, cache_eviction_split_leaf);
	AE_STAT_FAST_DATA_INCR(session, cache_eviction_split_leaf);
	 ...
	 */
	AE_RET(__ae_calloc_def(session, new_entries, &ref_new));
 ...
 */
int
__ae_split_multi(AE_SESSION_IMPL *session, AE_REF *ref, int closing)
{
	AE_DECL_RET;
	AE_PAGE *parent;
	bool hazard;
	...
	AE_RET(__ae_verbose(
	    session, AE_VERB_SPLIT, "%p: split-multi", ref->page));

	AE_RET(__split_internal_lock(session, ref, false, &parent, &hazard));
 ...
 */
int
__ae_split_reverse(AE_SESSION_IMPL *session, AE_REF *ref)
{
	AE_DECL_RET;
	AE_PAGE *parent;
	bool hazard;
	...
	AE_RET(__ae_verbose(
	    session, AE_VERB_SPLIT, "%p: reverse-split", ref->page));
 ...
 */
int
__ae_split_rewrite(AE_SESSION_IMPL *session, AE_REF *ref)
{
	AE_DECL_RET;
	AE_PAGE *page;
	AE_PAGE_MODIFY *mod;
	...
	AE_RET(__ae_verbose(
	    session, AE_VERB_SPLIT, "%p: split-rewrite", ref->page));
	...
	AE_ERR(__split_multi_inmem(session, page, &new, &mod->mod_multi[0]));
