...
int
__ae_block_map(
    AE_SESSION_IMPL *session, AE_BLOCK *block, void *mapp, size_t *maplenp,
    void **mappingcookie)
{
	*(void **)mapp = NULL;
	*maplenp = 0;
	 ...
	 */
	(void)__ae_mmap(session, block->fh, mapp, maplenp, mappingcookie);
...
int
__ae_block_unmap(
    AE_SESSION_IMPL *session, AE_BLOCK *block, void *map, size_t maplen,
    void **mappingcookie)
{
	/* Unmap the file from memory. */
	return (__ae_munmap(session, block->fh, map, maplen, mappingcookie));
}
