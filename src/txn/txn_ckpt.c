...
/*
 * __ae_checkpoint_name_ok --
 *	Complain if the checkpoint name isn't acceptable.
 */
int
__ae_checkpoint_name_ok(AE_SESSION_IMPL *session, const char *name, size_t len)
{
	/* Check for characters we don't want to see in a metadata file. */
	AE_RET(__ae_name_check(session, name, len));
	 ...
	 */
	if (len < strlen(AE_CHECKPOINT))
		return (0);
	if (!AE_PREFIX_MATCH(name, AE_CHECKPOINT))
		return (0);

	AE_RET_MSG(session, EINVAL,
	    "the checkpoint name \"%s\" is reserved", AE_CHECKPOINT);
}
 ...
 */
static int
__checkpoint_name_check(AE_SESSION_IMPL *session, const char *uri)
{
	AE_CURSOR *cursor;
	AE_DECL_RET;
	const char *fail;
 ...
 */
static int
__checkpoint_apply_all(AE_SESSION_IMPL *session, const char *cfg[],
	int (*op)(AE_SESSION_IMPL *, const char *[]), bool *fullp)
{
	...
	AE_RET(__ae_config_gets(session, cfg, "name", &cval));
	named = cval.len != 0;
	if (named)
		AE_RET(__ae_checkpoint_name_ok(session, cval.str, cval.len));

	/* Step through the targets and optionally operate on each one. */
	AE_ERR(__ae_config_gets(session, cfg, "target", &cval));
	AE_ERR(__ae_config_subinit(session, &targetconf, &cval));
	while ((ret = __ae_config_next(&targetconf, &k, &v)) == 0) {
		if (!target_list) {
		...
		if (named)
			AE_ERR(__checkpoint_name_check(session, k.str));

		if (op == NULL)
			continue;
		AE_ERR(__ae_buf_fmt(session, tmp, "%.*s", (int)k.len, k.str));
		if ((ret = __ae_schema_worker(
		    session, tmp->data, op, NULL, cfg, 0)) != 0)
			AE_ERR_MSG(session, ret, "%s", (const char *)tmp->data);
	}
	AE_ERR_NOTFOUND_OK(ret);

	if (!target_list && named)
		/* Some objects don't support named checkpoints. */
		AE_ERR(__checkpoint_name_check(session, NULL));
	...
	if (!target_list && op != NULL) {
		...
		ckpt_closed = named;
		if (!ckpt_closed) {
			AE_ERR(__ae_config_gets(session, cfg, "drop", &cval));
			ckpt_closed = cval.len != 0;
		}
		AE_ERR(ckpt_closed ?
		    __ae_meta_btree_apply(session, op, cfg) :
		    __ae_conn_btree_apply(session, false, NULL, op, cfg));
	}
 ...
 */
static int
__checkpoint_apply(AE_SESSION_IMPL *session, const char *cfg[],
	int (*op)(AE_SESSION_IMPL *, const char *[]))
{
	AE_DECL_RET;
	u_int i;
	...
	for (i = 0; i < session->ckpt_handle_next; ++i) {
		if (session->ckpt_handle[i].dhandle != NULL)
			...
			AE_WITH_HANDLE_LIST_LOCK(session,
			    ret = __ae_conn_btree_apply_single(session,
 ...
 */
static int
__checkpoint_data_source(AE_SESSION_IMPL *session, const char *cfg[])
{
	 ...
	 * targets (__ae_schema_worker is the underlying function that will do
	...
	TAILQ_FOREACH(ndsrc, &S2C(session)->dsrcqh, q) {
		...
		if (dsrc->checkpoint != NULL)
			AE_RET(dsrc->checkpoint(dsrc,
			    (AE_SESSION *)session, (AE_CONFIG_ARG *)cfg));
	}
	return (0);
}

/*
 * __ae_checkpoint_list --
 *	Get a list of handles to flush.
 */
int
__ae_checkpoint_list(AE_SESSION_IMPL *session, const char *cfg[])
{
	AE_DECL_RET;
	const char *name;
	...
	AE_ASSERT(session, AE_PREFIX_MATCH(session->dhandle->name, "file:"));

	/* Skip files that are never involved in a checkpoint. */
	if (F_ISSET(S2BT(session), AE_BTREE_NO_CHECKPOINT))
		return (0);

	/* Make sure there is space for the next entry. */
	AE_RET(__ae_realloc_def(session, &session->ckpt_handle_allocated,
	...
	if ((ret = __ae_session_get_btree(session, name, NULL, NULL, 0)) == 0)
		session->ckpt_handle[session->ckpt_handle_next++].dhandle =
		    session->dhandle;
	else if (ret == EBUSY)
		ret = __ae_strdup(session, name,
 ...
 */
static int
__checkpoint_write_leaves(AE_SESSION_IMPL *session, const char *cfg[])
{
	AE_UNUSED(cfg);

	return (__ae_cache_op(session, NULL, AE_SYNC_WRITE_LEAVES));
}
...
static void
__checkpoint_stats(
    AE_SESSION_IMPL *session, struct timespec *start, struct timespec *stop)
{
	 ...
	 */
	msec = AE_TIMEDIFF_MS(*stop, *start);
 ...
 */
static int
__checkpoint_verbose_track(AE_SESSION_IMPL *session,
    const char *msg, struct timespec *start)
{
	...
	AE_RET(__ae_epoch(session, &stop));
	...
	AE_RET(__ae_verbose(session,
	    AE_VERB_CHECKPOINT, "time: %" PRIu64 " us, gen: %" PRIu64
 ...
 */
static int
__txn_checkpoint(AE_SESSION_IMPL *session, const char *cfg[])
{
	...
	AE_TXN_ISOLATION saved_isolation;
	AE_TXN_STATE *txn_state;
	void *saved_meta_next;
	u_int i;
	bool full, idle, logging, tracking;
	const char *txn_cfg[] = { AE_CONFIG_BASE(session,
	    AE_SESSION_begin_transaction), "isolation=snapshot", NULL };
	 ...
	 */
	AE_RET(__checkpoint_apply_all(session, cfg, NULL, &full));

	/* Configure logging only if doing a full checkpoint. */
	logging = FLD_ISSET(conn->log_flags, AE_CONN_LOG_ENABLED);
	...
	AE_WITH_SCHEMA_LOCK(session,
		...
		AE_WITH_HANDLE_LIST_LOCK(session,
		    ...
		    session, cfg, __ae_checkpoint_list, NULL))));
	AE_ERR(ret);
	...
	AE_ERR(__checkpoint_data_source(session, cfg));

	AE_ERR(__ae_epoch(session, &verb_timer));
	AE_ERR(__checkpoint_verbose_track(session,
	    "starting write leaves", &verb_timer));

	/* Flush dirty leaf pages before we start the checkpoint. */
	session->isolation = txn->isolation = AE_ISO_READ_COMMITTED;
	AE_ERR(__checkpoint_apply(session, cfg, __checkpoint_write_leaves));
	 ...
	 */
	if (F_ISSET(conn, AE_CONN_CKPT_SYNC))
		AE_ERR(__checkpoint_apply(session, cfg, __ae_checkpoint_sync));

	/* Acquire the schema lock. */
	F_SET(session, AE_SESSION_LOCKED_SCHEMA);
	__ae_spin_lock(session, &conn->schema_lock);

	AE_ERR(__ae_meta_track_on(session));
	tracking = true;

	/* Tell logging that we are about to start a database checkpoint. */
	if (full && logging)
		AE_ERR(__ae_txn_checkpoint_log(
		    session, full, AE_TXN_LOG_CKPT_PREPARE, NULL));
	 ...
	 */
	AE_ASSERT(session,
	    AE_TXNID_LE(txn_global->oldest_id, txn_state->id) &&
	    AE_TXNID_LE(txn_global->oldest_id, txn_state->snap_min));
	 ...
	 * details).
	 */
	txn_state->id = txn_state->snap_min = AE_TXN_NONE;

	/* Tell logging that we have started a database checkpoint. */
	if (full && logging)
		AE_ERR(__ae_txn_checkpoint_log(
		    session, full, AE_TXN_LOG_CKPT_START, NULL));

	AE_ERR(__checkpoint_apply(session, cfg, __ae_checkpoint));
	 ...
	 */
	AE_ERR(__ae_txn_commit(session, NULL));
	 ...
	 */
	if (full || !logging) {
		session->isolation = txn->isolation = AE_ISO_READ_UNCOMMITTED;
		/* Disable metadata tracking during the metadata checkpoint. */
		saved_meta_next = session->meta_track_next;
		session->meta_track_next = NULL;
		AE_WITH_DHANDLE(session,
		    session->meta_dhandle, ret = __ae_checkpoint(session, cfg));
	...
	if (full) {
		AE_ERR(__ae_epoch(session, &stop));
...
err:	/*
	...
	session->isolation = txn->isolation = AE_ISO_READ_UNCOMMITTED;
	if (tracking)
		AE_TRET(__ae_meta_track_off(session, false, ret != 0));
	...
	if (full && logging) {
		if (ret == 0 &&
		    F_ISSET((AE_BTREE *)session->meta_dhandle->handle,
		    AE_BTREE_SKIP_CKPT))
			idle = true;
		AE_TRET(__ae_txn_checkpoint_log(session, full,
		    (ret == 0 && !idle) ?
		    AE_TXN_LOG_CKPT_STOP : AE_TXN_LOG_CKPT_CLEANUP, NULL));
	}

	for (i = 0; i < session->ckpt_handle_next; ++i) {
		if (session->ckpt_handle[i].dhandle == NULL) {
			__ae_free(session, session->ckpt_handle[i].name);
			continue;
		}
		AE_WITH_DHANDLE(session, session->ckpt_handle[i].dhandle,
		    AE_TRET(__ae_session_release_btree(session)));
	}

	__ae_free(session, session->ckpt_handle);
	...
	if (F_ISSET(session, AE_SESSION_LOCKED_SCHEMA)) {
		...
		__ae_spin_unlock(session, &conn->schema_lock);
	}
 ...
 */
int
__ae_txn_checkpoint(AE_SESSION_IMPL *session, const char *cfg[])
{
	AE_DECL_RET;
	 ...
	 * some implementation of AE_CURSOR::reset might need the schema lock.
	 */
	AE_RET(__ae_session_reset_cursors(session, false));
	 ...
	 */
	F_SET(session, AE_SESSION_CAN_WAIT | AE_SESSION_NO_EVICTION);
	 ...
	 */
	AE_STAT_FAST_CONN_SET(session, txn_checkpoint_running, 1);

	AE_WITH_CHECKPOINT_LOCK(session, ret = __txn_checkpoint(session, cfg));

	AE_STAT_FAST_CONN_SET(session, txn_checkpoint_running, 0);

	F_CLR(session, AE_SESSION_CAN_WAIT | AE_SESSION_NO_EVICTION);
 ...
 */
static void
__drop(AE_CKPT *ckptbase, const char *name, size_t len)
{
	AE_CKPT *ckpt;
	 ...
	 */
	if (strncmp(AE_CHECKPOINT, name, len) == 0) {
		AE_CKPT_FOREACH(ckptbase, ckpt)
			if (AE_PREFIX_MATCH(ckpt->name, AE_CHECKPOINT))
				F_SET(ckpt, AE_CKPT_DELETE);
	} else
		AE_CKPT_FOREACH(ckptbase, ckpt)
			if (AE_STRING_MATCH(ckpt->name, name, len))
				F_SET(ckpt, AE_CKPT_DELETE);
}
 ...
 */
static void
__drop_from(AE_CKPT *ckptbase, const char *name, size_t len)
{
	AE_CKPT *ckpt;
	bool matched;
	 ...
	 * of the checkpoints.
	 */
	if (AE_STRING_MATCH("all", name, len)) {
		AE_CKPT_FOREACH(ckptbase, ckpt)
			F_SET(ckpt, AE_CKPT_DELETE);
		return;
	}
	...
	matched = false;
	AE_CKPT_FOREACH(ckptbase, ckpt) {
		if (!matched && !AE_STRING_MATCH(ckpt->name, name, len))
			continue;
 ...
 */
static void
__drop_to(AE_CKPT *ckptbase, const char *name, size_t len)
{
	AE_CKPT *ckpt, *mark;
 ...
 */
static int
__checkpoint_worker(AE_SESSION_IMPL *session,
    const char *cfg[], bool is_checkpoint, bool need_tracking)
{
	 ...
	 */
	AE_ASSERT(session, !need_tracking ||
	    AE_IS_METADATA(dhandle) || AE_META_TRACKING(session));
	...
	AE_RET(__ae_meta_ckptlist_get(session, dhandle->name, &ckptbase));

	/* This may be a named checkpoint, check the configuration. */
	cval.len = 0;
	if (cfg != NULL)
		AE_ERR(__ae_config_gets(session, cfg, "name", &cval));
	if (cval.len == 0)
		name = AE_CHECKPOINT;
	else {
		AE_ERR(__ae_checkpoint_name_ok(session, cval.str, cval.len));
		AE_ERR(__ae_strndup(session, cval.str, cval.len, &name_alloc));
		name = name_alloc;
	}
	...
	if (cfg != NULL) {
		...
		AE_ERR(__ae_config_gets(session, cfg, "drop", &cval));
		if (cval.len != 0) {
			AE_ERR(__ae_config_subinit(session, &dropconf, &cval));
			while ((ret =
			    __ae_config_next(&dropconf, &k, &v)) == 0) {
				/* Disallow unsafe checkpoint names. */
				if (v.len == 0)
					AE_ERR(__ae_checkpoint_name_ok(
					    session, k.str, k.len));
				else
					AE_ERR(__ae_checkpoint_name_ok(
					    session, v.str, v.len));
				...
				else if (AE_STRING_MATCH("from", k.str, k.len))
					__drop_from(ckptbase, v.str, v.len);
				else if (AE_STRING_MATCH("to", k.str, k.len))
	 ...
	 */
	force = false;
	F_CLR(btree, AE_BTREE_SKIP_CKPT);
	if (!btree->modified && cfg != NULL) {
		ret = __ae_config_gets(session, cfg, "force", &cval);
	...
	if (!btree->modified && !force) {
		...
		if (ckpt > ckptbase &&
		    ...
		    (AE_PREFIX_MATCH(name, AE_CHECKPOINT) &&
		    AE_PREFIX_MATCH((ckpt - 1)->name, AE_CHECKPOINT))) &&
		    deleted < 2) {
nockpt:			F_SET(btree, AE_BTREE_SKIP_CKPT);
	...
	AE_CKPT_FOREACH(ckptbase, ckpt)
		;
	AE_ERR(__ae_strdup(session, name, &ckpt->name));
	F_SET(ckpt, AE_CKPT_ADD);

	/*
	 * We can't delete checkpoints if a backup cursor is open.  ArchEngine
	 ...
	 */
	AE_ERR(__ae_readlock(session, conn->hot_backup_lock));
	...
	if (AE_META_TRACKING(session))
		AE_CKPT_FOREACH(ckptbase, ckpt) {
			 ...
			 * ArchEngine checkpoints are uniquely named and it's
			 * OK to have multiple in the system: clear the delete
			 * flag for them, and otherwise fail.
			 */
			ret = __ae_session_lock_checkpoint(session, ckpt->name);
			if (ret == 0)
				continue;
			if (ret == EBUSY &&
			    AE_PREFIX_MATCH(ckpt->name, AE_CHECKPOINT)) {
				F_CLR(ckpt, AE_CKPT_DELETE);
				continue;
			}
	 ...
	 */
	if (is_checkpoint)
		switch (F_MASK(btree, AE_BTREE_SPECIAL_FLAGS)) {
		case 0:
			break;
		case AE_BTREE_BULK:
			/*
			 ...
			 */
			AE_CKPT_FOREACH(ckptbase, ckpt)
				if (!F_ISSET(ckpt, AE_CKPT_ADD | AE_CKPT_FAKE))
					AE_ERR_MSG(session, ret,
					    "block-manager checkpoint found "
					    "for a bulk-loaded file");
			fake_ckpt = true;
			goto fake;
		case AE_BTREE_SALVAGE:
		case AE_BTREE_UPGRADE:
		case AE_BTREE_VERIFY:
			AE_ERR_MSG(session, EINVAL,
	 ...
	 */
	AE_ERR(__ae_page_modify_init(session, btree->root.page));
	__ae_page_modify_set(session, btree->root.page);
	...
	if (FLD_ISSET(conn->log_flags, AE_CONN_LOG_ENABLED))
		AE_ERR(__ae_txn_checkpoint_log(
		    session, false, AE_TXN_LOG_CKPT_START, &ckptlsn));

	/* Flush the file from the cache, creating the checkpoint. */
	if (is_checkpoint)
		AE_ERR(__ae_cache_op(session, ckptbase, AE_SYNC_CHECKPOINT));
	else
		AE_ERR(__ae_cache_op(session, ckptbase, AE_SYNC_CLOSE));
...
fake:	/*
	 ...
	 */
	if (fake_ckpt && FLD_ISSET(conn->log_flags, AE_CONN_LOG_ENABLED))
		AE_INIT_LSN(&ckptlsn);

	/*
	 * Update the object's metadata.
	 *
	 * If the object is the metadata, the call to __ae_meta_ckptlist_set
	...
	if (F_ISSET(conn, AE_CONN_CKPT_SYNC) &&
	    ...
	    !F_ISSET(&session->txn, AE_TXN_RUNNING)))
		AE_ERR(__ae_checkpoint_sync(session, NULL));
	 ...
	 */
	if (!fake_ckpt) {
		if (AE_META_TRACKING(session) && is_checkpoint)
			AE_ERR(__ae_meta_track_checkpoint(session));
		else
			AE_ERR(bm->checkpoint_resolve(bm, session));
	}

	/* Tell logging that the checkpoint is complete. */
	if (FLD_ISSET(conn->log_flags, AE_CONN_LOG_ENABLED))
		AE_ERR(__ae_txn_checkpoint_log(
		    session, false, AE_TXN_LOG_CKPT_STOP, NULL));
 ...
 */
int
__ae_checkpoint(AE_SESSION_IMPL *session, const char *cfg[])
{
...
/*
 * __ae_checkpoint_sync --
 *	Sync a file that has been checkpointed, and wait for the result.
 */
int
__ae_checkpoint_sync(AE_SESSION_IMPL *session, const char *cfg[])
{
	AE_BM *bm;
	...
	AE_ASSERT(session, bm != NULL);

	return (bm->sync(bm, session, false));
}

/*
 * __ae_checkpoint_close --
 *	Checkpoint a single file as part of closing the handle.
 */
int
__ae_checkpoint_close(AE_SESSION_IMPL *session, bool final)
{
	AE_BTREE *btree;
	AE_DECL_RET;
	...
	btree = S2BT(session);
	bulk = F_ISSET(btree, AE_BTREE_BULK);
	...
	if (F_ISSET(session->dhandle, AE_DHANDLE_DEAD))
		return (__ae_cache_op(session, NULL, AE_SYNC_DISCARD));
	...
	if (!btree->modified && !bulk) {
		__ae_txn_update_oldest(session, true);
		return (__ae_txn_visible_all(session, btree->rec_max_txn) ?
		    __ae_cache_op(session, NULL, AE_SYNC_DISCARD) : EBUSY);
	}
	 ...
	 */
	AE_ASSERT(session,
	    final || bulk || F_ISSET(session, AE_SESSION_LOCKED_SCHEMA));
	 ...
	 */
	need_tracking = !AE_META_TRACKING(session) && !bulk && !final;

	if (need_tracking)
		AE_RET(__ae_meta_track_on(session));

	AE_TRET(__checkpoint_worker(session, NULL, false, need_tracking));

	if (need_tracking)
		AE_RET(__ae_meta_track_off(session, true, ret != 0));
