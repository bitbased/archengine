/*-
 * Copyright (c) 2014-2015 MongoDB, Inc.
 * Copyright (c) 2008-2014 ArchEngine, Inc.
 *	All rights reserved.
 *
 * See the file LICENSE for redistribution information.
 */

#include "ae_internal.h"

static int __session_checkpoint(AE_SESSION *, const char *);
static int __session_snapshot(AE_SESSION *, const char *);
static int __session_rollback_transaction(AE_SESSION *, const char *);

/*
 * __ae_session_reset_cursors --
 *	Reset all open cursors.
 */
int
__ae_session_reset_cursors(AE_SESSION_IMPL *session, bool free_buffers)
{
	AE_CURSOR *cursor;
	AE_DECL_RET;

	TAILQ_FOREACH(cursor, &session->cursors, q) {
		/* Stop when there are no positioned cursors. */
		if (session->ncursors == 0)
			break;
		AE_TRET(cursor->reset(cursor));
		/* Optionally, free the cursor buffers */
		if (free_buffers) {
			__ae_buf_free(session, &cursor->key);
			__ae_buf_free(session, &cursor->value);
		}
	}

	AE_ASSERT(session, session->ncursors == 0);
	return (ret);
}

/*
 * __ae_session_copy_values --
 *	Copy values into all positioned cursors, so that they don't keep
 *	transaction IDs pinned.
 */
int
__ae_session_copy_values(AE_SESSION_IMPL *session)
{
	AE_CURSOR *cursor;
	AE_DECL_RET;

	TAILQ_FOREACH(cursor, &session->cursors, q)
		if (F_ISSET(cursor, AE_CURSTD_VALUE_INT)) {
			F_CLR(cursor, AE_CURSTD_VALUE_INT);
			AE_RET(__ae_buf_set(session, &cursor->value,
			    cursor->value.data, cursor->value.size));
			F_SET(cursor, AE_CURSTD_VALUE_EXT);
		}

	return (ret);
}

/*
 * __ae_session_release_resources --
 *	Release common session resources.
 */
int
__ae_session_release_resources(AE_SESSION_IMPL *session)
{
	AE_DECL_RET;

	/* Block manager cleanup */
	if (session->block_manager_cleanup != NULL)
		AE_TRET(session->block_manager_cleanup(session));

	/* Reconciliation cleanup */
	if (session->reconcile_cleanup != NULL)
		AE_TRET(session->reconcile_cleanup(session));

	/*
	 * Discard scratch buffers, error memory; last, just in case a cleanup
	 * routine uses scratch buffers.
	 */
	__ae_scr_discard(session);
	__ae_buf_free(session, &session->err);

	return (ret);
}

/*
 * __session_clear --
 *	Clear a session structure.
 */
static void
__session_clear(AE_SESSION_IMPL *session)
{
	/*
	 * There's no serialization support around the review of the hazard
	 * array, which means threads checking for hazard pointers first check
	 * the active field (which may be 0) and then use the hazard pointer
	 * (which cannot be NULL).
	 *
	 * Additionally, the session structure can include information that
	 * persists past the session's end-of-life, stored as part of page
	 * splits.
	 *
	 * For these reasons, be careful when clearing the session structure.
	 */
	memset(session, 0, AE_SESSION_CLEAR_SIZE(session));
	session->hazard_size = 0;
	session->nhazard = 0;
	AE_INIT_LSN(&session->bg_sync_lsn);
}

/*
 * __session_close --
 *	AE_SESSION->close method.
 */
static int
__session_close(AE_SESSION *ae_session, const char *config)
{
	AE_CONNECTION_IMPL *conn;
	AE_CURSOR *cursor;
	AE_DECL_RET;
	AE_SESSION_IMPL *session;

	conn = (AE_CONNECTION_IMPL *)ae_session->connection;
	session = (AE_SESSION_IMPL *)ae_session;

	SESSION_API_CALL(session, close, config, cfg);
	AE_UNUSED(cfg);

	/* Rollback any active transaction. */
	if (F_ISSET(&session->txn, AE_TXN_RUNNING))
		AE_TRET(__session_rollback_transaction(ae_session, NULL));

	/*
	 * Also release any pinned transaction ID from a non-transactional
	 * operation.
	 */
	if (conn->txn_global.states != NULL)
		__ae_txn_release_snapshot(session);

	/* Close all open cursors. */
	while ((cursor = TAILQ_FIRST(&session->cursors)) != NULL) {
		/*
		 * Notify the user that we are closing the cursor handle
		 * via the registered close callback.
		 */
		if (session->event_handler->handle_close != NULL &&
		    !AE_STREQ(cursor->uri, AE_LAS_URI))
			AE_TRET(session->event_handler->handle_close(
			    session->event_handler, ae_session, cursor));
		AE_TRET(cursor->close(cursor));
	}

	AE_ASSERT(session, session->ncursors == 0);

	/* Discard cached handles. */
	__ae_session_close_cache(session);

	/* Close all tables. */
	AE_TRET(__ae_schema_close_tables(session));

	/* Confirm we're not holding any hazard pointers. */
	__ae_hazard_close(session);

	/* Discard metadata tracking. */
	__ae_meta_track_discard(session);

	/* Free transaction information. */
	__ae_txn_destroy(session);

	/* Release common session resources. */
	AE_TRET(__ae_session_release_resources(session));

	/* Destroy the thread's mutex. */
	AE_TRET(__ae_cond_destroy(session, &session->cond));

	/* The API lock protects opening and closing of sessions. */
	__ae_spin_lock(session, &conn->api_lock);

	/* Decrement the count of open sessions. */
	AE_STAT_FAST_CONN_DECR(session, session_open);

	/*
	 * Sessions are re-used, clear the structure: the clear sets the active
	 * field to 0, which will exclude the hazard array from review by the
	 * eviction thread. Because some session fields are accessed by other
	 * threads, the structure must be cleared carefully.
	 *
	 * We don't need to publish here, because regardless of the active field
	 * being non-zero, the hazard pointer is always valid.
	 */
	__session_clear(session);
	session = conn->default_session;

	/*
	 * Decrement the count of active sessions if that's possible: a session
	 * being closed may or may not be at the end of the array, step toward
	 * the beginning of the array until we reach an active session.
	 */
	while (conn->sessions[conn->session_cnt - 1].active == 0)
		if (--conn->session_cnt == 0)
			break;

	__ae_spin_unlock(session, &conn->api_lock);

	/* We no longer have a session, don't try to update it. */
	session = NULL;

err:	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __session_reconfigure --
 *	AE_SESSION->reconfigure method.
 */
static int
__session_reconfigure(AE_SESSION *ae_session, const char *config)
{
	AE_CONFIG_ITEM cval;
	AE_DECL_RET;
	AE_SESSION_IMPL *session;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL(session, reconfigure, config, cfg);

	if (F_ISSET(&session->txn, AE_TXN_RUNNING))
		AE_ERR_MSG(session, EINVAL, "transaction in progress");

	AE_TRET(__ae_session_reset_cursors(session, false));

	AE_ERR(__ae_config_gets_def(session, cfg, "isolation", 0, &cval));
	if (cval.len != 0)
		session->isolation = session->txn.isolation =
		    AE_STRING_MATCH("snapshot", cval.str, cval.len) ?
		    AE_ISO_SNAPSHOT :
		    AE_STRING_MATCH("read-uncommitted", cval.str, cval.len) ?
		    AE_ISO_READ_UNCOMMITTED : AE_ISO_READ_COMMITTED;

err:	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __session_open_cursor_int --
 *	Internal version of AE_SESSION::open_cursor, with second cursor arg.
 */
static int
__session_open_cursor_int(AE_SESSION_IMPL *session, const char *uri,
    AE_CURSOR *owner, AE_CURSOR *other, const char *cfg[], AE_CURSOR **cursorp)
{
	AE_COLGROUP *colgroup;
	AE_DATA_SOURCE *dsrc;
	AE_DECL_RET;

	*cursorp = NULL;

	/*
	 * Open specific cursor types we know about, or call the generic data
	 * source open function.
	 *
	 * Unwind a set of string comparisons into a switch statement hoping
	 * the compiler can make it fast, but list the common choices first
	 * instead of sorting so if/else patterns are still fast.
	 */
	switch (uri[0]) {
	/*
	 * Common cursor types.
	 */
	case 't':
		if (AE_PREFIX_MATCH(uri, "table:"))
			AE_RET(__ae_curtable_open(
			    session, uri, owner, cfg, cursorp));
		break;
	case 'c':
		if (AE_PREFIX_MATCH(uri, "colgroup:")) {
			/*
			 * Column groups are a special case: open a cursor on
			 * the underlying data source.
			 */
			AE_RET(__ae_schema_get_colgroup(
			    session, uri, false, NULL, &colgroup));
			AE_RET(__ae_open_cursor(
			    session, colgroup->source, owner, cfg, cursorp));
		} else if (AE_PREFIX_MATCH(uri, "config:"))
			AE_RET(__ae_curconfig_open(
			    session, uri, cfg, cursorp));
		break;
	case 'i':
		if (AE_PREFIX_MATCH(uri, "index:"))
			AE_RET(__ae_curindex_open(
			    session, uri, owner, cfg, cursorp));
		break;
	case 'j':
		if (AE_PREFIX_MATCH(uri, "join:"))
			AE_RET(__ae_curjoin_open(
			    session, uri, owner, cfg, cursorp));
		break;
	case 'l':
		if (AE_PREFIX_MATCH(uri, "lsm:"))
			AE_RET(__ae_clsm_open(
			    session, uri, owner, cfg, cursorp));
		else if (AE_PREFIX_MATCH(uri, "log:"))
			AE_RET(__ae_curlog_open(session, uri, cfg, cursorp));
		break;

	/*
	 * Less common cursor types.
	 */
	case 'f':
		if (AE_PREFIX_MATCH(uri, "file:"))
			AE_RET(__ae_curfile_open(
			    session, uri, owner, cfg, cursorp));
		break;
	case 'm':
		if (AE_PREFIX_MATCH(uri, AE_METADATA_URI))
			AE_RET(__ae_curmetadata_open(
			    session, uri, owner, cfg, cursorp));
		break;
	case 'b':
		if (AE_PREFIX_MATCH(uri, "backup:"))
			AE_RET(__ae_curbackup_open(
			    session, uri, cfg, cursorp));
		break;
	case 's':
		if (AE_PREFIX_MATCH(uri, "statistics:"))
			AE_RET(__ae_curstat_open(session, uri, other, cfg,
			    cursorp));
		break;
	default:
		break;
	}

	if (*cursorp == NULL &&
	    (dsrc = __ae_schema_get_source(session, uri)) != NULL)
		AE_RET(dsrc->open_cursor == NULL ?
		    __ae_object_unsupported(session, uri) :
		    __ae_curds_open(session, uri, owner, cfg, dsrc, cursorp));

	if (*cursorp == NULL)
		return (__ae_bad_object_type(session, uri));

	/*
	 * When opening simple tables, the table code calls this function on the
	 * underlying data source, in which case the application's URI has been
	 * copied.
	 */
	if ((*cursorp)->uri == NULL &&
	    (ret = __ae_strdup(session, uri, &(*cursorp)->uri)) != 0) {
		AE_TRET((*cursorp)->close(*cursorp));
		*cursorp = NULL;
	}

	return (ret);
}

/*
 * __ae_open_cursor --
 *	Internal version of AE_SESSION::open_cursor.
 */
int
__ae_open_cursor(AE_SESSION_IMPL *session,
    const char *uri, AE_CURSOR *owner, const char *cfg[], AE_CURSOR **cursorp)
{
	return (__session_open_cursor_int(session, uri, owner, NULL, cfg,
	    cursorp));
}

/*
 * __session_open_cursor --
 *	AE_SESSION->open_cursor method.
 */
static int
__session_open_cursor(AE_SESSION *ae_session,
    const char *uri, AE_CURSOR *to_dup, const char *config, AE_CURSOR **cursorp)
{
	AE_CURSOR *cursor;
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	bool statjoin;

	cursor = *cursorp = NULL;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL(session, open_cursor, config, cfg);

	statjoin = (to_dup != NULL && uri != NULL &&
	    AE_STREQ(uri, "statistics:join"));
	if ((to_dup == NULL && uri == NULL) ||
	    (to_dup != NULL && uri != NULL && !statjoin))
		AE_ERR_MSG(session, EINVAL,
		    "should be passed either a URI or a cursor to duplicate, "
		    "but not both");

	if (to_dup != NULL && !statjoin) {
		uri = to_dup->uri;
		if (!AE_PREFIX_MATCH(uri, "colgroup:") &&
		    !AE_PREFIX_MATCH(uri, "index:") &&
		    !AE_PREFIX_MATCH(uri, "file:") &&
		    !AE_PREFIX_MATCH(uri, "lsm:") &&
		    !AE_PREFIX_MATCH(uri, AE_METADATA_URI) &&
		    !AE_PREFIX_MATCH(uri, "table:") &&
		    __ae_schema_get_source(session, uri) == NULL)
			AE_ERR(__ae_bad_object_type(session, uri));
	}

	AE_ERR(__session_open_cursor_int(session, uri, NULL,
	    statjoin ? to_dup : NULL, cfg, &cursor));
	if (to_dup != NULL && !statjoin)
		AE_ERR(__ae_cursor_dup_position(to_dup, cursor));

	*cursorp = cursor;

	if (0) {
err:		if (cursor != NULL)
			AE_TRET(cursor->close(cursor));
	}

	/*
	 * Opening a cursor on a non-existent data source will set ret to
	 * either of ENOENT or AE_NOTFOUND at this point.  However,
	 * applications may reasonably do this inside a transaction to check
	 * for the existence of a table or index.
	 *
	 * Prefer AE_NOTFOUND here: that does not force running transactions to
	 * roll back.  It will be mapped back to ENOENT.
	 */
	if (ret == ENOENT)
		ret = AE_NOTFOUND;

	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __ae_session_create --
 *	Internal version of AE_SESSION::create.
 */
int
__ae_session_create(
    AE_SESSION_IMPL *session, const char *uri, const char *config)
{
	AE_DECL_RET;

	AE_WITH_SCHEMA_LOCK(session,
	    AE_WITH_TABLE_LOCK(session,
		ret = __ae_schema_create(session, uri, config)));
	return (ret);
}

/*
 * __session_create --
 *	AE_SESSION->create method.
 */
static int
__session_create(AE_SESSION *ae_session, const char *uri, const char *config)
{
	AE_CONFIG_ITEM cval;
	AE_DECL_RET;
	AE_SESSION_IMPL *session;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL(session, create, config, cfg);
	AE_UNUSED(cfg);

	/* Disallow objects in the ArchEngine name space. */
	AE_ERR(__ae_str_name_check(session, uri));

	/*
	 * Type configuration only applies to tables, column groups and indexes.
	 * We don't want applications to attempt to layer LSM on top of their
	 * extended data-sources, and the fact we allow LSM as a valid URI is an
	 * invitation to that mistake: nip it in the bud.
	 */
	if (!AE_PREFIX_MATCH(uri, "colgroup:") &&
	    !AE_PREFIX_MATCH(uri, "index:") &&
	    !AE_PREFIX_MATCH(uri, "table:")) {
		/*
		 * We can't disallow type entirely, a configuration string might
		 * innocently include it, for example, a dump/load pair.  If the
		 * URI type prefix and the type are the same, let it go.
		 */
		if ((ret =
		    __ae_config_getones(session, config, "type", &cval)) == 0 &&
		    (strncmp(uri, cval.str, cval.len) != 0 ||
		    uri[cval.len] != ':'))
			AE_ERR_MSG(session, EINVAL,
			    "%s: unsupported type configuration", uri);
		AE_ERR_NOTFOUND_OK(ret);
	}

	ret = __ae_session_create(session, uri, config);

err:	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __session_log_flush --
 *	AE_SESSION->log_flush method.
 */
static int
__session_log_flush(AE_SESSION *ae_session, const char *config)
{
	AE_CONFIG_ITEM cval;
	AE_CONNECTION_IMPL *conn;
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	uint32_t flags;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL(session, log_flush, config, cfg);
	AE_STAT_FAST_CONN_INCR(session, log_flush);

	conn = S2C(session);
	flags = 0;
	/*
	 * If logging is not enabled there is nothing to do.
	 */
	if (!FLD_ISSET(conn->log_flags, AE_CONN_LOG_ENABLED))
		AE_ERR_MSG(session, EINVAL, "logging not enabled");

	AE_ERR(__ae_config_gets_def(session, cfg, "sync", 0, &cval));
	if (AE_STRING_MATCH("background", cval.str, cval.len))
		flags = AE_LOG_BACKGROUND;
	else if (AE_STRING_MATCH("off", cval.str, cval.len))
		flags = AE_LOG_FLUSH;
	else if (AE_STRING_MATCH("on", cval.str, cval.len))
		flags = AE_LOG_FSYNC;
	ret = __ae_log_flush(session, flags);

err:	API_END_RET(session, ret);
}

/*
 * __session_log_printf --
 *	AE_SESSION->log_printf method.
 */
static int
__session_log_printf(AE_SESSION *ae_session, const char *fmt, ...)
    AE_GCC_FUNC_ATTRIBUTE((format (printf, 2, 3)))
{
	AE_SESSION_IMPL *session;
	AE_DECL_RET;
	va_list ap;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL_NOCONF(session, log_printf);

	va_start(ap, fmt);
	ret = __ae_log_vprintf(session, fmt, ap);
	va_end(ap);

err:	API_END_RET(session, ret);
}

/*
 * __session_rename --
 *	AE_SESSION->rename method.
 */
static int
__session_rename(AE_SESSION *ae_session,
    const char *uri, const char *newuri, const char *config)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL(session, rename, config, cfg);

	/* Disallow objects in the ArchEngine name space. */
	AE_ERR(__ae_str_name_check(session, uri));
	AE_ERR(__ae_str_name_check(session, newuri));

	AE_WITH_SCHEMA_LOCK(session,
	    AE_WITH_TABLE_LOCK(session,
		ret = __ae_schema_rename(session, uri, newuri, cfg)));

err:	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __session_reset --
 *	AE_SESSION->reset method.
 */
static int
__session_reset(AE_SESSION *ae_session)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;

	session = (AE_SESSION_IMPL *)ae_session;

	SESSION_API_CALL_NOCONF(session, reset);

	if (F_ISSET(&session->txn, AE_TXN_RUNNING))
		AE_ERR_MSG(session, EINVAL, "transaction in progress");

	AE_TRET(__ae_session_reset_cursors(session, true));

	/* Release common session resources. */
	AE_TRET(__ae_session_release_resources(session));

err:	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __ae_session_drop --
 *	Internal version of AE_SESSION::drop.
 */
int
__ae_session_drop(AE_SESSION_IMPL *session, const char *uri, const char *cfg[])
{
	AE_DECL_RET;

	AE_WITH_SCHEMA_LOCK(session,
	    AE_WITH_TABLE_LOCK(session,
		ret = __ae_schema_drop(session, uri, cfg)));
	return (ret);
}

/*
 * __session_drop --
 *	AE_SESSION->drop method.
 */
static int
__session_drop(AE_SESSION *ae_session, const char *uri, const char *config)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL(session, drop, config, cfg);

	/* Disallow objects in the ArchEngine name space. */
	AE_ERR(__ae_str_name_check(session, uri));

	ret = __ae_session_drop(session, uri, cfg);

err:	/* Note: drop operations cannot be unrolled (yet?). */
	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __session_join --
 *	AE_SESSION->join method.
 */
static int
__session_join(AE_SESSION *ae_session, AE_CURSOR *join_cursor,
    AE_CURSOR *ref_cursor, const char *config)
{
	AE_CONFIG_ITEM cval;
	AE_CURSOR_INDEX *cindex;
	AE_CURSOR_JOIN *cjoin;
	AE_CURSOR_TABLE *ctable;
	AE_DECL_RET;
	AE_INDEX *idx;
	AE_SESSION_IMPL *session;
	AE_TABLE *table;
	uint64_t count;
	uint32_t bloom_bit_count, bloom_hash_count;
	uint8_t flags, range;

	count = 0;
	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL(session, join, config, cfg);
	table = NULL;

	if (!AE_PREFIX_MATCH(join_cursor->uri, "join:"))
		AE_ERR_MSG(session, EINVAL, "not a join cursor");

	if (AE_PREFIX_MATCH(ref_cursor->uri, "index:")) {
		cindex = (AE_CURSOR_INDEX *)ref_cursor;
		idx = cindex->index;
		table = cindex->table;
		AE_CURSOR_CHECKKEY(ref_cursor);
	} else if (AE_PREFIX_MATCH(ref_cursor->uri, "table:")) {
		idx = NULL;
		ctable = (AE_CURSOR_TABLE *)ref_cursor;
		table = ctable->table;
		AE_CURSOR_CHECKKEY(ctable->cg_cursors[0]);
	} else
		AE_ERR_MSG(session, EINVAL, "not an index or table cursor");

	cjoin = (AE_CURSOR_JOIN *)join_cursor;
	if (cjoin->table != table)
		AE_ERR_MSG(session, EINVAL,
		    "table for join cursor does not match table for index");
	if (F_ISSET(ref_cursor, AE_CURSTD_JOINED))
		AE_ERR_MSG(session, EINVAL,
		    "index cursor already used in a join");

	/* "ge" is the default */
	range = AE_CURJOIN_END_GT | AE_CURJOIN_END_EQ;
	flags = 0;
	AE_ERR(__ae_config_gets(session, cfg, "compare", &cval));
	if (cval.len != 0) {
		if (AE_STRING_MATCH("gt", cval.str, cval.len))
			range = AE_CURJOIN_END_GT;
		else if (AE_STRING_MATCH("lt", cval.str, cval.len))
			range = AE_CURJOIN_END_LT;
		else if (AE_STRING_MATCH("le", cval.str, cval.len))
			range = AE_CURJOIN_END_LE;
		else if (AE_STRING_MATCH("eq", cval.str, cval.len))
			range = AE_CURJOIN_END_EQ;
		else if (!AE_STRING_MATCH("ge", cval.str, cval.len))
			AE_ERR(EINVAL);
	}
	AE_ERR(__ae_config_gets(session, cfg, "count", &cval));
	if (cval.len != 0)
		count = (uint64_t)cval.val;

	AE_ERR(__ae_config_gets(session, cfg, "strategy", &cval));
	if (cval.len != 0) {
		if (AE_STRING_MATCH("bloom", cval.str, cval.len))
			LF_SET(AE_CURJOIN_ENTRY_BLOOM);
		else if (!AE_STRING_MATCH("default", cval.str, cval.len))
			AE_ERR(EINVAL);
	}
	AE_ERR(__ae_config_gets(session, cfg, "bloom_bit_count", &cval));
	if ((uint64_t)cval.val > UINT32_MAX)
		AE_ERR_MSG(session, EINVAL,
		    "bloom_bit_count: value too large");
	bloom_bit_count = (uint32_t)cval.val;
	AE_ERR(__ae_config_gets(session, cfg, "bloom_hash_count", &cval));
	if ((uint64_t)cval.val > UINT32_MAX)
		AE_ERR_MSG(session, EINVAL,
		    "bloom_hash_count: value too large");
	bloom_hash_count = (uint32_t)cval.val;
	if (LF_ISSET(AE_CURJOIN_ENTRY_BLOOM)) {
		if (count == 0)
			AE_ERR_MSG(session, EINVAL,
			    "count must be nonzero when strategy=bloom");
		if (cjoin->entries_next == 0)
			AE_ERR_MSG(session, EINVAL,
			    "the first joined cursor cannot specify "
			    "strategy=bloom");
	}
	AE_ERR(__ae_curjoin_join(session, cjoin, idx, ref_cursor, flags,
	    range, count, bloom_bit_count, bloom_hash_count));
	/*
	 * There's an implied ownership ordering that isn't
	 * known when the cursors are created: the join cursor
	 * must be closed before any of the indices.  Enforce
	 * that here by reordering.
	 */
	if (TAILQ_FIRST(&session->cursors) != join_cursor) {
		TAILQ_REMOVE(&session->cursors, join_cursor, q);
		TAILQ_INSERT_HEAD(&session->cursors, join_cursor, q);
	}
	/* Disable the reference cursor for regular operations */
	F_SET(ref_cursor, AE_CURSTD_JOINED);

err:	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __session_salvage --
 *	AE_SESSION->salvage method.
 */
static int
__session_salvage(AE_SESSION *ae_session, const char *uri, const char *config)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;

	session = (AE_SESSION_IMPL *)ae_session;

	SESSION_API_CALL(session, salvage, config, cfg);

	if (F_ISSET(S2C(session), AE_CONN_IN_MEMORY))
		AE_ERR(ENOTSUP);

	/* Block out checkpoints to avoid spurious EBUSY errors. */
	AE_WITH_CHECKPOINT_LOCK(session,
	    AE_WITH_SCHEMA_LOCK(session, ret =
		__ae_schema_worker(session, uri, __ae_salvage,
		NULL, cfg, AE_DHANDLE_EXCLUSIVE | AE_BTREE_SALVAGE)));

err:	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __session_truncate --
 *	AE_SESSION->truncate method.
 */
static int
__session_truncate(AE_SESSION *ae_session,
    const char *uri, AE_CURSOR *start, AE_CURSOR *stop, const char *config)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	AE_CURSOR *cursor;
	int cmp;
	bool local_start;

	local_start = false;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_TXN_API_CALL(session, truncate, config, cfg);
	AE_STAT_FAST_CONN_INCR(session, cursor_truncate);

	/*
	 * If the URI is specified, we don't need a start/stop, if start/stop
	 * is specified, we don't need a URI.  One exception is the log URI
	 * which may truncate (archive) log files for a backup cursor.
	 *
	 * If no URI is specified, and both cursors are specified, start/stop
	 * must reference the same object.
	 *
	 * Any specified cursor must have been initialized.
	 */
	if ((uri == NULL && start == NULL && stop == NULL) ||
	    (uri != NULL && !AE_PREFIX_MATCH(uri, "log:") &&
	    (start != NULL || stop != NULL)))
		AE_ERR_MSG(session, EINVAL,
		    "the truncate method should be passed either a URI or "
		    "start/stop cursors, but not both");

	if (uri != NULL) {
		/* Disallow objects in the ArchEngine name space. */
		AE_ERR(__ae_str_name_check(session, uri));

		if (AE_PREFIX_MATCH(uri, "log:")) {
			/*
			 * Verify the user only gave the URI prefix and not
			 * a specific target name after that.
			 */
			if (!AE_STREQ(uri, "log:"))
				AE_ERR_MSG(session, EINVAL,
				    "the truncate method should not specify any"
				    "target after the log: URI prefix.");
			ret = __ae_log_truncate_files(session, start, cfg);
		} else
			/* Wait for checkpoints to avoid EBUSY errors. */
			AE_WITH_CHECKPOINT_LOCK(session,
			    AE_WITH_SCHEMA_LOCK(session,
				ret = __ae_schema_truncate(session, uri, cfg)));
		goto done;
	}

	/*
	 * Cursor truncate is only supported for some objects, check for the
	 * supporting methods we need, range_truncate and compare.
	 */
	cursor = start == NULL ? stop : start;
	if (cursor->compare == NULL)
		AE_ERR(__ae_bad_object_type(session, cursor->uri));

	/*
	 * If both cursors set, check they're correctly ordered with respect to
	 * each other.  We have to test this before any search, the search can
	 * change the initial cursor position.
	 *
	 * Rather happily, the compare routine will also confirm the cursors
	 * reference the same object and the keys are set.
	 */
	if (start != NULL && stop != NULL) {
		AE_ERR(start->compare(start, stop, &cmp));
		if (cmp > 0)
			AE_ERR_MSG(session, EINVAL,
			    "the start cursor position is after the stop "
			    "cursor position");
	}

	/*
	 * Truncate does not require keys actually exist so that applications
	 * can discard parts of the object's name space without knowing exactly
	 * what records currently appear in the object.  For this reason, do a
	 * search-near, rather than a search.  Additionally, we have to correct
	 * after calling search-near, to position the start/stop cursors on the
	 * next record greater than/less than the original key.
	 */
	if (start != NULL) {
		AE_ERR(start->search_near(start, &cmp));
		if (cmp < 0 && (ret = start->next(start)) != 0) {
			AE_ERR_NOTFOUND_OK(ret);
			goto done;
		}
	}
	if (stop != NULL) {
		AE_ERR(stop->search_near(stop, &cmp));
		if (cmp > 0 && (ret = stop->prev(stop)) != 0) {
			AE_ERR_NOTFOUND_OK(ret);
			goto done;
		}
	}

	/*
	 * We always truncate in the forward direction because the underlying
	 * data structures can move through pages faster forward than backward.
	 * If we don't have a start cursor, create one and position it at the
	 * first record.
	 */
	if (start == NULL) {
		AE_ERR(__session_open_cursor(
		    ae_session, stop->uri, NULL, NULL, &start));
		local_start = true;
		AE_ERR(start->next(start));
	}

	/*
	 * If the start/stop keys cross, we're done, the range must be empty.
	 */
	if (stop != NULL) {
		AE_ERR(start->compare(start, stop, &cmp));
		if (cmp > 0)
			goto done;
	}

	AE_ERR(__ae_schema_range_truncate(session, start, stop));

done:
err:	TXN_API_END_RETRY(session, ret, 0);

	/*
	 * Close any locally-opened start cursor.
	 */
	if (local_start)
		AE_TRET(start->close(start));

	/*
	 * Only map AE_NOTFOUND to ENOENT if a URI was specified.
	 */
	return (ret == AE_NOTFOUND && uri != NULL ? ENOENT : ret);
}

/*
 * __session_upgrade --
 *	AE_SESSION->upgrade method.
 */
static int
__session_upgrade(AE_SESSION *ae_session, const char *uri, const char *config)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;

	session = (AE_SESSION_IMPL *)ae_session;

	SESSION_API_CALL(session, upgrade, config, cfg);
	/* Block out checkpoints to avoid spurious EBUSY errors. */
	AE_WITH_CHECKPOINT_LOCK(session,
	    AE_WITH_SCHEMA_LOCK(session,
		ret = __ae_schema_worker(session, uri, __ae_upgrade,
		NULL, cfg, AE_DHANDLE_EXCLUSIVE | AE_BTREE_UPGRADE)));

err:	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __session_verify --
 *	AE_SESSION->verify method.
 */
static int
__session_verify(AE_SESSION *ae_session, const char *uri, const char *config)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;

	session = (AE_SESSION_IMPL *)ae_session;

	SESSION_API_CALL(session, verify, config, cfg);

	if (F_ISSET(S2C(session), AE_CONN_IN_MEMORY))
		AE_ERR(ENOTSUP);

	/* Block out checkpoints to avoid spurious EBUSY errors. */
	AE_WITH_CHECKPOINT_LOCK(session,
	    AE_WITH_SCHEMA_LOCK(session,
		ret = __ae_schema_worker(session, uri, __ae_verify,
		NULL, cfg, AE_DHANDLE_EXCLUSIVE | AE_BTREE_VERIFY)));

err:	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __session_begin_transaction --
 *	AE_SESSION->begin_transaction method.
 */
static int
__session_begin_transaction(AE_SESSION *ae_session, const char *config)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL(session, begin_transaction, config, cfg);
	AE_STAT_FAST_CONN_INCR(session, txn_begin);

	if (F_ISSET(&session->txn, AE_TXN_RUNNING))
		AE_ERR_MSG(session, EINVAL, "Transaction already running");

	ret = __ae_txn_begin(session, cfg);

err:	API_END_RET(session, ret);
}

/*
 * __session_commit_transaction --
 *	AE_SESSION->commit_transaction method.
 */
static int
__session_commit_transaction(AE_SESSION *ae_session, const char *config)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	AE_TXN *txn;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL(session, commit_transaction, config, cfg);
	AE_STAT_FAST_CONN_INCR(session, txn_commit);

	txn = &session->txn;
	if (F_ISSET(txn, AE_TXN_ERROR) && txn->mod_count != 0)
		AE_ERR_MSG(session, EINVAL,
		    "failed transaction requires rollback");

	if (ret == 0)
		ret = __ae_txn_commit(session, cfg);
	else {
		AE_TRET(__ae_session_reset_cursors(session, false));
		AE_TRET(__ae_txn_rollback(session, cfg));
	}

err:	API_END_RET(session, ret);
}

/*
 * __session_rollback_transaction --
 *	AE_SESSION->rollback_transaction method.
 */
static int
__session_rollback_transaction(AE_SESSION *ae_session, const char *config)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL(session, rollback_transaction, config, cfg);
	AE_STAT_FAST_CONN_INCR(session, txn_rollback);

	AE_TRET(__ae_session_reset_cursors(session, false));

	AE_TRET(__ae_txn_rollback(session, cfg));

err:	API_END_RET(session, ret);
}

/*
 * __session_transaction_pinned_range --
 *	AE_SESSION->transaction_pinned_range method.
 */
static int
__session_transaction_pinned_range(AE_SESSION *ae_session, uint64_t *prange)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	AE_TXN_STATE *txn_state;
	uint64_t pinned;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL_NOCONF(session, pinned_range);

	txn_state = AE_SESSION_TXN_STATE(session);

	/* Assign pinned to the lesser of id or snap_min */
	if (txn_state->id != AE_TXN_NONE &&
	    AE_TXNID_LT(txn_state->id, txn_state->snap_min))
		pinned = txn_state->id;
	else
		pinned = txn_state->snap_min;

	if (pinned == AE_TXN_NONE)
		*prange = 0;
	else
		*prange = S2C(session)->txn_global.current - pinned;

err:	API_END_RET(session, ret);
}

/*
 * __session_transaction_sync --
 *	AE_SESSION->transaction_sync method.
 */
static int
__session_transaction_sync(AE_SESSION *ae_session, const char *config)
{
	AE_CONFIG_ITEM cval;
	AE_CONNECTION_IMPL *conn;
	AE_DECL_RET;
	AE_LOG *log;
	AE_SESSION_IMPL *session;
	AE_TXN *txn;
	struct timespec now, start;
	uint64_t timeout_ms, waited_ms;
	bool forever;

	session = (AE_SESSION_IMPL *)ae_session;
	SESSION_API_CALL(session, transaction_sync, config, cfg);
	AE_STAT_FAST_CONN_INCR(session, txn_sync);

	conn = S2C(session);
	txn = &session->txn;
	if (F_ISSET(txn, AE_TXN_RUNNING))
		AE_ERR_MSG(session, EINVAL, "transaction in progress");

	/*
	 * If logging is not enabled there is nothing to do.
	 */
	if (!FLD_ISSET(conn->log_flags, AE_CONN_LOG_ENABLED))
		AE_ERR_MSG(session, EINVAL, "logging not enabled");

	log = conn->log;
	timeout_ms = waited_ms = 0;
	forever = true;

	/*
	 * If there is no background sync LSN in this session, there
	 * is nothing to do.
	 */
	if (AE_IS_INIT_LSN(&session->bg_sync_lsn))
		goto err;

	/*
	 * If our LSN is smaller than the current sync LSN then our
	 * transaction is stable.  We're done.
	 */
	if (__ae_log_cmp(&session->bg_sync_lsn, &log->sync_lsn) <= 0)
		goto err;

	/*
	 * Our LSN is not yet stable.  Wait and check again depending on the
	 * timeout.
	 */
	AE_ERR(__ae_config_gets_def(
	    session, cfg, "timeout_ms", (int)UINT_MAX, &cval));
	if ((unsigned int)cval.val != UINT_MAX) {
		timeout_ms = (uint64_t)cval.val;
		forever = false;
	}

	if (timeout_ms == 0)
		AE_ERR(ETIMEDOUT);

	AE_ERR(__ae_epoch(session, &start));
	/*
	 * Keep checking the LSNs until we find it is stable or we reach
	 * our timeout.
	 */
	while (__ae_log_cmp(&session->bg_sync_lsn, &log->sync_lsn) > 0) {
		AE_ERR(__ae_cond_signal(session, conn->log_file_cond));
		AE_ERR(__ae_epoch(session, &now));
		waited_ms = AE_TIMEDIFF_MS(now, start);
		if (forever || waited_ms < timeout_ms)
			/*
			 * Note, we will wait an increasing amount of time
			 * each iteration, likely doubling.  Also note that
			 * the function timeout value is in usecs (we are
			 * computing the wait time in msecs and passing that
			 * in, unchanged, as the usecs to wait).
			 */
			AE_ERR(__ae_cond_wait(
			    session, log->log_sync_cond, waited_ms));
		else
			AE_ERR(ETIMEDOUT);
	}

err:	API_END_RET(session, ret);
}

/*
 * __session_checkpoint --
 *	AE_SESSION->checkpoint method.
 */
static int
__session_checkpoint(AE_SESSION *ae_session, const char *config)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	AE_TXN *txn;

	session = (AE_SESSION_IMPL *)ae_session;

	AE_STAT_FAST_CONN_INCR(session, txn_checkpoint);
	SESSION_API_CALL(session, checkpoint, config, cfg);

	if (F_ISSET(S2C(session), AE_CONN_IN_MEMORY))
		AE_ERR(ENOTSUP);

	/*
	 * Checkpoints require a snapshot to write a transactionally consistent
	 * snapshot of the data.
	 *
	 * We can't use an application's transaction: if it has uncommitted
	 * changes, they will be written in the checkpoint and may appear after
	 * a crash.
	 *
	 * Use a real snapshot transaction: we don't want any chance of the
	 * snapshot being updated during the checkpoint.  Eviction is prevented
	 * from evicting anything newer than this because we track the oldest
	 * transaction ID in the system that is not visible to all readers.
	 */
	txn = &session->txn;
	if (F_ISSET(txn, AE_TXN_RUNNING))
		AE_ERR_MSG(session, EINVAL,
		    "Checkpoint not permitted in a transaction");

	ret = __ae_txn_checkpoint(session, cfg);

	/*
	 * Release common session resources (for example, checkpoint may acquire
	 * significant reconciliation structures/memory).
	 */
	AE_TRET(__ae_session_release_resources(session));

err:	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __session_snapshot --
 *	AE_SESSION->snapshot method.
 */
static int
__session_snapshot(AE_SESSION *ae_session, const char *config)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	AE_TXN_GLOBAL *txn_global;
	bool has_create, has_drop;

	has_create = has_drop = false;
	session = (AE_SESSION_IMPL *)ae_session;
	txn_global = &S2C(session)->txn_global;

	SESSION_API_CALL(session, snapshot, config, cfg);

	AE_ERR(__ae_txn_named_snapshot_config(
	    session, cfg, &has_create, &has_drop));

	AE_ERR(__ae_writelock(session, txn_global->nsnap_rwlock));

	/* Drop any snapshots to be removed first. */
	if (has_drop)
		AE_ERR(__ae_txn_named_snapshot_drop(session, cfg));

	/* Start the named snapshot if requested. */
	if (has_create)
		AE_ERR(__ae_txn_named_snapshot_begin(session, cfg));

err:	AE_TRET(__ae_writeunlock(session, txn_global->nsnap_rwlock));

	API_END_RET_NOTFOUND_MAP(session, ret);
}

/*
 * __session_strerror --
 *	AE_SESSION->strerror method.
 */
static const char *
__session_strerror(AE_SESSION *ae_session, int error)
{
	AE_SESSION_IMPL *session;

	session = (AE_SESSION_IMPL *)ae_session;

	return (__ae_strerror(session, error, NULL, 0));
}

/*
 * __open_session --
 *	Allocate a session handle.
 */
static int
__open_session(AE_CONNECTION_IMPL *conn,
    AE_EVENT_HANDLER *event_handler, const char *config,
    AE_SESSION_IMPL **sessionp)
{
	static const AE_SESSION stds = {
		NULL,
		NULL,
		__session_close,
		__session_reconfigure,
		__session_strerror,
		__session_open_cursor,
		__session_create,
		__ae_session_compact,
		__session_drop,
		__session_join,
		__session_log_flush,
		__session_log_printf,
		__session_rename,
		__session_reset,
		__session_salvage,
		__session_truncate,
		__session_upgrade,
		__session_verify,
		__session_begin_transaction,
		__session_commit_transaction,
		__session_rollback_transaction,
		__session_checkpoint,
		__session_snapshot,
		__session_transaction_pinned_range,
		__session_transaction_sync
	};
	AE_DECL_RET;
	AE_SESSION_IMPL *session, *session_ret;
	uint32_t i;

	*sessionp = NULL;

	session = conn->default_session;
	session_ret = NULL;

	__ae_spin_lock(session, &conn->api_lock);

	/*
	 * Make sure we don't try to open a new session after the application
	 * closes the connection.  This is particularly intended to catch
	 * cases where server threads open sessions.
	 */
	AE_ASSERT(session, F_ISSET(conn, AE_CONN_SERVER_RUN));

	/* Find the first inactive session slot. */
	for (session_ret = conn->sessions,
	    i = 0; i < conn->session_size; ++session_ret, ++i)
		if (!session_ret->active)
			break;
	if (i == conn->session_size)
		AE_ERR_MSG(session, ENOMEM,
		    "only configured to support %" PRIu32 " sessions"
		    " (including %d additional internal sessions)",
		    conn->session_size, AE_EXTRA_INTERNAL_SESSIONS);

	/*
	 * If the active session count is increasing, update it.  We don't worry
	 * about correcting the session count on error, as long as we don't mark
	 * this session as active, we'll clean it up on close.
	 */
	if (i >= conn->session_cnt)	/* Defend against off-by-one errors. */
		conn->session_cnt = i + 1;

	session_ret->id = i;
	session_ret->iface = stds;
	session_ret->iface.connection = &conn->iface;

	AE_ERR(__ae_cond_alloc(session, "session", false, &session_ret->cond));

	if (AE_SESSION_FIRST_USE(session_ret))
		__ae_random_init(&session_ret->rnd);

	__ae_event_handler_set(session_ret,
	    event_handler == NULL ? session->event_handler : event_handler);

	TAILQ_INIT(&session_ret->cursors);
	TAILQ_INIT(&session_ret->dhandles);
	/*
	 * If we don't have one, allocate the dhandle hash array.
	 * Allocate the table hash array as well.
	 */
	if (session_ret->dhhash == NULL)
		AE_ERR(__ae_calloc(session_ret, AE_HASH_ARRAY_SIZE,
		    sizeof(struct __dhandles_hash), &session_ret->dhhash));
	if (session_ret->tablehash == NULL)
		AE_ERR(__ae_calloc(session_ret, AE_HASH_ARRAY_SIZE,
		    sizeof(struct __tables_hash), &session_ret->tablehash));
	for (i = 0; i < AE_HASH_ARRAY_SIZE; i++) {
		TAILQ_INIT(&session_ret->dhhash[i]);
		TAILQ_INIT(&session_ret->tablehash[i]);
	}

	/* Initialize transaction support: default to read-committed. */
	session_ret->isolation = AE_ISO_READ_COMMITTED;
	AE_ERR(__ae_txn_init(session_ret));

	/*
	 * The session's hazard pointer memory isn't discarded during normal
	 * session close because access to it isn't serialized.  Allocate the
	 * first time we open this session.
	 */
	if (AE_SESSION_FIRST_USE(session_ret))
		AE_ERR(__ae_calloc_def(
		    session, conn->hazard_max, &session_ret->hazard));

	/*
	 * Set an initial size for the hazard array. It will be grown as
	 * required up to hazard_max. The hazard_size is reset on close, since
	 * __ae_hazard_close ensures the array is cleared - so it is safe to
	 * reset the starting size on each open.
	 */
	session_ret->hazard_size = AE_HAZARD_INCR;

	/*
	 * Configuration: currently, the configuration for open_session is the
	 * same as session.reconfigure, so use that function.
	 */
	if (config != NULL)
		AE_ERR(
		    __session_reconfigure((AE_SESSION *)session_ret, config));

	session_ret->name = NULL;

	/*
	 * Publish: make the entry visible to server threads.  There must be a
	 * barrier for two reasons, to ensure structure fields are set before
	 * any other thread will consider the session, and to push the session
	 * count to ensure the eviction thread can't review too few slots.
	 */
	AE_PUBLISH(session_ret->active, 1);

	AE_STATIC_ASSERT(offsetof(AE_SESSION_IMPL, iface) == 0);
	*sessionp = session_ret;

	AE_STAT_FAST_CONN_INCR(session, session_open);

err:	__ae_spin_unlock(session, &conn->api_lock);
	return (ret);
}

/*
 * __ae_open_session --
 *	Allocate a session handle.
 */
int
__ae_open_session(AE_CONNECTION_IMPL *conn,
    AE_EVENT_HANDLER *event_handler, const char *config,
    bool open_metadata, AE_SESSION_IMPL **sessionp)
{
	AE_DECL_RET;
	AE_SESSION_IMPL *session;
	AE_SESSION *ae_session;

	*sessionp = NULL;

	/* Acquire a session. */
	AE_RET(__open_session(conn, event_handler, config, &session));

	/*
	 * Acquiring the metadata handle requires the schema lock; we've seen
	 * problems in the past where a session has acquired the schema lock
	 * unexpectedly, relatively late in the run, and deadlocked. Be
	 * defensive, get it now.  The metadata file may not exist when the
	 * connection first creates its default session or the shared cache
	 * pool creates its sessions, let our caller decline this work.
	 */
	if (open_metadata) {
		AE_ASSERT(session, !F_ISSET(session, AE_SESSION_LOCKED_SCHEMA));
		if ((ret = __ae_metadata_open(session)) != 0) {
			ae_session = &session->iface;
			AE_TRET(ae_session->close(ae_session, NULL));
			return (ret);
		}
	}

	*sessionp = session;
	return (0);
}

/*
 * __ae_open_internal_session --
 *	Allocate a session for ArchEngine's use.
 */
int
__ae_open_internal_session(AE_CONNECTION_IMPL *conn, const char *name,
    bool open_metadata, uint32_t session_flags, AE_SESSION_IMPL **sessionp)
{
	AE_DECL_RET;
	AE_SESSION *ae_session;
	AE_SESSION_IMPL *session;

	*sessionp = NULL;

	/* Acquire a session. */
	AE_RET(__ae_open_session(conn, NULL, NULL, open_metadata, &session));
	session->name = name;

	/*
	 * Public sessions are automatically closed during AE_CONNECTION->close.
	 * If the session handles for internal threads were to go on the public
	 * list, there would be complex ordering issues during close.  Set a
	 * flag to avoid this: internal sessions are not closed automatically.
	 */
	F_SET(session, session_flags | AE_SESSION_INTERNAL);

	/*
	 * Acquiring the lookaside table cursor requires various locks; we've
	 * seen problems in the past where deadlocks happened because sessions
	 * deadlocked getting the cursor late in the process.  Be defensive,
	 * get it now.
	 */
	if (F_ISSET(session, AE_SESSION_LOOKASIDE_CURSOR)) {
		AE_WITHOUT_DHANDLE(session, ret =
		    __ae_las_cursor_create(session, &session->las_cursor));
		if (ret != 0) {
			ae_session = &session->iface;
			AE_TRET(ae_session->close(ae_session, NULL));
			return (ret);
		}
	}

	*sessionp = session;
	return (0);
}
