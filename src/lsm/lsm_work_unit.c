...
static int __lsm_bloom_create(
    AE_SESSION_IMPL *, AE_LSM_TREE *, AE_LSM_CHUNK *, u_int);
static int __lsm_discard_handle(AE_SESSION_IMPL *, const char *, const char *);
 ...
 */
static int
__lsm_copy_chunks(AE_SESSION_IMPL *session,
    AE_LSM_TREE *lsm_tree, AE_LSM_WORKER_COOKIE *cookie, bool old_chunks)
{
	AE_DECL_RET;
	u_int i, nchunks;
	size_t alloc;
	...
	if (!F_ISSET(lsm_tree, AE_LSM_TREE_ACTIVE))
		return (__ae_lsm_tree_readunlock(session, lsm_tree));
	 ...
	 */
	for (i = 0; i < nchunks; i++)
		(void)__ae_atomic_add32(&cookie->chunk_array[i]->refcnt, 1);

err:	AE_TRET(__ae_lsm_tree_readunlock(session, lsm_tree));
...
/*
 * __ae_lsm_get_chunk_to_flush --
 *	Find and pin a chunk in the LSM tree that is likely to need flushing.
 */
int
__ae_lsm_get_chunk_to_flush(AE_SESSION_IMPL *session,
    AE_LSM_TREE *lsm_tree, bool force, AE_LSM_CHUNK **chunkp)
{
	AE_DECL_RET;
	AE_LSM_CHUNK *chunk, *evict_chunk, *flush_chunk;
	u_int i;
	...
	*chunkp = NULL;
	...
	AE_RET(__ae_lsm_tree_readlock(session, lsm_tree));
	if (!F_ISSET(lsm_tree, AE_LSM_TREE_ACTIVE) || lsm_tree->nchunks == 0)
		return (__ae_lsm_tree_readunlock(session, lsm_tree));
	...
	for (i = 0; i < lsm_tree->nchunks; i++) {
		...
		if (F_ISSET(chunk, AE_LSM_CHUNK_ONDISK)) {
			...
			if (evict_chunk == NULL &&
			    !chunk->evicted &&
			    !F_ISSET(chunk, AE_LSM_CHUNK_STABLE))
				evict_chunk = chunk;
	...
	if (evict_chunk != NULL && flush_chunk != NULL) {
		chunk = (__ae_random(&session->rnd) & 1) ?
...
err:	AE_RET(__ae_lsm_tree_readunlock(session, lsm_tree));
 ...
 */
static void
__lsm_unpin_chunks(AE_SESSION_IMPL *session, AE_LSM_WORKER_COOKIE *cookie)
{
	u_int i;
	...
	for (i = 0; i < cookie->nchunks; i++) {
		if (cookie->chunk_array[i] == NULL)
			continue;
		AE_ASSERT(session, cookie->chunk_array[i]->refcnt > 0);
		(void)__ae_atomic_sub32(&cookie->chunk_array[i]->refcnt, 1);
	}
...
/*
 * __ae_lsm_work_switch --
 *	Do a switch if the LSM tree needs one.
 */
int
__ae_lsm_work_switch(
    AE_SESSION_IMPL *session, AE_LSM_WORK_UNIT **entryp, bool *ran)
{
	AE_DECL_RET;
	...
	*entryp = NULL;
	...
	if (F_ISSET(entry->lsm_tree, AE_LSM_TREE_NEED_SWITCH)) {
		AE_WITH_SCHEMA_LOCK(session,
		    ret = __ae_lsm_tree_switch(session, entry->lsm_tree));
		/* Failing to complete the switch is fine */
		if (ret == EBUSY) {
			if (F_ISSET(entry->lsm_tree, AE_LSM_TREE_NEED_SWITCH))
				AE_ERR(__ae_lsm_manager_push_entry(session,
				    AE_LSM_WORK_SWITCH, 0, entry->lsm_tree));
			ret = 0;
		} else
			*ran = true;
	}
 ...
 */
int
__ae_lsm_work_bloom(AE_SESSION_IMPL *session, AE_LSM_TREE *lsm_tree)
{
	AE_DECL_RET;
	AE_LSM_CHUNK *chunk;
	AE_LSM_WORKER_COOKIE cookie;
	u_int i, merge;

	AE_CLEAR(cookie);

	AE_RET(__lsm_copy_chunks(session, lsm_tree, &cookie, false));
	...
	for (i = 0; i < cookie.nchunks; i++) {
		 ...
		 */
		if (!F_ISSET(chunk, AE_LSM_CHUNK_ONDISK) ||
		    F_ISSET(chunk, AE_LSM_CHUNK_BLOOM | AE_LSM_CHUNK_MERGING) ||
		    chunk->generation > 0 ||
		    chunk->count == 0)
			continue;
		 ...
		 */
		if (__ae_atomic_cas32(&chunk->bloom_busy, 0, 1)) {
...
err:
	...
	__ae_free(session, cookie.chunk_array);
	return (ret);
}

/*
 * __ae_lsm_checkpoint_chunk --
 *	Flush a single LSM chunk to disk.
 */
int
__ae_lsm_checkpoint_chunk(AE_SESSION_IMPL *session,
    AE_LSM_TREE *lsm_tree, AE_LSM_CHUNK *chunk)
{
	AE_DECL_RET;
	...
	if (F_ISSET(chunk, AE_LSM_CHUNK_ONDISK) &&
		...
		if (ret == 0)
			...
			AE_RET_MSG(session, ret, "discard handle");
	}
	if (F_ISSET(chunk, AE_LSM_CHUNK_ONDISK)) {
	...
	if (chunk->switch_txn == AE_TXN_NONE ||
	    !__ae_txn_visible_all(session, chunk->switch_txn)) {
		AE_RET(__ae_verbose(session, AE_VERB_LSM,
	...
	if (!__ae_atomic_cas8(&chunk->flushing, 0, 1))
		return (0);
	flush_set = true;

	AE_ERR(__ae_verbose(session, AE_VERB_LSM, "LSM worker flushing %s",
	    chunk->uri));
	...
	if ((ret = __ae_session_get_btree(
		...
		ret = __ae_cache_op(session, NULL, AE_SYNC_WRITE_LEAVES);
		session->txn.isolation = saved_isolation;
		AE_TRET(__ae_session_release_btree(session));
	}
	AE_ERR(ret);

	AE_ERR(__ae_verbose(session, AE_VERB_LSM, "LSM worker checkpointing %s",
	    chunk->uri));
	...
	AE_WITH_SCHEMA_LOCK(session, ret = __ae_schema_worker(
	    session, chunk->uri, __ae_checkpoint, NULL, NULL, 0));
	AE_TRET(__ae_meta_track_off(session, false, ret != 0));
	if (ret != 0)
		AE_ERR_MSG(session, ret, "LSM checkpoint");
	...
	ret = __ae_lsm_meta_write(session, lsm_tree);
	++lsm_tree->dsk_gen;
	...
	if (ret != 0)
		AE_ERR_MSG(session, ret, "LSM metadata write");

	AE_PUBLISH(chunk->flushing, 0);
	flush_set = false;
	 ...
	 * forced eviction.
	 */
	AE_ERR(__ae_session_get_btree(session, chunk->uri, NULL, NULL, 0));
	...
	AE_ERR(__ae_verbose(session, AE_VERB_LSM, "LSM worker checkpointed %s",
	    chunk->uri));
 ...
 */
static int
__lsm_bloom_create(AE_SESSION_IMPL *session,
    AE_LSM_TREE *lsm_tree, AE_LSM_CHUNK *chunk, u_int chunk_off)
{
	AE_BLOOM *bloom;
	AE_CURSOR *src;
	AE_DECL_RET;
	...
	AE_RET(__ae_lsm_tree_setup_bloom(session, lsm_tree, chunk));
	...
	AE_RET(__ae_bloom_create(session, chunk->bloom_uri,
	...
	AE_ERR(__ae_clsm_init_merge(src, chunk_off, chunk->id, 1));
	...
	AE_CLEAR(key);
	AE_ERR_NOTFOUND_OK(__ae_bloom_get(bloom, &key));
	...
	if (ret != 0)
		AE_ERR_MSG(session, ret, "LSM bloom worker metadata write");
    ...
    AE_SESSION_IMPL *session, const char *uri, const char *checkpoint)
{
	...
	AE_RET(__ae_session_get_btree(session, uri, checkpoint, NULL,
	    AE_DHANDLE_EXCLUSIVE | AE_DHANDLE_LOCK_ONLY));

	F_SET(session->dhandle, AE_DHANDLE_DISCARD_FORCE);
 ...
 */
static int
__lsm_drop_file(AE_SESSION_IMPL *session, const char *uri)
{
	AE_DECL_RET;
	const char *drop_cfg[] = { AE_CONFIG_BASE(
	    session, AE_SESSION_drop), "remove_files=false", NULL };
	 ...
	 */
	AE_WITH_HANDLE_LIST_LOCK(session,
	   ret = __lsm_discard_handle(session, uri, AE_CHECKPOINT));
	AE_RET(ret);

	/*
	 * Take the schema lock for the drop operation.  Since __ae_schema_drop
	 * results in the hot backup lock being taken when it updates the
	 * metadata (which would be too late to prevent our drop).
	 */
	AE_WITH_SCHEMA_LOCK(session,
	    ret = __ae_schema_drop(session, uri, drop_cfg));

	if (ret == 0)
		ret = __ae_remove(session, uri + strlen("file:"));
	AE_RET(__ae_verbose(session, AE_VERB_LSM, "Dropped %s", uri));
...
/*
 * __ae_lsm_free_chunks --
 *	Try to drop chunks from the tree that are no longer required.
 */
int
__ae_lsm_free_chunks(AE_SESSION_IMPL *session, AE_LSM_TREE *lsm_tree)
{
	AE_DECL_RET;
	AE_LSM_CHUNK *chunk;
	AE_LSM_WORKER_COOKIE cookie;
	 ...
	 * at any time.
	 */
	if (!__ae_atomic_cas32(&lsm_tree->freeing_old_chunks, 0, 1))
		return (0);
	/*
	 ...
	 */
	AE_CLEAR(cookie);
	AE_RET(__lsm_copy_chunks(session, lsm_tree, &cookie, true));
	for (i = skipped = 0; i < cookie.nchunks; i++) {
		...
		if (F_ISSET(chunk, AE_LSM_CHUNK_BLOOM)) {
			...
			flush_metadata = true;
			F_CLR(chunk, AE_LSM_CHUNK_BLOOM);
		}
		if (chunk->uri != NULL) {
		 ...
		 */
		AE_ASSERT(session, lsm_tree->old_chunks[skipped] == chunk);
		__ae_free(session, chunk->bloom_uri);
		__ae_free(session, chunk->uri);
		__ae_free(session, lsm_tree->old_chunks[skipped]);
...
err:	/* Flush the metadata unless the system is in panic */
	if (flush_metadata && ret != AE_PANIC) {
		AE_TRET(__ae_lsm_tree_writelock(session, lsm_tree));
		AE_TRET(__ae_lsm_meta_write(session, lsm_tree));
		AE_TRET(__ae_lsm_tree_writeunlock(session, lsm_tree));
	}
	__lsm_unpin_chunks(session, &cookie);
	__ae_free(session, cookie.chunk_array);
